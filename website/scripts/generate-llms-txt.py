#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Allr docs site.
...
Both publish at:
  https://allr.work/docs/llms.txt
  https://allr.work/docs/llms-full.txt
...
SITE_BASE = "https://allr.work/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Allr"),
...
def emit_llms_index() -> str:
    ...
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/allr-ajmx/"
        "allr-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/allr-ajmx/allr-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Allr — Full Documentation\n",
        (
            "This file is the entire Allr documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Allr, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://allr.work/docs\n",
        "Short index: https://allr.work/docs/llms.txt\n",
        "\n---\n\n",
    ]
