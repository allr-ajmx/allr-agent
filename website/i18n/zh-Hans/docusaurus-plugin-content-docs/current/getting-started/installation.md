...
sidebar_position: 2
title: "安装"
description: "在 Linux、macOS、WSL2、原生 Windows 或通过 Termux 在 Android 上安装 Allr"
---

# 安装

使用一行安装命令，两分钟内即可启动并运行 Allr。
...
2. 否则，它会下载便携式 **PortableGit**（约 50MB，来自官方 `git-for-windows` GitHub 发布页）并解压到 `%LOCALAPPDATA%\allr\git`。无需管理员权限，完全隔离——不会干扰任何系统 Git 安装，无论其状态如何。（在 32 位 Windows 上会回退到 MinGit，因为 PortableGit 仅提供 64 位和 ARM64 资产；依赖 bash 的 Allr 功能在 32 位主机上无法使用。）

**为什么不使用 winget？** 早期设计通过 `winget install Git.Git` 自动安装 Git，但当系统 Git 安装处于部分损坏状态时，winget 会严重失败（而这恰恰是用户最需要安装程序正常工作的时候）。便携式 Git 方案绕过了 winget、Windows 安装程序注册表以及任何现有系统 Git。如果 Allr 的 Git 安装本身出现问题，执行 `Remove-Item %LOCALAPPDATA%\allr\git` 并重新运行安装程序即可——对系统无影响，无需卸载操作。

安装程序还会将 `ALLR_GIT_BASH_PATH` 设置为找到的 `bash.exe` 路径，以便 Allr 在新 shell 中确定性地解析它。

如果你偏好 WSL2，上方的 Linux 安装程序可在其中运行；原生安装和 WSL 安装可以共存而不冲突（原生数据位于 `%LOCALAPPDATA%\allr`，WSL 数据位于 `~/.allr`）。

**桌面安装程序（替代方案）：** 也提供一个轻量 GUI 安装程序——下载 Allr Desktop，运行 `.exe`，首次启动时它会在后台调用 `install.ps1` 来配置 Python（通过 `uv`）、Node、PortableGit 及其余依赖。桌面应用和 PowerShell 安装的 CLI 共享相同的安装目录和数据目录，可以单独或同时使用。详见 [Windows（原生）指南](../user-guide/windows-native#desktop-installer-alternative)。

### Android / Termux

Allr 现在也提供 Termux 感知的安装路径：
...
- **CLI（`allr chat`、`allr setup`、`allr gateway` 等）** — 原生，使用默认终端
...
如果遇到编码相关的 bug 并希望回退到旧版 cp1252 stdio 路径（用于问题定位），请在环境中设置 `ALLR_DISABLE_WINDOWS_UTF8=1`。
:::
...
Root 模式的 **FHS 布局**（`/usr/local/lib/…`、`/usr/local/bin/allr`）与其他系统级开发工具在 Linux 上的安装位置一致。适用于共享机器部署场景，一次系统安装可服务所有用户。每个用户的个人配置（认证、技能、会话）仍位于各自的 `~/.allr/` 或显式指定的 `ALLR_HOME` 下。
...
allr tools          # 配置启用的工具
allr gateway setup  # 配置消息平台
allr config set     # 设置单个配置项
allr setup          # 或运行完整的设置向导一次性配置所有内容
```
...
| `API key not set`           | 运行 `allr model` 配置提供商，或 `allr config set OPENROUTER_API_KEY your_key` |
| 更新后配置丢失              | 运行 `allr config check`，然后运行 `allr config migrate`                       |

如需更多诊断信息，运行 `allr doctor`——它会告诉你确切缺少什么以及如何修复。

## 安装方式自动检测

Allr 会自动检测安装方式（git 安装程序、Docker 或 NixOS），`allr update` 会打印对应路径的更新命令。无需设置任何环境变量——检测基于安装目录结构（`~/.allr/allr-agent/` 检出、Docker 镜像标记或 Nix store 路径）。`allr doctor` 也会在其环境摘要中显示检测到的安装方式。
