...
- 需要逐步执行某个函数并观察集合的变化
- 长期运行的进程（allr gateway、tui_gateway）出现异常且无法重启
...
适用于长期运行的进程：Allr gateway、tui_gateway、daemon，或已出现异常且无法干净重启的进程。

### 安装

```bash
source /home/bb/allr-agent/.venv/bin/activate
pip install debugpy
```
...
最简便的终端侧 DAP 客户端是 VS Code CLI 或一个小脚本。在 Allr 内部有两个实用选项：
...
```json
{
  ...
  "justMyCode": false,
  "pathMappings": [
    { "localRoot": "${workspaceFolder}", "remoteRoot": "/home/bb/allr-agent" }
  ]
}
```
...
9. **fork / 多进程。** pdb 不会跟随 fork。每个子进程需要自己的 `breakpoint()` 或 `set_trace()`。对于 Allr 子 agent，每次只调试一个进程。
