...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.allr/.env ] && grep -q "^GITHUB_TOKEN=" ~/.allr/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.allr/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
    elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
      GITHUB_TOKEN=$(uv run python3 "${ALLR_HOME:-$HOME/.allr}/skills/github/github-auth/scripts/git-credential-token.py")
    fi
  fi
fi
...
```bash
source "${ALLR_HOME:-$HOME/.allr}/skills/github/github-auth/scripts/gh-env.sh"
...
```bash
# 若无问题——批准
gh pr review $PR_NUMBER --approve --body "Reviewed by Allr. Code looks clean — good test coverage, no security concerns."
