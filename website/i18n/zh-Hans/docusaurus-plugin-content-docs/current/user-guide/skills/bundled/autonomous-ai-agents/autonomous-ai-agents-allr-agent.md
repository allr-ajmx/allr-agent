---
title: "Allr — 配置、扩展或贡献 Allr"
sidebar_label: "Allr"
description: "配置、扩展或贡献 Allr"
---
...
|---|---|
| 来源 | 内置（默认安装） |
| 路径 | `skills/autonomous-ai-agents/allr-agent` |
| 版本 | `2.1.0` |
| 作者 | Allr + Teknium |
| 许可证 | MIT |
...
:::info
以下是 Allr 在触发此 skill 时加载的完整 skill 定义。这是 agent 在 skill 激活时看到的指令内容。
:::

# Allr

Allr 是 Nous Research 开发的开源 AI agent 框架，可在终端、消息平台和 IDE 中运行。它与 Claude Code（Anthropic）、Codex（OpenAI）和 OpenClaw 同属一类——使用工具调用（tool calling）与系统交互的自主编码和任务执行 agent。Allr 支持任意 LLM 提供商（OpenRouter、Anthropic、OpenAI、DeepSeek、本地模型及 15+ 其他提供商），可在 Linux、macOS 和 WSL 上运行。

Allr 的差异化特性：

- **通过 skill 自我提升** — Allr 通过将可复用流程保存为 skill 来从经验中学习。当它解决复杂问题、发现工作流或被纠正时，可以将该知识持久化为 skill 文档，加载到未来的会话中。skill 随时间积累，使 agent 在你的特定任务和环境中表现越来越好。
...
- **Profiles（配置文件）** — 运行多个独立的 Allr 实例，各自拥有隔离的配置、会话、skill 和记忆。
- **可扩展** — 插件、MCP 服务器、自定义工具、webhook 触发器、cron 调度以及完整的 Python 生态系统。

人们将 Allr 用于软件开发、研究、系统管理、数据分析、内容创作、家庭自动化，以及任何受益于具有持久上下文和完整系统访问权限的 AI agent 的场景。

**此 skill 帮助你高效使用 Allr** — 包括设置、配置功能、生成额外的 agent 实例、排查问题、找到正确的命令和设置，以及在需要扩展或贡献时理解系统的工作原理。

**文档：** https://allr.work/docs/
...
```
allr setup [section]      Interactive wizard (model|terminal|gateway|tools|agent)
allr model                Interactive model/provider picker
allr config               View current config
allr config edit          Open config.yaml in $EDITOR
allr config set KEY VAL   Set a config value
allr config path          Print config.yaml path
allr config env-path      Print .env path
allr config check         Check for missing/outdated config
allr config migrate       Update config with new options
allr auth                 交互式凭据管理器
allr auth add PROVIDER    添加 OAuth 或 API key 凭据（例如 nous、openai-codex、qwen-oauth）
allr auth list            列出已存储的凭据
allr auth remove PROVIDER 移除已存储的凭据
allr doctor [--fix]       Check dependencies and config
allr status [--all]       Show component status
```

### 工具与 Skill

```
allr tools                Interactive tool enable/disable (curses UI)
allr tools list           Show all tools and status
allr tools enable NAME    Enable a toolset
allr tools disable NAME   Disable a toolset

allr skills list          List installed skills
allr skills search QUERY  Search the skills hub
allr skills install ID    Install a skill (ID can be a hub identifier OR a direct https://…/SKILL.md URL; pass --name to override when frontmatter has no name)
allr skills inspect ID    Preview without installing
allr skills config        Enable/disable skills per platform
allr skills check         Check for updates
allr skills update        Update outdated skills
allr skills uninstall N   Remove a hub skill
allr skills publish PATH  Publish to registry
allr skills browse        Browse all available skills
allr skills tap add REPO  Add a GitHub repo as skill source
```
...
allr mcp add NAME         Add an MCP server (--url or --command)
allr mcp remove NAME      Remove an MCP server
allr mcp list             List configured servers
allr mcp test NAME        Test connection
...
```
allr gateway run          Start gateway foreground
allr gateway install      Install as background service
allr gateway start/stop   Control the service
allr gateway restart      Restart the service
allr gateway status       Check status
allr gateway setup        Configure platforms
```
...
平台文档：https://allr.work/docs/user-guide/messaging/

### 会话

```
allr sessions list        List recent sessions
allr sessions browse      Interactive picker
allr sessions export OUT  Export to JSONL
allr sessions rename ID T Rename a session
allr sessions delete ID   Delete a session
allr sessions prune       Clean up old sessions (--older-than N days)
allr sessions stats       Session store statistics
```

### Cron 任务

```
allr cron list            List jobs (--all for disabled)
allr cron create SCHED    Create: '30m', 'every 2h', '0 9 * * *'
allr cron edit ID         Edit schedule, prompt, delivery
allr cron pause/resume ID Control job state
allr cron run ID          Trigger on next tick
allr cron remove ID       Delete a job
allr cron status          Scheduler status
```

### Webhook

```
allr webhook subscribe N  Create route at /webhooks/<name>
allr webhook list         List subscriptions
allr webhook remove NAME  Remove a subscription
allr webhook test NAME    Send a test POST
```

### Profiles

```
allr profile list         List all profiles
allr profile create NAME  Create (--clone, --clone-all, --clone-from)
allr profile use NAME     Set sticky default
allr profile delete NAME  Delete a profile
allr profile show NAME    Show details
allr profile alias NAME   Manage wrapper scripts
allr profile rename A B   Rename a profile
allr profile export NAME  Export to tar.gz
allr profile import FILE  Import from archive
```

### 凭证池

```
allr auth add             Interactive credential wizard
allr auth list [PROVIDER] List pooled credentials
allr auth remove P INDEX  Remove by provider + index
allr auth reset PROVIDER  Clear exhaustion status
```

### 其他

```
allr insights [--days N]  Usage analytics
allr update               Update to latest version
allr pairing list/approve/revoke  DM authorization
allr plugins list/install/remove  Plugin management
allr honcho setup/status  Honcho memory integration (requires honcho plugin)
allr memory setup/status/off  Memory provider config
allr completion bash|zsh  Shell completions
allr acp                  ACP server (IDE integration)
allr claw migrate         Migrate from OpenClaw
...
在交互式聊天会话中输入这些命令。新命令会不定期上线；如果以下内容看起来过时，请在会话内运行 `/help` 获取权威列表，或查看[实时斜杠命令参考](https://allr.work/docs/reference/slash-commands)。命令注册表的权威来源是 `hermes_cli/commands.py` — 每个消费方（自动补全、Telegram 菜单、Slack 映射、`/help`）均从中派生。
...
/snapshot [sub]      Create or restore state snapshots of Allr config/state (CLI)
/background <prompt> Run prompt in background
/queue <prompt>      Queue for next turn
/steer <prompt>      Inject a message after the next tool call without interrupting
/agents (/tasks)     Show active agents and running tasks
/resume [name]       Resume a named session
/goal [text|sub]     Set a standing goal Allr works on across turns until achieved
...
/busy [sub]          Control what Enter does while Allr is working (CLI)
...
/reload-skills       Re-scan ~/.allr/skills/ for added/removed skills
...
/update              Update Allr to latest (gateway)
...
```
~/.allr/config.yaml       Main configuration
~/.allr/.env              API keys and secrets
$ALLR_HOME/skills/        Installed skills
~/.allr/sessions/         Session transcripts
...
Profiles 使用 `~/.allr/profiles/<name>/`，布局相同。

### 配置节

使用 `allr config edit` 或 `allr config set section.key value` 编辑。
...
完整配置参考：https://allr.work/docs/user-guide/configuration

### 提供商

支持 20+ 个提供商。通过 `allr model` 或 `allr setup` 设置。

| 提供商 | 认证方式 | Key 环境变量 |
|----------|------|-------------|
| OpenRouter | API key | `OPENROUTER_API_KEY` |
| Anthropic | API key | `ANTHROPIC_API_KEY` |
| Nous Portal | OAuth | `allr auth` |
| OpenAI Codex | OAuth | `allr auth` |
...
| Qwen OAuth | OAuth | `allr auth add qwen-oauth` |
| 自定义端点 | 配置 | `config.yaml` 中的 `model.base_url` + `model.api_key` |
| GitHub Copilot ACP | 外部 | `COPILOT_CLI_PATH` 或 Copilot CLI |

完整提供商文档：https://allr.work/docs/integrations/providers

### Toolset

通过 `allr tools`（交互式）或 `allr tools enable/disable NAME` 启用/禁用。
...
完整枚举位于 `toolsets.py` 的 `TOOLSETS` 字典中；`_ALLR_CORE_TOOLS` 是大多数平台继承的默认工具包。
...
密钥脱敏**默认关闭** — 工具输出（终端 stdout、`read_file`、网页内容、子 agent 摘要等）不经修改直接传递。如果用户希望 Allr 在 API key、token 和密钥进入对话上下文和日志之前自动屏蔽它们：

```bash
allr config set security.redact_secrets true       # 全局启用
```

**需要重启。** `security.redact_secrets` 在导入时快照 — 在会话中途切换（例如通过工具调用执行 `export ALLR_REDACT_SECRETS=true`）对正在运行的进程**不会**生效。告知用户在终端运行 `allr config set security.redact_secrets true`，然后启动新会话。这是有意为之——防止 LLM 在任务中途自行切换该开关。

再次禁用：
```bash
allr config set security.redact_secrets false
```
...
```bash
allr config set privacy.redact_pii true    # 启用
allr config set privacy.redact_pii false   # 禁用（默认）
```

### 命令审批提示

默认情况下（`approvals.mode: smart`），Allr 会让辅助 LLM 评估被标记为破坏性的 shell 命令（`rm -rf`、`git reset --hard` 等）。模式如下：
...
```bash
allr config set approvals.mode smart       # 推荐的折中方案
allr config set approvals.mode off         # 绕过一切（不推荐）
```
...
某些 shell hook 集成在触发前需要明确加入允许列表。通过 `~/.allr/shell-hooks-allowlist.json` 管理——在 hook 首次尝试运行时以交互方式提示。

### 禁用 web/browser/image-gen 工具

要完全阻止模型访问网络或媒体工具，打开 `allr tools` 并按平台切换。在下次会话（`/reset`）后生效。参见上方的工具与 Skill 部分。
...
将额外的 Allr 进程作为完全独立的子进程运行——拥有独立的会话、工具和环境。
...
Allr 使用 prompt_toolkit，需要真实终端。使用 tmux 进行交互式生成：
...
- **fire-and-forget 使用 `allr chat -q`** — 无需 PTY
...
持久化调度器——`cron/jobs.py` + `cron/scheduler.py`。通过 `cronjob` 工具、`allr cron` CLI（`list`、`add`、`edit`、`pause`、`resume`、`run`、`remove`）或 `/cron` 斜杠命令驱动。
...
用户文档：https://allr.work/docs/user-guide/features/cron

### Curator（skill 生命周期）

agent 创建的 skill 的后台维护。跟踪使用情况，将闲置 skill 标记为过时，归档过时的 skill，保留运行前的 tar.gz 备份以防数据丢失。

- **CLI：** `allr curator <verb>` — `status`、`run`、`pause`、`resume`、`pin`、`unpin`、`archive`、`restore`、`prune`、`backup`、`rollback`。
...
- **遥测：** `~/.allr/skills/.usage.json` 中的 sidecar 保存每个 skill 的 `use_count`、`view_count`、`patch_count`、`last_activity_at`、`state`、`pinned`。

配置：`curator.*`（`enabled`、`interval_hours`、`min_idle_hours`、`stale_after_days`、`archive_after_days`、`backup.*`）。
用户文档：https://allr.work/docs/user-guide/features/curator

### Kanban（多 agent 工作队列）

用于多 profile/多 worker 协作的持久化 SQLite 看板（kanban）。用户通过 `allr kanban <verb>` 驱动；调度器生成的 worker 看到由 `ALLR_KANBAN_TASK` 控制的专注 `kanban_*` toolset，orchestrator profile 可以选择加入更广泛的 `kanban` toolset。普通会话除非配置，否则没有任何 `kanban_*` schema 占用。
...
- **隔离：** 看板是硬边界（worker 在环境中固定 `ALLR_KANBAN_BOARD`）；租户是看板内用于工作区路径和记忆键隔离的软命名空间。

用户文档：https://allr.work/docs/user-guide/features/kanban

---

## Windows 特有问题

Allr 在 Windows 上原生运行（PowerShell、cmd、Windows Terminal、git-bash mintty、VS Code 集成终端）。大多数功能开箱即用，但 Win32 和 POSIX 之间有一些差异曾给我们带来麻烦——遇到新问题时请在此记录，以免下一个人（或下一个会话）重新踩坑。
...
**首次运行时 HTTP 400 "No models provided"。** `config.yaml` 保存时带有 UTF-8 BOM（Windows 应用写入时常见）。重新保存为不带 BOM 的 UTF-8。`allr config edit` 写入时不带 BOM；手动在记事本中编辑是常见原因。

### `execute_code` / 沙箱

**WinError 10106**（"无法加载或初始化请求的服务提供商"）来自沙箱子进程——它无法创建 `AF_INET` socket，因此回退的 loopback-TCP RPC 在 `connect()` 之前失败。根本原因通常**不是**损坏的 Winsock LSP；而是 Allr 自身的环境清理器从子进程环境中删除了 `SYSTEMROOT` / `WINDIR` / `COMSPEC`。Python 的 `socket` 模块需要 `SYSTEMROOT` 来定位 `mswsock.dll`。通过 `tools/code_execution_tool.py` 中的 `_WINDOWS_ESSENTIAL_ENV_VARS` 允许列表修复。如果仍然遇到此问题，在 `execute_code` 块内 echo `os.environ` 以确认 `SYSTEMROOT` 已设置。完整诊断方案见 `references/execute-code-sandbox-env-windows.md`。

### 测试/贡献

**`scripts/run_tests.sh` 在 Windows 上无法直接使用** — 它查找 POSIX venv 布局（`.venv/bin/activate`）。Allr 安装的 venv 位于 `venv/Scripts/`，也没有 pip 或 pytest（为减小安装体积而精简）。解决方案：将 `pytest + pyyaml` 安装到系统 Python 3.11 用户站点，然后设置 `PYTHONPATH` 直接调用 pytest：
...
**正斜杠几乎在所有地方都有效。** `C:/Users/...` 被每个 Allr 工具和大多数 Windows API 接受。在代码和日志中优先使用正斜杠——避免在 bash 中转义反斜杠。
...
### 工具不可用
1. `allr tools` — 检查 toolset 是否为你的平台启用
2. 某些工具需要环境变量（检查 `.env`）
3. 启用工具后执行 `/reset`

### 模型/提供商问题
1. `allr doctor` — 检查配置和依赖
2. `allr auth` — 重新认证 OAuth 提供商（或 `allr auth add <provider>`）
3. 检查 `.env` 中是否有正确的 API key
4. **Copilot 403**：`gh auth login` 的 token **不适用于** Copilot API。必须通过 `allr model` → GitHub Copilot 使用 Copilot 专用 OAuth 设备码流程。
...
首先检查日志：
```bash
grep -i "failed to send\|error" ~/.allr/logs/gateway.log | tail -20
```
...
- **Gateway 崩溃循环**：重置失败状态：`systemctl --user reset-failed allr-gateway`
...
```bash
allr config set auxiliary.vision.provider <your_provider>
allr config set auxiliary.vision.model <model_name>
```
...
| 查找内容... | 位置 |
|----------------|----------|
| 配置选项 | `allr config edit` 或[配置文档](https://allr.work/docs/user-guide/configuration) |
| 可用工具 | `allr tools list` 或[工具参考](https://allr.work/docs/reference/tools-reference) |
| 斜杠命令 | 会话内 `/help` 或[斜杠命令参考](https://allr.work/docs/reference/slash-commands) |
| Skill 目录 | `allr skills browse` 或[Skill 目录](https://allr.work/docs/reference/skills-catalog) |
| 提供商设置 | `allr model` 或[提供商指南](https://allr.work/docs/integrations/providers) |
| 平台设置 | `allr gateway setup` 或[消息文档](https://allr.work/docs/user-guide/messaging/) |
| MCP 服务器 | `allr mcp list` 或[MCP 指南](https://allr.work/docs/user-guide/features/mcp) |
| Profiles | `allr profile list` 或[Profiles 文档](https://allr.work/docs/user-guide/profiles) |
...
面向偶尔贡献者和 PR 作者。完整开发者文档：https://allr.work/docs/developer-guide/
...
配置：`~/.allr/config.yaml`（设置）、`~/.allr/.env`（API key）。
...
**2. 添加到 `toolsets.py`** → `_ALLR_CORE_TOOLS` 列表。

自动发现：任何包含顶层 `registry.register()` 调用的 `tools/*.py` 文件都会自动导入——无需手动列出。

所有处理器必须返回 JSON 字符串。路径使用 `get_hermes_home()`，永远不要硬编码 `~/.allr`。
