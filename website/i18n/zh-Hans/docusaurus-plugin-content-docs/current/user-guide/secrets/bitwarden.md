...
在进程启动时从 [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) 拉取 API 密钥，而不是以明文形式存储在 `~/.allr/.env` 中。一个引导密钥（机器账户访问令牌）替代了 N 个提供商密钥，轮换凭据只需在 Bitwarden Web 应用中修改一次即可。
...
2. 创建或选择一个**项目**（例如"Allr keys"）。
3. 将提供商密钥添加为 secret。secret 的**名称**将成为环境变量名——使用 `OPENROUTER_API_KEY`、`ANTHROPIC_API_KEY` 等。
4. **Machine accounts → New machine account → My Allr machine** → **Projects** 标签页 → 授予对你的项目的 Read 权限。
...
1. 下载并验证 `bws v2.0.0`，存放至 `~/.allr/bin/bws`。
2. 提示输入访问令牌（输入内容隐藏）。以 `BWS_ACCESS_TOKEN` 形式存储在 `~/.allr/.env` 中。
...
| 命令 | 功能 |
|---|---|
| `allr secrets bitwarden setup` | 交互式向导（安装二进制文件、提示输入令牌、选择项目、测试拉取） |
| `allr secrets bitwarden status` | 显示配置、二进制版本，以及令牌是否存在/是否通过校验 |
| `allr secrets bitwarden token` | 轮换访问令牌：先向 Bitwarden 验证新令牌，验证通过后再写入 `.env` |
| `allr secrets bitwarden sync` | 演习模式：立即拉取 secret 并显示将应用的内容 |
| `allr secrets bitwarden sync --apply` | 拉取并导出到当前 shell 的环境中 |
| `allr secrets bitwarden install` | 仅下载固定版本的 `bws` 二进制文件（无需认证） |
| `allr secrets bitwarden disable` | 将 `enabled` 设为 `false`；保留令牌和项目 ID |
...
```bash
allr secrets bitwarden token                     # 隐藏输入提示
allr secrets bitwarden token --access-token 0.…  # 非交互式
```
...
Bitwarden 永远不会阻塞 Allr 启动。如果出现任何问题，stderr 会显示一行警告，Allr 继续使用 `.env` 中已有的凭据：

| 现象 | 原因 | 修复方法 |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | 配置中已启用，但令牌已从 `.env` 中清除 | 重新运行 `allr secrets bitwarden setup` |
| `Bitwarden rejected the machine-account access token … invalid_client` | 令牌已吊销、过期、机器账户被删除——或令牌属于其他区域（例如欧盟令牌访问了美国 identity 端点） | 运行 `allr secrets bitwarden token` 粘贴新令牌；区域不匹配时重新运行 setup 选择欧盟/自托管（或设置 `secrets.bitwarden.server_url`） |
| `bws exited 1: invalid access token` | 令牌已吊销或有误 | 运行 `allr secrets bitwarden token` 提供新令牌 |
...
- 即使 `override_existing: true`，Allr 也会拒绝让 Bitwarden 覆盖引导令牌本身。如果你将 `BWS_ACCESS_TOKEN` 作为 secret 存储在项目中，应用时会静默跳过。
- `bws` 二进制文件的下载会与同一 GitHub release 中发布的 SHA-256 校验和进行验证。不匹配时将中止安装。
- 固定版本（撰写本文时为 `bws v2.0.0`）通过向本仓库提交 PR 的方式更新——Allr 不会将 `bws` 自动升级到"最新版本"，因为上游 release 的结构可能发生变化。

## 不适用场景

- **单机个人使用**，`~/.allr/.env` 已经够用。你只是用一个凭据换了另一个，并在启动时增加了网络依赖。
- **无法访问 `api.bitwarden.com` 的隔离环境**。
- **CI/CD** 场景，已有现成的 secret 注入机制（GitHub Actions secrets、Vault 等）——选择一种方式，不要两者并用。

适合使用此功能的场景：多机器集群、共享开发机、gateway VPS，或任何需要跨多个 Allr 安装进行集中轮换和吊销管理的场景。
