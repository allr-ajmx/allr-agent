...
在同一台机器上运行多个独立的 Allr agent——每个 agent 拥有各自的配置、API 密钥、记忆、会话、技能和 gateway 状态。

## 什么是 profile？

profile 是一个独立的 Allr 主目录。每个 profile 拥有自己的目录，其中包含各自的 `config.yaml`、`.env`、`SOUL.md`、记忆、会话、技能、cron 任务和状态数据库。profile 让你可以为不同用途运行独立的 agent——编程助手、个人机器人、研究 agent——而不会混淆 Allr 状态。
...
```bash
allr profile create coder       # 创建 profile + "coder" 命令别名
coder setup                       # 配置 API 密钥和模型
coder chat                        # 开始对话
```

就这些。`coder` 现在是拥有独立配置、记忆和状态的 Allr profile。
...
你也可以稍后通过 `allr profile describe` 设置或自动生成描述——完整路由模型请参阅 [Kanban 指南](./features/kanban#auto-vs-manual-orchestration)。
...
将当前 profile 的 `config.yaml`、`.env`、`SOUL.md` 和 skills 复制到新 profile。API 密钥、模型和能力相同，但会话和记忆是全新的。编辑 `~/.allr/profiles/work/.env` 可使用不同的 API 密钥，编辑 `~/.allr/profiles/work/SOUL.md` 可设置不同的人格。
...
复制**所有内容**——配置、API 密钥、人格、记忆、技能、cron 任务、插件。会排除每个 profile 自己的历史数据（会话历史、`state.db`、`backups/`、`state-snapshots/`、`checkpoints/`），这些数据属于源 profile 且可能达到数十 GB。若要包含历史的完整备份，请使用 `allr profile export` 或 `allr backup`。
...
### 粘性默认值（`allr profile use`）

```bash
allr profile use coder
allr chat                   # 现在指向 coder
allr tools                  # 配置 coder 的工具
...
- **`allr profile`**：显示当前 profile 名称、路径、模型、gateway 状态

## Profile vs 工作区 vs 沙箱

profile 常与工作区或沙箱混淆，但它们是不同的概念：

- **profile** 为 Allr 提供独立的状态目录：`config.yaml`、`.env`、`SOUL.md`、会话、记忆、日志、cron 任务和 gateway 状态。
...
在 local 后端使用 `cwd: "."` 表示"Allr 启动时所在的目录"，而非"profile 目录"。
...
```bash
coder gateway install         # 创建 allr-gateway-coder systemd/launchd 服务
assistant gateway install     # 创建 allr-gateway-assistant 服务
```
...
`allr update` 拉取一次代码（共享），并自动将新的内置技能同步到**所有** profile：
...
```bash
allr profile list           # 显示所有 profile 及其状态
allr profile show coder     # 显示某个 profile 的详细信息
allr profile rename coder dev-bot   # 重命名（同步更新别名和服务）
allr profile export coder   # 导出为 coder.tar.gz
allr profile import coder.tar.gz   # 从归档文件导入
```
...
使用 `--yes` 跳过确认：`allr profile delete coder --yes`

:::note
你无法删除默认 profile（`~/.allr`）。如需删除所有内容，请使用 `allr uninstall`。
:::
...
profile 使用 `ALLR_HOME` 环境变量。运行 `coder chat` 时，包装脚本在启动 hermes 前将 `ALLR_HOME` 设置为 `~/.allr/profiles/coder`。由于代码库中 119+ 个文件通过 `get_hermes_home()` 解析路径，Allr 状态会自动限定在 profile 目录范围内——包括配置、会话、记忆、技能、状态数据库、gateway PID、日志和 cron 任务。

这与终端工作目录是分开的。工具执行从 `terminal.cwd` 开始（或在 local 后端使用 `cwd: "."` 时从启动目录开始），而非自动从 `ALLR_HOME` 开始。

默认 profile 就是 `~/.allr` 本身。无需迁移——现有安装的工作方式完全不变。
