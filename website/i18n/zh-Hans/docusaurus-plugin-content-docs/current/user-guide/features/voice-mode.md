...
sidebar_position: 10
title: "语音模式"
description: "与 Allr 进行实时语音对话 — CLI、Telegram、Discord（私信、文字频道和语音频道）"
---
...
付费的 [Nous Portal](/user-guide/features/tool-gateway) 订阅通过 Tool Gateway 同时提供 LLM（第 2 步）**和** OpenAI TTS — 无需单独的 OpenAI 密钥。全新安装时，`allr setup --portal` 可一次性完成两项配置。
:::
...
:::tip
录音键可通过 `~/.allr/config.yaml` 中的 `voice.record_key` 配置（默认：`ctrl+b`）。
:::
...
```bash
allr gateway        # 启动 gateway（连接到已配置的平台）
allr gateway setup  # 首次配置的交互式设置向导
```
...
:::tip
如需在服务器频道中禁用提及要求，在 `~/.allr/.env` 中添加：
```bash
...
2. 或改用私信 — 无需提及
3. 或在 `~/.allr/.env` 中设置 `DISCORD_REQUIRE_MENTION=false`
...
- 检查 LLM 模型是否已配置且可访问
- 查看 gateway 日志：`tail -f ~/.allr/logs/gateway.log`
