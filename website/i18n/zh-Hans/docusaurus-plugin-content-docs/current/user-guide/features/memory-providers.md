...
| **AI peer**（`aiPeer`） | 每个 Allr profile 一个。host key `hermes` → 默认；其他 profile 使用 `hermes.<profile>`。 |
...
扫描所有 Allr profile，为没有 host 块的 profile 创建 host 块，从默认 `hermes` 块继承设置，并提前创建新的 AI peer。幂等操作——跳过已有 host 块的 profile。
