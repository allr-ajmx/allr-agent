---
title: "Plan — Write a markdown plan to .allr/plans/; no execution"
sidebar_label: "Plan"
...
Write a markdown plan to .allr/plans/; no execution.
...
- Your deliverable is a markdown plan saved inside the active workspace under `.allr/plans/`.
...
- `.allr/plans/YYYY-MM-DD_HHMMSS-<slug>.md`

Treat that as relative to the active working directory / backend workspace. Allr file tools are backend-aware, so using this relative path keeps the plan with the workspace on local, docker, ssh, modal, and daytona backends.

If the runtime provides a specific target path, use that exact path.
If not, create a sensible timestamped filename yourself under `.allr/plans/`.
...
> **For Allr:** Use subagent-driven-development skill to implement this plan task-by-task.
...
Use Allr tools to understand the project:
