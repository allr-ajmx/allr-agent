...
This skill is separate from the Allr Email gateway adapter. The gateway
adapter lets people email the agent and uses Allr's built-in IMAP/SMTP
...
To reply non-interactively from Allr, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Allr)** — pipe the message via stdin:
