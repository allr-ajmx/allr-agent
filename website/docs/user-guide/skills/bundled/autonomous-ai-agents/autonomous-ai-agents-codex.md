...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`allr-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-allr-agent) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Allr terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Allr itself, `model.provider: openai-codex` uses Allr-managed Codex
OAuth from `~/.allr/auth.json` after `allr auth add openai-codex`. For the
...
When invoking the Codex CLI from an Allr gateway/service context (for example,
