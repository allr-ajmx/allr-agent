...
| | |
|---|---|
| Source | Optional — install with `allr skills install official/research/parallel-cli` |
...
- It overlaps with Allr native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Allr native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Allr native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
## Recommended Allr usage patterns
...
- Do not silently choose Parallel when Allr native tools are already sufficient.
