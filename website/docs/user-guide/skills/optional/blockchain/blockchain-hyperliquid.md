...
| | |
|---|---|
| Source | Optional — install with `allr skills install official/blockchain/hyperliquid` |
| Path | `optional-skills/blockchain/hyperliquid` |
| Version | `0.1.0` |
| Author | Hugo Sequier (Hugo-SEQUIER), Allr |
| License | MIT |
...
The script reads `${ALLR_HOME:-~/.allr}/.env` for two optional defaults:
...
Helper script: `~/.allr/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
optional when `HYPERLIQUID_USER_ADDRESS` is set in `${ALLR_HOME:-~/.allr}/.env`.
