---
title: "Honcho — Configure and troubleshoot Honcho memory for Allr"
...
Configure and troubleshoot Honcho memory for Allr.
...
# Honcho Memory for Allr

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Allr profile its own peer identity while sharing a unified view of the user.
...
```bash
allr honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Allr profile's AI peer
...
Honcho models conversations as interactions between **peers**. Allr creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Allr instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Allr run |
| `global` | Single session across all directories |

Manual override: `allr honcho map my-project-name`
...
Each Allr profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
allr honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Allr when Honcho memory is active.
...
Config file: `$ALLR_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `allr honcho setup`. Ensure `memory.provider: honcho` is in `~/.allr/config.yaml`.

### Memory not persisting across sessions
Check `allr honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `allr profile create <name> --clone`. For existing profiles: `allr honcho sync`.
...
| Command | Description |
|---------|-------------|
| `allr honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `allr honcho status` | Show resolved config, connection test, peer info for active profile |
| `allr honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `allr honcho disable` | Disable Honcho for the active profile |
| `allr honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `allr honcho peers` | Show peer identities across all profiles |
| `allr honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `allr honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `allr honcho sessions` | List known directory-to-session-name mappings |
| `allr honcho map <name>` | Map current working directory to a Honcho session name |
| `allr honcho identity` | Seed AI peer identity or show both peer representations |
| `allr honcho sync` | Create host blocks for all Allr profiles that don't have one yet |
| `allr honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Allr + Honcho |
| `allr memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `allr memory status` | Show active memory provider and config |
| `allr memory off` | Disable external memory provider |
