...
| | |
|---|---|
| Source | Optional — install with `allr skills install official/creative/kanban-video-orchestrator` |
...
3. **Generates** a setup script that creates Allr profiles, project workspace, and the initial kanban task
...
**[references/intake.md](https://github.com/allr-ajmx/allr-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/intake.md)**.
...
**[references/role-archetypes.md](https://github.com/allr-ajmx/allr-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/role-archetypes.md)**.

For mapping role → which Allr skills + toolsets it loads, see
**[references/tool-matrix.md](https://github.com/allr-ajmx/allr-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/tool-matrix.md)**.
...
3. Creates each Allr profile via `allr profile create --clone`
...
7. Fires the initial `allr kanban create` task assigned to the director

Use `scripts/bootstrap_pipeline.py` to generate setup.sh from a brief +
team-design JSON. See **[references/kanban-setup.md](https://github.com/allr-ajmx/allr-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/kanban-setup.md)**
...
```bash
allr kanban watch --tenant <project-tenant>     # live events
allr kanban list  --tenant <project-tenant>     # board snapshot
allr dashboard                                   # visual board UI
```
...
playbook, see **[references/monitoring.md](https://github.com/allr-ajmx/allr-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/monitoring.md)**.
...
task graphs. See **[references/examples.md](https://github.com/allr-ajmx/allr-agent/blob/main/optional-skills/creative/kanban-video-orchestrator/references/examples.md)**.
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `allr kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${ALLR_HOME:-~/.allr}/.env` or the user's secret store.
