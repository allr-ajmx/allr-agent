...
| | |
|---|---|
| Source | Optional — install with `allr skills install official/mcp/fastmcp` |
...
| Related skills | [`allr-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-allr-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Allr or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Allr. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.allr/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.allr/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Allr integration, either:

- configure the server in `~/.allr/config.yaml` using the `native-mcp` skill, or
...
### Allr cannot see the deployed server

The server-building part may be correct while the Allr config is not. Load the `native-mcp` skill and configure the server in `~/.allr/config.yaml`, then restart Allr.
