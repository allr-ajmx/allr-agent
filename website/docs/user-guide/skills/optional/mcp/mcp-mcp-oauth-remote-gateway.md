...
| Related skills | [`allr-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-allr-agent), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter), [`fastmcp`](/docs/user-guide/skills/optional/mcp/mcp-fastmcp) |
...
# MCP OAuth on a Remote Allr Gateway
...
2. Allr is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Allr** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
3. Starts an HTTP server on `127.0.0.1:P` **inside the Allr process**.
4. Prints the authorize URL and waits for the code at its local endpoint.

When Allr runs remotely, the `127.0.0.1` in the `redirect_uri` is the remote
...
laptop and fails to connect. The callback never reaches the Allr process, the
...
empty or missing tokens directory (`$ALLR_HOME/mcp-tokens/`), and a reload that
...
already cover the deployment. When Allr detects a remote session it prints two
...
Both require an interactive terminal to the Allr host. The rest of this skill
is for when there is NO interactive TTY — Allr running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Allr Dashboard (try this BEFORE manual token surgery)

A remote Allr gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `allr dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'allr dashboard'`). It exposes a connector/MCP console —
...
```bash
env | grep -iE "ALLR_DASHBOARD_PUBLIC_URL|RAILWAY_PUBLIC_DOMAIN|RAILWAY_STATIC_URL|RAILWAY_SERVICE_.*_URL|PUBLIC_URL|BASE_URL|DOMAIN" \
  | sed -E 's/(TOKEN|SECRET|KEY|PASSWORD)=.*/\1=***REDACTED***/I'
```

`ALLR_DASHBOARD_PUBLIC_URL` is authoritative when present. On Railway also check
...
across restarts) and anything reading credentials from `$ALLR_HOME/.env`. Those
...
Allr' `HermesTokenStorage` would have written, so on `/reload-mcp` Allr finds
cached tokens and skips the browser flow entirely.
...
```bash
env | grep -iE "ALLR|RAILWAY|CONTAINER"
...
```bash
ALLR_HOME=$(python3 -c 'from hermes_constants import get_hermes_home; print(get_hermes_home())')
echo "config: $ALLR_HOME/config.yaml"
echo "tokens: $ALLR_HOME/mcp-tokens/"
```
...
UA** — Cloudflare will 403 you even though Allr (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Allr sees `auth: oauth`, calls `HermesTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Allr process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Allr needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Allr does natively).
...
- **Don't claim Allr doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
