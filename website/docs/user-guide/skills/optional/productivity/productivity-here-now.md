...
| | |
|---|---|
| Source | Optional — install with `allr skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ALLR_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ALLR_SKILL_DIR}/scripts/drive.sh` for private Drive storage
