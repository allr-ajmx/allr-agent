---
title: "Actual Setup — Set up Actual Computer (actual.inc) inference in Allr"
...
Set up Actual Computer (actual.inc) inference in Allr.
...
- User has an `ac_` key and wants Allr routed through their Actual cluster.
...
- Allr has **first-class `actual` provider support** (provider id `actual`,
  aliases `actual-computer`, `actualcomputer`, `aci`). Do NOT configure Actual
  as a `custom_providers` / `providers.actual.*` entry on current Allr — the
...
1. Put the key in `.env` (secrets only — never config.yaml):
   append `ACTUAL_API_KEY=ac_...` to `~/.allr/.env`.
...
3. Select provider + model:
   ```bash
   allr config set model.provider actual
   allr config set model.default "MODEL_ID_FROM_DISCOVERY"
   ```
4. Verify end-to-end:
   ```bash
   allr chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL_ID
   ```
...
2. Download and load a model (scriptable once authorized):
   ...
   ```
3. Point Allr at the daemon. `ACTUAL_BASE_URL` with a loopback host flips the
   built-in provider into local no-auth mode automatically — no key needed:
   append `ACTUAL_BASE_URL=http://127.0.0.1:8080` to `~/.allr/.env`, then:
   ```bash
   allr config set model.provider actual
   allr config set model.default "INSTALLED_MODEL_NAME"
   ```
4. Verify (reduced toolset — see context-window pitfall below):
   ```bash
   allr chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m INSTALLED_NAME -t file,web
   ```
...
1. **reasoning_effort trap (handled by Allr since the first-class provider).**
...
5. **Do not create a custom provider named `actual`.** Older setup guides
   ...
   current Allr the built-in provider wins the name; stale custom blocks
...
```bash
# Relay:
allr chat -Q -q "Reply with exactly: ACTUAL_OK" --provider actual -m MODEL
# Local (small model — reduced toolset):
allr chat -Q -q "Reply with exactly: LOCAL_OK" --provider actual -m MODEL -t file,web
