---
title: "Allr S6 Container Supervision — Modify or debug s6 services in the Allr Docker image"
sidebar_label: "Allr S6 Container Supervision"
description: "Modify or debug s6 services in the Allr Docker image"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Allr S6 Container Supervision

Modify or debug s6 services in the Allr Docker image.
...
# Allr s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the Allr Docker image (something that should be supervised at every container start, like the dashboard)
...
If you're just running the Allr and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
│   ├── 01-allr-setup                ← docker/stage2-hook.sh
│   │   ├── UID/GID remap
│   │   ├── chown /opt/data
...
│       └── walk $ALLR_HOME/profiles/<name>/gateway_state.json
│           → recreate /run/service/gateway-<name>/
│           → auto-start only those with prior_state == "running"
│
├── s6-rc.d (static services, in /etc/s6-overlay/s6-rc.d/)
│   ├── main-allr/run                ← exec sleep infinity (no-op slot)
│   └── dashboard/run                  ← if ALLR_DASHBOARD=1, runs `allr dashboard`
│
...
│   │   └── log/run     (s6-log → $ALLR_HOME/logs/gateways/coder/current)
│   └── ...
│
...
| `docker/stage2-hook.sh` | The "old entrypoint logic" — UID remap, chown, seed, skills sync. Runs as cont-init.d/01-allr-setup. |
...
| `docker/s6-rc.d/main-allr/run` | No-op `sleep infinity` — slot exists so the s6-rc user bundle is valid; main hermes runs as the CMD, not as a supervised service. |
| `docker/s6-rc.d/dashboard/run` | Conditional service — `exec sleep infinity` unless `ALLR_DASHBOARD` is truthy. |
...
| `hermes_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `allr gateway start/stop/restart` and routes to s6 when running in a container. |
...
1. **cont-init.d scripts receive no CMD args** — so the stage2 hook can't parse `docker run <image> chat -q "hi"` to set `ALLR_ARGS` for a service `run` script to consume.
...
```sh
docker build -t allr-agent-harness:latest .
ALLR_TEST_IMAGE=allr-agent-harness:latest scripts/run_tests.sh tests/docker/ -v
...
The cont-init reconciler runs as hermes (`s6-setuidgid hermes` in `02-reconcile-profiles`). If a profile dir ends up root-owned (e.g. because `docker exec <c> allr profile create …` ran as root by default), the reconciler can't read SOUL.md and fails with `PermissionError`. Mitigation: `stage2-hook.sh` chowns `$ALLR_HOME/profiles` to hermes on **every** boot, idempotently. Don't remove that block.

### Files written by `docker exec` are root-owned

`docker exec` defaults to root. Either pass `--user hermes` or rely on the stage2 chown sweep next reboot. Don't write files under `$ALLR_HOME/profiles/<name>/` as root manually — the next reconcile pass will sweep them but in-flight operations may hit perm errors.
...
The reconciler keys on the **presence of `SOUL.md`** as the "real profile" marker. `allr profile create` always seeds it. If a profile dir is missing SOUL.md (stray directory, partial restore, backup-in-progress), the reconciler skips it intentionally. Add a `SOUL.md` (even empty) to opt back in.
...
- `allr-agent-dev`: General allr-agent codebase navigation
- `hermes-tool-quirks`: Specific Allr-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with hermes built-in tools.
