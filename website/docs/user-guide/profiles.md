...
Run multiple independent Allr agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate Allr home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Allr state.

:::caution Give every agent its own profile
Never point two agent processes at the same profile (the same Allr home). Both write memory automatically, and each loads the other's writes into its system prompt at session start — so two writers on one home compound each other's state until it stops being anything you configured. Profiles exist exactly to prevent this; agents that need shared memory should use an [external memory provider](/user-guide/features/memory-providers) instead.
:::
...
```bash
allr profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Allr profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `allr setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::

### Blank profile

```bash
allr profile create mybot
```
...
```bash
allr profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `allr profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
Copies your current profile's `config.yaml`, `.env`, `SOUL.md`, and skills into the new profile. Same API keys, model, and capabilities, but fresh sessions and memory. Edit `~/.allr/profiles/work/.env` for different API keys, or `~/.allr/profiles/work/SOUL.md` for a different personality.

### Clone everything (`--clone-all`)

```bash
allr profile create backup --clone-all
```

Copies **everything** — config, API keys, personality, all memories, skills, cron jobs, plugins. A complete working snapshot. Per-profile history is excluded (session history, `state.db`, `backups/`, `state-snapshots/`, `checkpoints/`) — these belong to the source profile and can reach tens of GB. For a full backup including history, use `allr profile export` or `allr backup` instead.

### Clone from a specific profile

```bash
allr profile create work --clone-from coder
```
...
```bash
allr profile create work-backup --clone-from coder --clone-all
```
...
### Sticky default (`allr profile use`)

```bash
allr profile use coder
allr chat                   # now targets coder
allr tools                  # configures coder's tools
...
- **`allr profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives Allr its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Allr was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.allr/profiles/coder/.env

# Edit assistant's tokens
nano ~/.allr/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates allr-gateway-coder systemd/launchd service
assistant gateway install     # creates allr-gateway-assistant service
```
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.allr/profiles/coder/SOUL.md
```
...
default for **future CLI/gateway runs** (same as `allr profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`allr update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
allr profile list           # show all profiles with status
allr profile show coder     # detailed info for one profile
allr profile rename coder dev-bot   # rename (updates alias + service)
allr profile export coder   # export to coder.tar.gz
allr profile import coder.tar.gz   # import from archive
```

## Deleting a profile

```bash
allr profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `allr profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.allr`). To remove everything, use `allr uninstall`.
:::
...
```bash
# Bash
eval "$(allr completion bash)"

# Zsh
eval "$(allr completion zsh)"
```
...
Profiles use the `ALLR_HOME` environment variable. When you run `coder chat`, the wrapper script sets `ALLR_HOME=~/.allr/profiles/coder` before launching hermes. Since 119+ files in the codebase resolve paths via `get_hermes_home()`, Allr state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.

This is separate from terminal working directory. Tool execution starts from `terminal.cwd` (or the launch directory when `cwd: "."` on the local backend), not automatically from `ALLR_HOME`.
...
isolated by `ALLR_HOME`, not by changing `HOME`. Container backends still use
`{ALLR_HOME}/home` for persistent tool state, and host users who need strict
per-profile tool config can opt in with `terminal.home_mode: profile`.

This means two things that are easy to mix up:

- `ALLR_HOME` is the profile boundary. It controls Allr config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other Allr
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, Allr keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode Allr launches tool
subprocesses with `HOME={ALLR_HOME}/home`; you then need to initialize or link
...
Allr also exposes `ALLR_REAL_HOME` to subprocesses so scripts can still find
the actual account home when `home_mode: profile` is active.

The default profile is simply `~/.allr` itself. No migration needed — existing installs work identically.
