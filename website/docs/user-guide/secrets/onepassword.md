...
`op` supports two non-interactive-friendly modes; Allr works with either:

- **Service accounts** (recommended for servers/CI): create a service account in 1Password, grant it read access to the relevant vault, and export its token as `OP_SERVICE_ACCOUNT_TOKEN` in `~/.allr/.env`. The token is the credential — treat it like any other bearer token.
- **Desktop / interactive sessions** (laptops): run `op signin` (or enable CLI integration in the 1Password app). Allr passes your `OP_SESSION_*` variables through to the `op` child process. The 1Password cache key includes those session variables, so signing into a different account never serves a value cached under the previous identity.

## Bootstrap token

When you authenticate with a **service-account token**, that token is itself the bootstrap credential Allr needs *before* it can resolve any `op://` reference. It must be present in `os.environ` of every process that resolves secrets — including cron jobs (`kanban.dispatch_in_gateway: false`), subprocess invocations, CLI runs, macOS launchd agents, and Docker containers — not just the interactive gateway. There are three ways to make it available, in order of precedence:

1. **In `~/.allr/.env` (recommended).** `allr secrets onepassword setup --token <token>` writes the token to `~/.allr/.env`, exactly like Bitwarden's `BWS_ACCESS_TOKEN`. Because `load_hermes_dotenv()` always loads `.env`, the token is available everywhere with zero extra setup. This is the simplest reliable option.

2. **In `~/.allr/.op.env` (gitignored).** If you'd rather keep the service-account token out of `.env` — for example so `.env` can be checked into a private dotfiles repo while the token stays out of version control — place it in `~/.allr/.op.env`:

   ```bash
   echo 'OP_SERVICE_ACCOUNT_TOKEN=ops_...' > ~/.allr/.op.env
   chmod 600 ~/.allr/.op.env
   ```

   Allr auto-loads `.op.env` at startup, **after** `.env`, and **never** overrides a token already present in the environment. `.op.env` is gitignored so the token never enters a committed file.
...
3. **Via systemd `EnvironmentFile` (Linux gateway).** If you run the gateway under systemd, you can inject the token directly into the service environment:
   ...
   ```ini
   [Service]
   EnvironmentFile=-/home/youruser/.allr/.op.env
   ```

   A token injected this way takes precedence — Allr detects that `OP_SERVICE_ACCOUNT_TOKEN` is already set and skips loading `.op.env` entirely.
...
```bash
allr secrets onepassword setup
```
...
```bash
allr secrets onepassword set OPENAI_API_KEY    "op://Private/OpenAI/api key"
allr secrets onepassword set ANTHROPIC_API_KEY "op://Private/Anthropic/credential"
```

### 4. Preview and confirm

```bash
allr secrets onepassword sync     # dry-run: resolve now, show what would apply
...
|---|---|
| `allr secrets onepassword setup` | Verify `op`, set account / token env var, enable |
| `allr secrets onepassword status` | Show config, binary, auth, and configured references |
| `allr secrets onepassword token` | Rotate the service-account token: validate with `op whoami`, then store it in `.env` |
| `allr secrets onepassword set ENV_VAR "op://…"` | Map an env var to a reference (stored stripped + validated) |
| `allr secrets onepassword remove ENV_VAR` | Drop a mapping |
| `allr secrets onepassword sync` | Dry-run: resolve references now and show what would apply |
| `allr secrets onepassword sync --apply` | Resolve and export into the current shell's environment |
| `allr secrets onepassword disable` | Flip `enabled: false`; leaves mappings in place |

`op` and `1password` are accepted as aliases for `onepassword`.

## Configuration

Defaults in `~/.allr/config.yaml`:
...
| `service_account_token_env` | `OP_SERVICE_ACCOUNT_TOKEN` | Env var Allr reads the service-account token from. Its value is exported to the `op` child as `OP_SERVICE_ACCOUNT_TOKEN` (the name `op` expects). Leave the var unset to use a desktop/interactive session. |
...
1Password never blocks Allr startup. If anything goes wrong you'll see a one-line warning in stderr and Allr continues:

| Symptom | Cause | Fix |
|---|---|---|
| `the op CLI was not found on PATH` | `op` not installed / not on PATH | Install the CLI, or set `secrets.onepassword.binary_path` |
| `op read failed for 'op://…': …` | Locked session, expired token, or no vault access | `op signin`, run `allr secrets onepassword token` to rotate the service-account token, or grant the service account access |
...
- A 1Password service-account token can read every secret the account has access to. Store it in `~/.allr/.env` (not `config.yaml`), and revoke + regenerate from 1Password if it leaks.
- Allr refuses to let a resolved value overwrite the token env var itself, even with `override_existing: true`.
...
- **Single-machine personal setups** where `~/.allr/.env` is fine.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or anywhere you want centralized rotation and revocation across multiple Allr installations.
