...
2. At startup, after `.env` loads, Allr runs the helper ONCE via `/bin/sh -c` and parses its stdout as a dotenv blob.
...
| `helper command failed` | Non-zero exit, timeout, spawn failure | Run the helper manually in a shell to see its real error (Allr discards its stderr on purpose) |
