...
2. Create or pick a **Project** (e.g. "Allr keys").
...
4. **Machine accounts → New machine account → My Allr machine** → **Projects** tab → grant Read access to your project.
...
1. Download and verify `bws v2.0.0` into `~/.allr/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.allr/.env` as `BWS_ACCESS_TOKEN`.
...
|---|---|
| `allr secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `allr secrets bitwarden status` | Show config + binary version + token presence/validation |
| `allr secrets bitwarden token` | Rotate the access token: validate the new token against Bitwarden, then store it in `.env` |
| `allr secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `allr secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `allr secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `allr secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |
...
Bitwarden Secrets Manager: → Run `allr secrets bitwarden token` to paste a fresh access token ...
```

Fix it without re-running the whole wizard:

```bash
allr secrets bitwarden token                     # masked prompt
allr secrets bitwarden token --access-token 0.…  # non-interactive
```
...
| `encrypted_cache.enabled` | `false` | Store the last successful fetch in an AES-GCM encrypted cache at `~/.allr/cache/bws_cache.enc.json`. |
...
| `auto_install` | `true` | When true, `bws` is auto-downloaded into `~/.allr/bin/` on first use. |

## Failure modes

Bitwarden never blocks Allr startup. If anything goes wrong, you'll see a one-line warning in stderr and Allr continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `allr secrets bitwarden setup` |
| `Bitwarden rejected the machine-account access token … invalid_client` | Token revoked, expired, machine account deleted — or the token belongs to another region (e.g. EU token hitting the US identity endpoint) | Run `allr secrets bitwarden token` to paste a fresh token; for region mismatches re-run setup and pick EU/self-hosted (or set `secrets.bitwarden.server_url`) |
| `bws exited 1: invalid access token` | Token revoked or wrong | Run `allr secrets bitwarden token` with a new token |
...
- Allr will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — Allr does not auto-upgrade `bws` to "latest" because upstream release shapes can change.

## When NOT to use this

- **Single-machine personal setups** where `~/.allr/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple Allr installations.
