...
description: "Package an agent's personality, skills, cron jobs, and MCP config as a git repo others can install and update."
---

# Profile Distributions: Share a Whole Agent

A **profile distribution** packages a complete Allr agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing an Allr agent meant sending someone:
...
```bash
allr profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `allr profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Allr in 2026, that's already true.
...
:::caution
**Allr does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `allr profile install` or `allr profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
allr profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.allr/profiles/research-bot/SOUL.md
...
Create `~/.allr/profiles/research-bot/distribution.yaml`:
...
Create `~/.allr/profiles/research-bot/.gitignore` with at minimum:
...
Recipients who run `allr profile update research-bot` will pull the latest.
...
```bash
allr profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.allr/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
allr profile install github.com/you/research-bot

# Full HTTPS
allr profile install https://github.com/you/research-bot.git

# SSH
allr profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
allr profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
allr profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
allr profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
allr profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
allr profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Allr distribution.
...
```bash
cp ~/.allr/profiles/research-bot/.env.EXAMPLE ~/.allr/profiles/research-bot/.env
...
```bash
allr profile info research-bot
```
...
`allr profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
allr profile update research-bot
```
...
```bash
allr profile delete research-bot
```
...
Path:    ~/.allr/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `allr profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
allr profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `allr profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.allr/profiles/polymarket-trader
...
# Anyone
allr profile install github.com/you/hermes-polymarket-trader --alias
```
...
You built Allr-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
allr profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
allr profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `allr profile info <name>` and hold off on updates until you're ready.
:::
...
allr profile info research-bot | grep Version
...
```yaml
# ~/.allr/profiles/research-bot/local/my-overrides.yaml
...
allr profile delete research-bot --yes
allr profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
allr profile update research-bot --force-config --yes
```
...
allr profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.allr/profiles/forked-research-bot/
...
allr profile install ~/.allr/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
allr profile delete research-bot-test --yes
allr profile install ~/.allr/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `allr -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `allr profile` — not a parallel command tree.
...
- Reserved profile names (`allr`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [Using SOUL with Allr](../guides/use-soul-with-allr.md) — authoring personalities
