...
Allr automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.allr/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Allr stores session history so it can resume conversations, but it does not
...
Allr explicitly injects for that turn.
...
For example, if a user sends an image and asks Allr to make a meme from it,
Allr may inspect that image once with vision and run an image-processing
...
`allr sessions prune` only when you want to delete old ended sessions from
storage. If `state.db` has simply grown large, start with the non-destructive
option first: `allr sessions optimize` merges FTS5 index segments and
...
Session IDs are shown when you exit a CLI session, and can be found with `allr sessions list`.
...
`allr sessions list` accepts `--workspace <needle>` to show only sessions whose workspace key (git repo root, else cwd) matches — by path substring or exact directory basename:

```bash
allr sessions list --workspace my-project
allr sessions list --workspace ~/code/allr-agent
```

### Conversation Recap on Resume

When you resume a session, Allr displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming an Allr session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.allr/config.yaml`:
...
Allr automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `allr sessions list` or `allr sessions browse`.
...
```bash
allr sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Allr creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:
...
Allr provides a full set of session management commands via `allr sessions`:
...
# Filter by platform
allr sessions list --source telegram

# Show more sessions
allr sessions list --limit 50
```
...
`allr sessions export` is one surface for every export format, selected with `--format`:
...
allr sessions export backup.jsonl

# Export sessions from a specific platform
allr sessions export telegram-history.jsonl --source telegram

# Export a single session
allr sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
allr sessions export backup.jsonl --redact
```
...
allr sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
allr sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
allr sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
allr sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
allr sessions export --format trace

# One session to a local trace file
allr sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
allr sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.allr/session-exports`).

```bash
# Export one session to Markdown
allr sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
allr sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
allr sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
allr sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
allr sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
allr sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
allr sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
allr sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
allr sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
allr sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
allr sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
allr sessions prune --newer-than 5h

# Explicit window with absolute timestamps
allr sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
allr sessions prune --source telegram
allr sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
allr sessions prune --newer-than 5h --title "smoke test"   # title substring
allr sessions prune --older-than 30 --max-messages 3        # tiny sessions
allr sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
allr sessions prune --model gpt-5 --older-than 1w           # by model (substring)
allr sessions prune --provider openrouter --older-than 60   # by billing provider
allr sessions prune --branch feature/old-experiment         # by git branch
allr sessions prune --user 12345678 --chat-type group       # by messaging origin
allr sessions prune --max-tokens 500 --older-than 7         # by token usage
allr sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
allr sessions prune --newer-than 5h --dry-run

# Skip confirmation
allr sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `allr sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `allr sessions prune` keeps the 90-day cutoff. Every
...
`allr sessions archive` takes the same filters as `prune` but soft-hides
...
allr sessions archive --newer-than 5h

# Archive by title substring, preview first
allr sessions archive --title "dry run" --dry-run
allr sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `allr sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`allr sessions list` and `/resume` but remain in the database and can be
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`allr insights`](/reference/cli-commands#hermes-insights).
...
When Allr cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Allr uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Allr in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.allr/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.allr/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `gateway_routing` table in `~/.allr/state.db` | Maps session keys to active session IDs (origin metadata, expiry flags) |
| Legacy routing mirror | `~/.allr/sessions/sessions.json` | Backward-compat mirror of the routing index, written when `gateway.write_sessions_json: true` (the default) |
...
`state.db`; `~/.allr/sessions/sessions.json` is a **legacy mirror** of it,
...
`allr sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.allr/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `allr sessions list`, the cause is
`state.db` not receiving them — run `allr sessions repair` and watch for a
...
`*.jsonl` files in `~/.allr/sessions/`. They are no longer written or
read by Allr. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Allr process in the same `ALLR_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.allr/config.yaml`:
...
allr sessions delete <session_id>
