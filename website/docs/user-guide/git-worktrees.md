...
title: "Git Worktrees"
description: "Run multiple Allr agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Allr is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Allr so each session has a clean, isolated working directory.

## Why Use Worktrees with Allr?
...
Now you can `cd` into the new worktree and run Allr there:
...
- Allr checkpoint data under `~/.allr/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Allr experiment**
...
- Use **git worktrees** to give each Allr session its own clean checkout.
...
The TypeScript surfaces (`ui-tui/`, `apps/hermes-universal/`) each need a `node_modules`, which a fresh `npm ci` per worktree duplicates across every branch. If you hack on the TUI from multiple worktrees, see [TUI from Worktrees](../developer-guide/worktree-ui-dev.md) for the `htui` helper that shares one install by symlink.
