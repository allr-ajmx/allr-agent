...
`~/.allr/config.yaml`, `~/.allr/.env`, and even the shell environment — for
...
```text
/etc/hermes/
├── config.yaml     # managed config layer (wins over ~/.allr/config.yaml)
└── .env            # managed env layer (wins over ~/.allr/.env + shell)
```
...
The location can be relocated with the `ALLR_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `ALLR_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Allr.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export ALLR_MANAGED_DIR=/opt/org/hermes-policy
```

:::warning
A user who can set `ALLR_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `allr doctor` reports the *resolved* managed directory so
...
| 2 | `~/.allr/config.yaml` (user) | `~/.allr/.env` (user) |
...
```bash
allr config        # shows a header naming the managed source + the pinned keys
allr doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Allr refuses and names the source:

```bash
$ allr config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/hermes/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `allr config set` / setup will not write
...
Changes take effect on the next Allr start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`allr doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Allr as `root`), managed scope is advisory.
