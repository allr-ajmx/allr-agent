...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Allr terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Allr's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `allr setup --portal` — and you're ready to `allr chat`. See [Nous Portal](/integrations/nous-portal).
:::
...
allr chat --model "anthropic/claude-sonnet-4"

# With a specific provider
allr chat --provider nous        # Use Nous Portal
allr chat --provider openrouter  # Force OpenRouter
...
The `allr plugins` commands manage native Allr plugins and portable Agent
...
allr plugins disable <plugin-name>
allr plugins update <plugin-name>
allr plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Allr currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Allr CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Allr CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Allr to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.allr/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Allr supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Allr replies |
...
Allr loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.allr/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.allr/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Allr..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Allr recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Allr — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Allr is working, Allr prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Allr to the background — just like any terminal process. The shell prints a confirmation:

```
Allr has been suspended. Run `fg` to bring Allr back.
```
...
Use `/title My Session Name` inside a chat to name the current session, or `allr sessions rename <id> <title>` from the command line. Use `allr sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Allr's SQLite state database under `~/.allr/state.db`. The database keeps:
...
Allr immediately confirms the task and gives you back the prompt:
