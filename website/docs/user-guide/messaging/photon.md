...
Connect Allr to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Allr — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Allr runs it in a
...
allr photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Allr` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.allr/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.allr/.env`
...
(device token, dashboard project id) lives in `~/.allr/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Allr
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Allr replies with a pairing code. Approve it with:

```bash
allr pairing approve photon <CODE>
```

Use `allr pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.allr/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.allr/.env`):
...
By default Allr responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Allr` and
`@Allr agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Allr will reply.
...
assigned iMessage line Allr uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run allr photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `allr photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `allr photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `allr photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Allr sends images, voice
  ...
  media.
- **Native polls are supported.** Allr sends poll content through
  spectrum-ts' `poll()` builder via the sidecar's `/send-poll` endpoint.
- **Message effects are supported.** Allr sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `allr send`, the dashboard) reuse the sidecar the
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Allr is set
...
| `PHOTON_MENTION_PATTERNS` | Allr wake words  | JSON list / comma / newline regex patterns for group mentions |
