...
Connect Allr to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Allr supports two WeCom integration modes:
...
> Run `allr gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Allr decrypts the message, queues it for the agent
...
(Use `allr gateway start` only after `allr gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Allr and the token the
...
in `~/.allr/.env` values around `=` will also break signature checks. After
fixing, restart `allr gateway run`.
...
Check `allr gateway run` logs for the bound host/port. If the adapter bound to
