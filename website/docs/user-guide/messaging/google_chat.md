...
sidebar_position: 12
title: "Google Chat"
description: "Set up Allr as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Allr to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Allr
...
> Run `allr gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Allr can read (e.g.,
`~/.allr/google-chat-sa.json`, `chmod 600`).
...
Also grant `Pub/Sub Viewer` on the same subscription — Allr calls
...
- **App name**: whatever you want users to see ("Allr" is reasonable).
...
`ADDED_TO_SPACE` event that Allr uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Allr

Add the Google Chat section to `~/.allr/.env`:
...
Send "hola" in the test DM. The bot posts a "Allr is thinking…" marker, then
...
`~/.allr/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Allr is thinking…").
...
Thread support: when a user replies inside a thread, Allr detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Allr session.
...
3. Download the JSON. Move it onto the host that runs Allr.
4. Register the client with Allr (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's Allr home (e.g.
`~/.allr/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.allr/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.allr/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Allr isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `allr gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Allr will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.allr/google_chat_user_tokens/<sanitized_email>.json` (filesystem
