...
sidebar_position: 4
title: "Slack"
description: "Set up Allr as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Allr to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Allr instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Allr uses the modern
...
The fastest path is to paste a manifest Allr generates for you. It
...
### Option A: From an Allr-generated manifest (recommended)

1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   allr slack manifest --agent-view --write
   ```
   This writes `~/.allr/slack-manifest.json` and prints paste-in
   ...
   ```bash
   allr slack manifest --agent-view \
...
4. Enter an app name (e.g., "Allr") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Allr can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not an Allr configuration issue.
:::
...
Allr uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
allr gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
allr gateway              # Foreground
allr gateway install      # Install as a user service
sudo allr gateway install --system   # Linux only: boot-time system service
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; Allr now
...
Every Allr command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Allr command with its description.

Under the hood: Allr ships with a generated Slack app manifest (see
...
New Slack apps use Slack's **Agent** messaging experience. Existing Allr
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so Allr can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. Allr only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When Allr adds new commands (e.g. after `allr update`), regenerate
the manifest and update your Slack app:

```bash
allr slack manifest --write
```
...
3. Paste the new contents of `~/.allr/slack-manifest.json`
...
`/hermes btw run the tests` — Allr routes it the same way as `/btw
...
Slack never delivers them to Allr.

As a workaround, Allr recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Allr treats it identically to the slash form and replies in the same
thread.
...
The bang form also works behind a mention (`@Allr !stop`) and with
...
Allr falls back to a text prompt, the prompt instructs you to reply
...
fails, Allr retries via a second ephemeral API path — a slash reply is never
posted publicly to the channel as a fallback. (Commands typed as regular
messages — `!cmd` in threads, `@Allr /cmd` — reply as normal visible
messages instead.)
...
```bash
allr slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Allr behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Allr what time is it?`). In channels, Allr replies in a thread attached to that message. |
| **Threads** | If you @mention Allr inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.allr/config.yaml`.
...
platforms:
  slack:
    ...
    extra:
      ...
      # that message itself @mentions Allr, and "all" accepts every
      # other bot. Allr always ignores its own bot user to prevent
      # self-echoes.
      allow_bots: "none"
...
name in the thread. By default Allr sets it to `is thinking...`; customize it
...
(`platforms.google_chat.typing_status_text`, default `"Allr is thinking…"`) —
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Allr in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Allr will respond.
...
By default Allr ignores every message authored by another Slack bot or app (including Workflow Builder posts). For multi-agent workspaces — several Allr instances or peer bots collaborating in one channel — opt in with `allow_bots`:
...
- Allr always ignores its own messages, in every mode, to prevent self-echo loops.

`mentions` is the recommended mode for bot-to-bot collaboration: each agent must explicitly summon the other per turn. Avoid `all` unless every peer bot's own reply policy is loop-safe — two bots that answer everything will answer each other forever. Detection covers labeled bot messages (`bot_id`, `subtype: bot_message`), app-originated events, and unlabeled bot *users* (probed via `users.info`), so peer Allr agents are filtered consistently across workspaces.
...
manifest — regenerate with `allr slack manifest`):
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Allr will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Allr`).
...
Delivery works even when the cron process isn't co-located with the gateway — Allr falls back to a standalone Web API sender using `SLACK_BOT_TOKEN`. `MEDIA:` attachments in the cron output are uploaded as native Slack file shares to the same target.
...
Allr can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Allr also loads tokens from an **OAuth token file** at:

```
~/.allr/slack_tokens.json
```
...
- When a message arrives, Allr uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Allr` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Allr` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Allr now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Allr`)
...
- Tokens should be stored in `~/.allr/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Allr config directory
