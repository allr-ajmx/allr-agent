...
sidebar_position: 30
title: "Allr Relay"
description: "Connect Allr to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Allr Relay (Connector)
...
Allr Relay is not a chat platform itself — it is a **connector system** that
...
secret. `allr gateway enroll` redeems a **single-use enrollment token**
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.allr/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.allr/.env`:
...
`allr gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.allr/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
token. Re-login with `allr auth add nous` (or `allr setup`) and retry.
...
startup. Restart it (`allr gateway restart`).
