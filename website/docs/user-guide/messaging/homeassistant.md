...
description: Control your smart home with Allr via Home Assistant integration.
...
Allr integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Allr"
5. Copy the token
...
Allr registers four tools for smart home control:
...
Configure which events the agent sees in `~/.allr/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Allr".
...
The adapter reads credentials from `~/.allr/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Allr profile
...
running Allr — `curl -H "Authorization: Bearer <token>" <url>/api/` should
