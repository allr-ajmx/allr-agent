...
Allr can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ALLR_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Allr renders the sprite at full fidelity when your
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `allr pets list [query] [--limit N]` |
| List installed pets | `allr pets list --installed` |
| Install a pet | `allr pets install <slug> [--select] [--force]` |
| Set the active pet | `allr pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `allr pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `allr pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `allr pets off` |
| Remove an installed pet | `allr pets remove <slug>` |
| Diagnose setup | `allr pets doctor` |

`allr pets show` flags:
...
Beyond installing pre-made pets from the gallery, Allr can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `allr pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `allr tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ALLR_PET_IMAGE_PROVIDER` env var (e.g. `ALLR_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Allr is
...
Run `allr pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Allr uses its own
  profile-scoped `<ALLR_HOME>/pets/` instead — install through `allr pets`.

## See also

- The [`allr-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-allr-agent.md)
