...
When Allr runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.allr/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.allr/config.yaml` (switchable per session
via `/personality`).
...
If you use Allr's kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.allr/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Allr's deliverable
