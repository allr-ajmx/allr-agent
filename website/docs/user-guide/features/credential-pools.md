...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Allr automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Allr auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
allr auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
allr auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
allr auth add anthropic --type oauth
...
Run `allr auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `allr auth` | Interactive pool management wizard |
| `allr auth list` | Show all pools and credentials |
| `allr auth list <provider>` | Show a specific provider's pool |
| `allr auth add <provider>` | Add a credential (prompts for type and key) |
| `allr auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `allr auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `allr auth remove <provider> <index>` | Remove credential by 1-based index |
| `allr auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `allr auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `allr model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via allr model:
allr auth list
# Shows:
...
allr auth add Together.ai --api-key sk-together-second-key
```
...
Allr automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Allr PKCE OAuth | `~/.allr/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `allr auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `allr auth add`) are never auto-pruned.

Borrowed runtime secrets (for example env vars, Bitwarden/Vault/keyring/systemd references, and custom config values) are reference-only at the `auth.json` boundary. Allr can use the resolved value in memory for the current run, but it persists only metadata such as the source ref, label, status, request counters, and a non-reversible fingerprint. Manual entries and Allr-owned OAuth/device-code state keep the durable tokens they need to refresh.
...
Pool state is stored in `~/.allr/auth.json` under the `credential_pool` key:
