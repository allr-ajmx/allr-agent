...
`_ALLR_CORE_TOOLS`) are *always* loaded directly. Only MCP tools and
...
When the model invokes `tool_call`, Allr **unwraps the bridge** and
...
- **No JS sandbox.** Allr uses the simpler "structured tools" mode
