...
Allr can drive your desktop — clicking, typing, scrolling,
...
```
allr computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `allr computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `allr tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Allr app). `allr computer-use doctor` will tell you which permission is missing. |
...
Allr maps its existing approval UX onto cua-driver 0.10's immutable daemon
modes. There is no second permission toggle to keep in sync:

| Allr session | cua-driver mode | Human intervention | `existing_profile` |
|---|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Allr approvals; Cua stops at its protected boundary | Refuses unless a certified protected host is available; Allr does not claim one today |
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Allr risk acceptance; no runtime Cua prompts | Allowed within Cua's built-in, managed, and user policy ceilings |

The unrestricted daemon is private to that Allr session. Turning `/yolo`
...
daemon's mode or grants another Allr conversation the same authority.
...
## `allr computer-use doctor` — your first triage stop

`allr computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ allr computer-use doctor
...
agent is acting here." Each Allr run declares its own cua-driver
...
Allr intentionally keeps its skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Allr-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Allr skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Allr
...
Allr applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.allr/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Allr applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Allr agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Allr agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Allr warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Allr from inside the RDP / console
...
```
ALLR_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
ALLR_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Allr disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Allr sets
...
When it's on, `allr computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Allr at a binary you built from source instead
of the published release. Allr resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`ALLR_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Allr testing (see notes).
...
### Option B — point Allr straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`ALLR_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
ALLR_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
ALLR_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Allr is using your build

- `allr computer-use status` prints the resolved binary path and
  version.
- `allr computer-use doctor` confirms the binary is reachable and
...
- **Allr spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Allr YOLO, Allr instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Allr
  change needed either way.
- **Local builds skip the version check.** Allr warns when the
...
**First action when anything's off: run `allr computer-use doctor`.**
...
Run `allr computer-use install` to fetch the cua-driver binary, or
run `allr tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `allr computer-use doctor` will
...
- **Allr-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Allr `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Allr (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Allr among them)
