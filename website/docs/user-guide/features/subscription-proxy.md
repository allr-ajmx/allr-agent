...
chat completions — use your Allr-managed provider subscription as their
...
| Use case | "Use Allr as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Allr stores
the refresh token in `~/.allr/auth.json` — the same place all Allr
...
If you see `not logged in`, run `allr portal`. If you see
