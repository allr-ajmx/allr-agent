...
All skills live in **`~/.allr/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Allr at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `allr update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.allr` profile):

```bash
curl -fsSL https://allr.work/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
allr profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
allr skills opt-out            # stop future seeding — nothing on disk is touched
allr skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
allr skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `allr update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `allr skills opt-in`) to re-enable.

:::note Safe by default
`allr skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Allr installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Allr to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.allr/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
allr chat --toolsets skills -q "What skills do you have?"
allr chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Allr-tool framing, no invented
commands).
...
/home/user/.allr/cache/chart-q4-2025.png
...
When a missing value is encountered, Allr asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `allr setup` or `~/.allr/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `allr config migrate` prompts for unconfigured settings, and `allr config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.allr/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. Allr scans the
...
If you maintain skills outside of Allr — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Allr to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.allr/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.allr/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Allr process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Allr ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.allr/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.allr/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `allr bundles list`.
...
allr bundles create research

# Overwrite an existing bundle
allr bundles create backend-dev --skill ... --force

# Delete a bundle
allr bundles delete backend-dev

# Re-scan ~/.allr/skill-bundles/ and report changes
allr bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.allr/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.allr/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.allr/pending/skills/` and are
...
```bash
allr skills browse                              # Browse all hub skills (official first)
allr skills browse --source official            # Browse only official optional skills
allr skills search kubernetes                   # Search all sources
allr skills search react --source skills-sh     # Search the skills.sh directory
allr skills search https://mintlify.com/docs --source well-known
allr skills inspect openai/skills/k8s           # Preview before installing
allr skills install openai/skills/k8s           # Install with security scan
allr skills install official/security/1password
allr skills install skills-sh/vercel-labs/json-render/json-render-react --force
allr skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
allr skills list --source hub                   # List hub-installed skills
allr skills check                               # Check installed hub skills for upstream updates
allr skills update                              # Reinstall hub skills with upstream changes when needed
allr skills audit                               # Re-scan all hub skills for security
allr skills uninstall k8s                       # Remove a hub skill
allr skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
allr skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
allr skills publish skills/my-skill --to github --repo owner/repo
allr skills snapshot export setup.json          # Export skill config
allr skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Allr. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `allr skills search <query> --source skills-sh`. Allr resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Allr currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Allr repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Allr can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
allr skills search https://mintlify.com/docs --source well-known
allr skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
allr skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Allr can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
allr skills install openai/skills/k8s
allr skills tap add myorg/skills-repo
```
...
[Skills Hub](https://allr.work/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Allr-side changes required.
...
- Allr source id: `clawhub`

#### 6. LobeHub (`lobehub`)

Allr can search and convert agent entries from LobeHub's public catalog into installable Allr skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Allr source id: `lobehub`
...
- Allr source id: `browse-sh`
- Trust level: `community`

```bash
allr skills search airbnb --source browse-sh
allr skills inspect browse-sh/airbnb.com/search-listings-ddgioa
allr skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Allr also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Allr source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Allr does not enumerate or copy unrelated files from the host.

```bash
allr skills install https://sharethis.chat/SKILL.md
allr skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
allr skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `allr skills update` re-fetches from the same URL automatically when you want to refresh.
...
`allr skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Allr | Always trusted |
...
```bash
allr skills check          # Report which installed hub skills changed upstream
allr skills update         # Reinstall only the skills with updates available
allr skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Allr users add with `allr skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
Allr discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Allr user can subscribe and install:

```bash
allr skills tap add my-org/hermes-skills
allr skills search deploy
allr skills install my-org/hermes-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.allr/skills/.hub/taps.json`:
...
The `allr skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `allr skills tap list` shows the effective path per tap.
...
```bash
allr skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.allr/skills/.hub/taps.json` (created on demand).

## Bundled skill updates (`allr skills reset`)

Allr ships with a set of bundled skills in `skills/` inside the repo. On install and on every `allr update`, a sync pass copies those into `~/.allr/skills/` and records a manifest at `~/.allr/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Allr recomputes the hash of your local copy and compares it to the origin hash:
...
allr skills reset google-workspace --restore
