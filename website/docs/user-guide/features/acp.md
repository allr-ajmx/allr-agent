...
description: "Use Allr inside ACP-compatible editors and collaboration platforms"
---

# ACP Host Integration

Allr can run as an ACP server, letting ACP-compatible hosts talk to
Allr over stdio. Editors can render:
...
Allr. ACP is a good fit when you want Allr to keep its existing identity,
provider setup, memory, skills, and tools while another application owns the
conversation transport.

## What Allr exposes in ACP mode

Allr runs with a curated `allr-acp` toolset designed for editor workflows. It includes:
...
Install Allr normally, then add the ACP extra from the install checkout:
...
Any of the following starts Allr in ACP mode:
...
Allr logs to stderr so stdout remains reserved for ACP JSON-RPC traffic.
...
```bash
allr acp --setup-browser           # interactive (prompts before ~400 MB download)
allr acp --setup-browser --yes     # accept the download non-interactively
```

This is the standalone command. The terminal-auth flow (`allr acp --setup`) also offers the browser bootstrap as a follow-up question after model selection, so most users never need to run `--setup-browser` directly.

What it does:

- Installs Node.js 26 into `~/.allr/node/` if missing
- `npm install -g agent-browser @askjo/camofox-browser` into that prefix (no sudo needed — `npm`'s `--prefix` points at the user-writable Allr-managed Node)
...
- Complete the ACP installation and `allr acp --check` above.
- Build `buzz-acp` and the `buzz` CLI from the
  [Buzz repository](https://github.com/block/buzz)
  (`cargo build --release -p buzz-acp`).
- Mint a dedicated Nostr keypair for Allr (`buzz-admin generate-key`) and
...
the same operating-system user that owns the intended Allr home. Setup,
...
The bridge discovers every Buzz channel where the Allr identity is a member
and automatically subscribes when it is added to another channel. Buzz channel
membership therefore remains the access boundary; Allr does not need a
separate channel list in its own configuration.

To expose Allr ACP activity in the owner's Buzz Desktop, add:
...
where Allr is expected to work.
...
2. Select **Allr** from the built-in agent list.
3. Connect and start chatting.

If you want to define Allr manually, add it through VS Code settings under `acp.agents`:
...
Configure Allr as a custom agent server in Zed settings:
...
3. Start a new Allr external-agent thread.

Prerequisites:

- Configure Allr provider credentials first with `allr model`, or set them in `~/.allr/.env` / `~/.allr/config.yaml`.

### JetBrains

Use an ACP-compatible plugin and point it at `allr acp` or `allr-acp`.

### Buzz Desktop

[Buzz](https://github.com/block/buzz) ships Allr as a preset runtime.
With Allr installed the normal way, Buzz discovers it automatically —
open **Settings → Runtimes** and Allr appears under your runtimes.
...
means its provider has no credentials configured on the Allr side.
...
change your Allr-wide default — use `allr model` for that.
...
- `approvals.mode: manual` does make Allr raise the permission request, but
...
ACP mode uses the same Allr configuration as the CLI:
...
harness) on the Allr subprocess it spawns. They are not user configuration —
do not set them by hand in `.env` or `config.yaml`.

| Variable | Value | Effect |
|----------|-------|--------|
| `ALLR_ACP_SKIP_CONFIGURED_MCP` | `1` | Skip starting the **globally configured** MCP servers from `config.yaml` before the ACP JSON-RPC loop begins. |

Allr normally starts every MCP server configured in `config.yaml` before it
...
ACP sessions bind the editor's cwd to the Allr task ID so file and terminal tools run relative to the editor workspace, not the server process cwd.
...
| `allow_always` | Allow always | All future sessions | Yes (written to the Allr permanent allowlist) |
...
- For manual/local development, verify the host command points to `allr acp`.
- Allr is installed and on your PATH.
- The ACP extra is installed (`cd ~/.allr/allr-agent && uv pip install -e '.[acp]'`).
...
or by editing `~/.allr/.env`. The terminal auth flow (`allr acp --setup`) can also trigger the interactive provider/model setup.
