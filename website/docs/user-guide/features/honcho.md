...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Allr's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Allr instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
allr memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `allr` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Allr syncs them back at session init.
...
The `allr honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `allr memory setup honcho` (or run `allr memory setup` and pick it from the list); the `allr honcho` subcommand then appears on the next invocation.

```bash
allr memory setup honcho    # Configure Honcho directly (works before activation)
allr honcho status          # Connection status, config, and key settings
allr honcho setup           # Redirects to `allr memory setup` (post-activation alias)
allr honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
allr honcho peer            # Show or update peer names + dialectic reasoning level
allr honcho mode            # Show or set recall mode (hybrid/context/tools)
allr honcho tokens          # Show or set token budget for context and dialectic
allr honcho identity        # Seed or show the AI peer's Honcho identity
allr honcho sync            # Sync Honcho config to all existing profiles
allr honcho peers           # Show peer identities across all profiles
allr honcho sessions        # List known Honcho session mappings
allr honcho map             # Map current directory to a Honcho session name
allr honcho enable          # Enable Honcho for the active profile
allr honcho disable         # Disable Honcho for the active profile
allr honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `allr honcho`

If you previously used the standalone `allr honcho setup`:
...
No re-login or re-setup needed. Run `allr memory setup` and select "honcho" — the wizard detects your existing config.
