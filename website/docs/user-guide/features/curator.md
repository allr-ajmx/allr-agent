...
On a brand-new install (or the first time a pre-curator install ticks after `allr update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `allr curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.allr/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
```bash
allr model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`allr model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
allr curator status         # last run, counts, pinned list, LRU top 5
allr curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
allr curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
allr curator run --background  # fire-and-forget: start the run in a background thread
allr curator run --dry-run  # preview only — report without any mutations
allr curator backup         # take a manual snapshot of ~/.allr/skills/
allr curator rollback       # restore from the newest snapshot
allr curator rollback --list     # list available snapshots
allr curator rollback --id <ts>  # restore a specific snapshot
allr curator rollback -y         # skip the confirmation prompt
allr curator pause          # stop runs until resumed
allr curator resume
allr curator pin <skill>    # never auto-transition this skill
allr curator unpin <skill>
allr curator adopt <skill>    # hand an unmanaged skill to the curator
allr curator adopt --all-unmanaged   # hand over every unmanaged skill
allr curator list-unmanaged   # itemize skills with no provenance marker
allr curator restore <skill>  # move an archived skill back to active
allr curator list-archived    # list skills currently in ~/.allr/skills/.archive/
allr curator archive <skill>  # manually archive a single skill now
allr curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
```

## Backups and rollback

Before every real curator pass, Allr takes a tar.gz snapshot of `~/.allr/skills/` at `~/.allr/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
allr curator rollback        # restore newest snapshot (with confirmation)
allr curator rollback -y     # skip the prompt
allr curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, Allr takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `allr curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `allr curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`allr curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
`~/.allr/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.allr/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.allr/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed Allr at an external skill
...
**To see which skills the curator actually manages**, run `allr curator status`.
...
`allr curator status` reports an **unmanaged** count alongside the managed
one:
...
unmanaged (no provenance marker): 112 total
  ...
  never auto-staled or archived — `allr curator adopt <name>` hands one over
```
...
```bash
allr curator list-unmanaged                    # itemize them, with reasons
allr curator adopt <name> [<name> ...]         # hand specific skills over
allr curator adopt --all-unmanaged --dry-run   # preview the full list
allr curator adopt --all-unmanaged             # hand over everything (prompts)
allr curator adopt --all-unmanaged --yes       # skip the prompt
```
...
`.usage.json`; read it as policy. `allr curator adopt` changes the policy, and
...
**wrote** it — Allr edits user-authored skills on your behalf constantly. An
...
- Archives are recoverable via `allr curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `allr curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `allr curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
allr curator pin <skill>
allr curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.allr/skills/.usage.json`, so it survives across sessions.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.allr/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.allr/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.allr/logs/curator/`:
...
```bash
allr curator restore <skill-name>
```

This moves the skill back from `~/.allr/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.allr/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `allr curator pause` — the pause persists across sessions; use `resume` to re-enable.
