...
Type `@` followed by a reference to inject content directly into your message. Allr expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Allr env: `$ALLR_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ALLR_HOME/skills/.hub/`
