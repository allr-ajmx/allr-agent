...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Allr' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Allr")

The wake word turns Allr into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Allr listens in the background for a spoken
trigger phrase. Say it, and Allr starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Allr backend (for example a
...
Allr supports **client capture** for that case:
...
By default the phrase is **"hey hermes"** — a model for it ships with Allr, so
...
```bash
cd ~/.allr/allr-agent && uv pip install -e ".[wake]"
```
...
desktop ear button — also writes `wake_word.enabled` to `~/.allr/config.yaml`,
...
default) to let Allr choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Allr installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Allr surfaces, and `surface` picks which one
...
including when Allr surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Allr does not silently fail over to another open surface.
...
"Hey Allr" works out of the box — the bundled openWakeWord model
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.allr/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
Create a "Hey Allr" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey hermes"
  porcupine:
    keyword: ~/.allr/wakewords/hey_hermes.ppn
```

Set your access key in `~/.allr/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `allr tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `allr-agent[wake]`).
...
but the phrase never fires. Allr detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Allr
backend (it may appear as your terminal, `python`, or Allr), then toggle the
...
```bash
allr config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
allr config set wake_word.input_device null
```
