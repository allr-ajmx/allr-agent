...
description: "Plugins shipped with Allr that run automatically via lifecycle hooks — disk-cleanup and friends"
---
...
2. **User** — `~/.allr/plugins/<name>/`
3. **Project** — `./.allr/plugins/<name>/` (requires `ALLR_ENABLE_PROJECT_PLUGINS=1`)
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `allr memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `allr plugins list` and the interactive `allr plugins` UI), but none load until you explicitly enable them:

```bash
allr plugins enable disk-cleanup
```

Or via `~/.allr/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Allr. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
allr plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `allr plugins enable <name>`.
...
| `allr-achievements` | dashboard tab | Steam-style collectible badges generated from your real Allr session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `allr memory` and `allr plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ALLR_HOME` or `/tmp/hermes-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under ALLR_HOME | always | Never |
...
**State** — everything lives at `$ALLR_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `ALLR_HOME` or `/tmp/hermes-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `allr plugins enable disk-cleanup` (or check the box in `allr plugins`).

**Disabling again:** `allr plugins disable disk-cleanup`.
...
**Enabling:** `allr plugins enable security-guidance` (or check the box in `allr plugins`).

**Disabling again:** `allr plugins disable security-guidance`.
...
Traces Allr turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Allr's canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `allr logs`.
...
```bash
allr tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Allr and the next turn ships a trace.
...
Then put the credentials in `~/.allr/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Allr turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `ALLR_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Allr session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `allr chat` session lives under one Langfuse session.

**Verify:**

```bash
allr plugins list                 # observability/langfuse should show "enabled"
allr chat -q "hello"              # check the Langfuse UI for a "Allr turn" trace
```
...
| `ALLR_LANGFUSE_RELEASE` | — | Release/version tag |
| `ALLR_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `ALLR_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `ALLR_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Allr-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Allr-prefixed wins when both are set.
...
**Disabling:** `allr plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, status) saved under `~/.allr/workspace/meetings/<meeting_id>/`

**Setup:**

```bash
allr plugins enable google_meet
allr meet setup   # preflight: playwright, chromium, auth file
allr meet auth    # opens a browser to sign into Google and saves session state —
...
**Disabling:** `allr plugins disable google_meet`. Any saved transcripts stay in `~/.allr/workspace/meetings/` until you remove them.

### allr-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Allr session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Allr feature changes.

**How it works:**

- Scans your entire `~/.allr/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$ALLR_HOME/plugins/allr-achievements/state.json`
...
| Secret | Hidden until Allr detects the first related signal in your history |

**API** — routes mount under `/api/plugins/allr-achievements/`:
...
**State files** — live under `$ALLR_HOME/plugins/allr-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Allr updates. |
...
**Enabling:** Nothing to enable — `allr-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `allr dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/allr-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.allr/plugins/allr-achievements/` that ships no dashboard. The plugin's state files under `$ALLR_HOME/plugins/allr-achievements/` survive — reinstalling preserves your unlock history.
