...
description: "Run Allr directly on an Android phone with Termux"
---

# Allr on Android with Termux
...
Allr can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Allr from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Allr now ships a Termux-aware installer path:
...
Or set keys directly in `~/.allr/.env`.
...
### `allr doctor` says ripgrep or Node is missing
