...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Allr — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Allr setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Allr Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Allr Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Allr for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Allr working on my machine | `allr setup` | Run a real chat and verify it responds |
| I already know my provider | `allr model` | Save the config, then start chatting |
| I want a bot or always-on setup | `allr gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `allr model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `allr model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Allr cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
The single most important setup step. Use `allr model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `allr setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `allr update`. Re-enable anything later with `allr tools`, seed skills with `allr skills opt-in --sync`, or tune settings with `allr setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `allr model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `allr model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `allr model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `allr model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `allr model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `allr model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `allr model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `allr model` (requires `copilot` CLI + `copilot login`) |
...
Allr requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `allr model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Allr separates secrets from normal config:

- **Secrets and tokens** → `~/.allr/.env`
- **Non-secret settings** → `~/.allr/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
allr config set model anthropic/claude-opus-4.6
allr config set terminal.backend docker
allr config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
allr gateway setup    # Interactive platform configuration
```
...
- `allr tools` — tune tool access per platform
- `allr skills` — browse and install reusable workflows
...
```bash
allr config set terminal.backend docker    # Docker isolation
allr config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `allr egress setup && allr egress start`; `allr setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Allr install directory (the curl installer placed it at
# ~/.allr/allr-agent on Linux/macOS or %LOCALAPPDATA%\allr\allr-agent on Windows):
cd ~/.allr/allr-agent
...
Skills are on-demand instruction documents that teach Allr how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Allr ships with a catalog of bundled skills already installed in `~/.allr/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
allr skills browse                      # list everything available
allr skills search kubernetes           # find skills by keyword
allr skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `allr skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Allr ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.allr/allr-agent && uv pip install -e ".[acp]"` first.)
...
- **[Moving to another machine](/reference/faq#exporting-hermes-to-another-machine)** — `allr backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
