...
sidebar_position: 2
title: "Installation"
description: "Install Allr on Linux, macOS, WSL2, native Windows, or Android via Termux"
---

# Installation

Get Allr up and running in under two minutes!
...
The root-mode **FHS layout** (`/usr/local/lib/…`, `/usr/local/bin/allr`) matches where other system-wide developer tools land on Linux. It's useful for shared-machine deployments where one system install should serve every user. Per-user config (auth, skills, sessions) still lives under each user's `~/.allr/` or explicit `ALLR_HOME`.
...
allr gateway setup  # Set up messaging platforms
allr config set     # Set individual config values
allr config get     # Inspect individual config values
allr setup          # Or run the full setup wizard to configure everything at once
```
...
| `API key not set` | Run `allr model` to configure your provider, or `allr config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `allr config check` then `allr config migrate` |

For more diagnostics, run `allr doctor` — it will tell you exactly what's missing and how to fix it.

## Install method auto-detection

Allr auto-detects whether it was installed via the git installer, Docker, or NixOS, and `allr update` prints the matching update command for that path. There's no env var to set — the detection is based on the install layout (`~/.allr/allr-agent/` checkout, Docker image stamp, or Nix store path). `allr doctor` also surfaces the detected method under its environment summary.
