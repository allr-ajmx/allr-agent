...
This page covers all commands related to [Allr profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `allr profile` without a subcommand shows help.
...
allr profile use default
```

## `allr profile create`

```bash
allr profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `allr profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `allr update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.allr`), use `allr skills opt-out` / `allr skills opt-in`. |
...
allr profile create work2 --clone-from work

# Clone everything from a specific profile
allr profile create work2-backup --clone-from work --clone-all
```

## `allr profile describe`

```bash
allr profile describe [<name>] [options]
```
...
allr profile describe researcher

# Set it explicitly
allr profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
allr profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
allr profile describe --all --auto
```

## `allr profile delete`

```bash
allr profile delete <name> [options]
```
...
```bash
allr profile delete mybot
allr profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.allr`) cannot be deleted — use `allr uninstall` to remove everything.
:::

## `allr profile show`

```bash
allr profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Allr home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ allr profile show work
Profile: work
Path:    ~/.allr/profiles/work
...
```bash
allr profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Allr installation.
...
```bash
allr profile alias work
# Creates/updates ~/.local/bin/work

allr profile alias work --name mywork
# Creates ~/.local/bin/mywork

allr profile alias work --remove
...
```bash
allr profile rename <old-name> <new-name>
```
...
```bash
allr profile rename mybot assistant
# ~/.allr/profiles/mybot → ~/.allr/profiles/assistant
...
```bash
allr profile export <name> [options]
```
...
```bash
allr profile export work
# Creates work.tar.gz in the current directory

allr profile export work -o ./work-2026-03-29.tar.gz
```

## `allr profile import`

```bash
allr profile import <archive> [options]
```
...
```bash
allr profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

allr profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`allr profile export` / `import` are still the right commands for
...
### `allr profile install`

```bash
allr profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
allr profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
allr profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
allr profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
allr profile install ./telemetry/
```

### `allr profile update`

```bash
allr profile update <name> [--force-config] [--yes]
```
...
```bash
allr profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Allr version, author, env var requirements, the source URL/path, and
...
`allr profile list` also shows the distribution name and version in a
`Distribution` column, and `allr profile show <name>` / `delete <name>`
...
allr profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
allr profile install https://github.com/your-org/internal-assistant.git
```
...
Allr version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Allr can clone from.
3. Tell recipients to run `allr profile install <your-repo-url>`.
