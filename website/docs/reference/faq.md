...
description: "Frequently asked questions and solutions to common issues with Allr"
---
...
### What LLM providers work with Allr?

Allr works with any OpenAI-compatible API. Supported providers include:
...
- **Anthropic** — Claude models (direct API, OAuth via `allr auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `allr model` or by editing `~/.allr/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run Allr in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Allr inside WSL2
...
- let Allr use the resulting MCP browser tools

This is more reliable than trying to force Allr core browser transport to attach directly across the WSL2/Windows boundary.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Allr does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.allr/`.

### Can I use it offline / with local models?

Yes. Run `allr model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Allr minimum; set this to match your server's actual context window
```
...
Allr persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Allr — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Allr auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `ALLR_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Allr itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Allr instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Allr programmatically:
...
**Cause:** Allr requires Python 3.11 or newer.
...
**Cause:** Allr builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Allr runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Allr auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.allr/config.yaml`:
...
#### The agent says "Allr policy" or "Allr guardrails" refused my request

A model cannot reliably identify why it refused a request. If the refusal appears only in the assistant's prose, its claim that a hidden Allr runtime policy caused it may be a hallucinated explanation or a restriction applied by the selected model or provider.

Allr enforcement is explicit: a blocked tool action returns a tool error naming the denied command or path, and an approval-required action shows an approval prompt. Allr does not silently turn those execution controls into a general content-refusal layer. Provider-level controls can still apply when configured, such as Amazon Bedrock Guardrails.
...
**Solution:** Exit your session and use `allr model` from your terminal to add new providers:

```bash
# Exit the Allr chat session first (Ctrl+C or /quit)
...
After adding a new provider via `allr model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `allr model` (from terminal) |
| Enter/change API keys | `allr model` (from terminal) |
...
# Or set directly
allr config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.allr/.env` for conflicting entries.
:::
...
# Set a valid model
allr config set ALLR_MODEL anthropic/claude-opus-4.7

# Or specify per-session
allr chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `allr chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Allr detected the wrong context length for your model.
...
allr chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Allr may have the wrong context length for your model. Check what it detected:
...
**Cause:** Allr detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Allr never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `allr chat`
...
cat ~/.allr/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `allr gateway setup`
- Check gateway logs: `cat ~/.allr/logs/gateway.log | tail -50`
...
Configure in `~/.allr/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.allr/allr-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `allr gateway start` fails
...
tmux new -s hermes 'allr gateway run'
# Reattach later: tmux attach -t hermes

# Option 3: Background via nohup
nohup allr gateway run > ~/.allr/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'allr gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `allr gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
allr gateway install    # Re-snapshots your current PATH
allr gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/work.allr.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `allr chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `allr chat -t "terminal"`
...
Verify your `~/.allr/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
allr config show | grep -A 12 mcp_servers

# Restart Allr or reload MCP after config changes
allr chat
```
...
:::warning
If an MCP server crashes mid-request, Allr will report a timeout. Check the server's own logs (not just Allr logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `allr profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

This isolation is also the reason to never run two agents against the *same* profile or Allr home: both write memory automatically and each loads the other's writes at session start, so their stored state degrades with every session. One agent per profile; for genuinely shared memory across agents, use an [external memory provider](/user-guide/features/memory-providers).

### What happens when I run `allr update`?

`allr update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `allr update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.allr/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Allr can route subagents to a different model automatically. Set this in `~/.allr/config.yaml`:
...
Now when you tell Allr "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Allr do this?

**Current limitation:** Allr profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Allr reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `allr skills config` settings don't seem to take effect.

**Solution:** Use `allr skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`allr gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Allr creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Allr to another machine
...
1. Install Allr on the new machine:
   ```bash
   curl -fsSL https://allr.work/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   allr backup
   ```
   This creates a zip of your entire `~/.allr/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/hermes-backup-<timestamp>.zip`.
...
3. Copy the zip to the new machine and import it:
   ...
   # On the new machine
   allr import ~/hermes-backup-<timestamp>.zip
   ```

4. On the new machine, run `allr setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
allr profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
allr profile import ./work-backup.tar.gz work
```
...
### `allr backup` vs `allr profile export`

| Feature | `allr backup` | `allr profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.allr` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='allr-agent' ~/.allr/ newmachine:~/.allr/
```

:::tip
`allr backup` produces a consistent snapshot even while Allr is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::
...
allr config show | head -20

# Re-run model selection
allr model

# Or test with a known-good model
allr chat -q "hello" --model anthropic/claude-opus-4.7
```
