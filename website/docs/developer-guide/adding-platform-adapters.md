...
This guide covers adding a new messaging platform to the Allr gateway. A platform adapter connects Allr to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.allr/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core Allr code. Your plugin is a directory with two files:

```
~/.allr/plugins/my-platform/
...
| `allr config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `allr status` | Shows plugin platforms with `(plugin)` tag |
| `allr gateway setup` | Plugin platforms appear in setup menu |
| `allr tools` / `allr skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.allr/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `allr gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `allr cron run` separate from `allr gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `allr config`
...
version: 1.0.0
description: >
  My Platform gateway adapter for Allr.
author: Your Name
requires_env:
...
:::note
This checklist is for adding a platform directly to the Allr core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`toolsets.py`** — Add `"hermes-newplat"` toolset definition with `_ALLR_CORE_TOOLS`
2. **`toolsets.py`** — Add `"hermes-newplat"` to the `"allr-gateway"` includes list
