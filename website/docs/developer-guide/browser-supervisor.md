...
description: "How Allr detects and responds to native JS dialogs and interacts with cross-origin iframes via a persistent CDP connection."
---
...
One `asyncio.Task` running in a background daemon thread per Allr `task_id`.
...
- `toolsets.py` — registers `browser_dialog` in `browser`, `allr-acp`, `hermes-api-server`, and core toolsets (gated on CDP reachability)
