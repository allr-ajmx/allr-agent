...
description: "How Allr stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `hermes_cli/cron.py` | CLI `allr cron` subcommands |
...
Jobs are stored in `~/.allr/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `allr cron` commands are run or during active CLI sessions.
...
```python
# ~/.allr/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.allr/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `allr cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `allr cron` CLI provides direct job management:

```bash
allr cron list                    # Show all jobs
allr cron create                  # Interactive job creation (alias: add)
allr cron edit <job_id>           # Edit job configuration
allr cron pause <job_id>          # Pause a running job
allr cron resume <job_id>         # Resume a paused job
allr cron run <job_id>            # Trigger immediate execution
allr cron remove <job_id>         # Delete a job
```
