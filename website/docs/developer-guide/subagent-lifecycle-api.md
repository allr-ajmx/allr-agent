...
Plugins can launch and supervise fresh Allr child sessions without importing
...
active agent turn fails closed with `No active Allr parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Allr can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Allr's existing unsafe-tool block remains enforced.
