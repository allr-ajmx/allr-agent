...
description: "Three protocols for driving allr-agent from external programs: ACP, the TUI gateway JSON-RPC, and the OpenAI-compatible HTTP API"
---

# Programmatic Integration

Allr ships three protocols for driving the agent from external programs — IDE plugins, custom UIs, CI pipelines, embedded sub-agents. Pick the one that matches your transport and consumer.
...
`allr acp` starts a stdio JSON-RPC server speaking ACP. Used in production by VS Code (Zed Industries' ACP extension), Zed, and any JetBrains IDE with an ACP plugin.
...
```bash
allr acp                  # serve ACP on stdio
allr acp --check          # verify ACP dependencies and adapter imports
allr acp --setup          # interactive provider/model setup for ACP terminal auth
```
...
GET  /v1/models                  Lists allr-agent
GET  /api/model/options          Provider-aware picker inventory
GET  /health, /health/detailed
```

Setup, headers (`X-Allr-Session-Id`, `X-Allr-Session-Key`), and frontend wiring: [API Server](../user-guide/features/api-server).
...
- Dashboard backend REST: `GET /api/model/options` with `X-Allr-Session-Token`
...
- **You're writing a custom desktop / web / TUI host and want every Allr feature** (slash commands, approvals, clarify, multi-agent, session branching) → TUI gateway JSON-RPC.
...
Allr does not have a `--mode rpc` flag. The three protocols above already cover the use cases — ACP for IDE-protocol clients, the TUI gateway for stdio JSON-RPC hosts, and the API server for HTTP. If you find a real gap that none of them fill, open an issue with the concrete consumer you're building.
