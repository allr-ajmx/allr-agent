...
sidebar_position: 1
title: "Architecture"
description: "Allr internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Allr internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── hermes_constants.py       # ALLR_HOME, profile-aware paths
...
│   ├── main.py               # Entry point — all `allr` subcommands (large file)
...
│   ├── skills_config.py      # allr skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # allr tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # allr gateway start/stop
│
...
Three discovery sources: `~/.allr/plugins/` (user), `.allr/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `allr plugins` or `config.yaml`.
...
Exposes Allr as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`allr -p <name>`) gets its own ALLR_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
