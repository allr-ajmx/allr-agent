...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Allr already has applies. The plugin never sees a
...
allr-agent itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Allr provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Allr provider
        # (must be one Allr already knows about — same names as
        # `allr model` and config.yaml model.provider).
        ...
        # plugin sends — Allr does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.allr/auth.json` / env, including the credential pool when
...
Existing `ctx.*` methods extend an existing Allr subsystem:
