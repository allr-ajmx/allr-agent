...
This tutorial walks you through setting up a Telegram bot powered by Allr that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Allr installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.allr/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Allr itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Allr Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Allr. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u allr-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `allr gateway install` to update the plist.
:::
...
```bash
allr gateway stop && allr gateway start
```
...
allr pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.allr/.env`:
...
Control how much detail the bot shows when using tools. In `~/.allr/config.yaml`:
...
```bash
# From the CLI
allr cron list          # View all scheduled jobs
allr cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u allr-gateway` (Linux) or `~/.allr/logs/gateway.log` (macOS) |
| Cron job output | `~/.allr/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.allr/cron/jobs.json` |
| Pairing data | `~/.allr/pairing/` |
| Session history | `~/.allr/sessions/` |
