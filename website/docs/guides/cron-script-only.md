...
Allr calls this **no-agent mode**. It's the cron system minus the LLM.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, Allr writes the script, schedules it, and tells you when it'll fire.

### Example transcript

> **You:** ping me on telegram if RAM is over 85% every 5 minutes
>
> **Allr:** *(writes `~/.allr/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
>
...
write_file(
    path="~/.allr/scripts/memory-watchdog.sh",
...
>
> **Allr:** *(calls `cronjob(action='pause', job_id='abc123')`)*
>
> Paused. Resume with "turn it back on" or via `allr cron resume abc123`.

> **You:** change it to every 15 minutes
>
> **Allr:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
```bash
# 1. Write your script
cat > ~/.allr/scripts/memory-watchdog.sh <<'EOF'
#!/usr/bin/env bash
...
EOF
chmod +x ~/.allr/scripts/memory-watchdog.sh
...
# 3. Verify
allr cron list
allr cron run <job_id>    # fire it once to test
```
...
Scripts must live in `~/.allr/scripts/`. This is enforced at both job-creation time and run time — absolute paths, `~/` expansion, and path-traversal patterns (`../`) are rejected. The same directory is shared with the pre-check script gate used by LLM jobs.
...
```bash
allr cron create "every 5m"        # interval
allr cron create "every 2h"
allr cron create "0 9 * * *"       # standard cron: 9am daily
allr cron create "30m"             # one-shot: run once in 30 minutes
```
...
--deliver local                          # just save to ~/.allr/cron/output/
```

No running gateway is required at script-run time for bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp) — the tool calls each platform's REST endpoint directly using the credentials already in `~/.allr/.env` / `~/.allr/config.yaml`.

## Editing and Lifecycle

```bash
allr cron list                                    # see all jobs
allr cron pause <job_id>                          # stop firing, keep definition
allr cron resume <job_id>
allr cron edit <job_id> --schedule "every 10m"    # adjust cadence
allr cron edit <job_id> --agent                   # flip to LLM mode
allr cron edit <job_id> --no-agent --script …     # flip back
allr cron remove <job_id>                         # delete it
```
...
```bash
cat > ~/.allr/scripts/disk-alert.sh <<'EOF'
#!/usr/bin/env bash
...
'
EOF
chmod +x ~/.allr/scripts/disk-alert.sh
