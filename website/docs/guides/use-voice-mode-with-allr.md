---
sidebar_position: 8
title: "Use Voice Mode with Allr"
description: "A practical guide to setting up and using Allr voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Allr
...
- you want Allr sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Allr.
...
## Step 1: make sure normal Allr works first
...
Allr supports both local and cloud speech stacks.
...
### If you use `allr setup`

If you choose NeuTTS in the setup wizard, Allr checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Allr transcribes and responds
...
- asking Allr to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Allr loop.

## Tuning CLI behavior

### Silence threshold

If Allr starts/stops too aggressively, tune:
...
Allr stays a normal chat bot, but can speak replies.
...
- you want Allr to function like a portable research or ops assistant
...
Allr joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Allr detects speech boundaries
- transcripts are posted in the associated text channel
- Allr responds in text and audio
...
1. get text Allr working
2. run `allr setup tts` to enable voice support
