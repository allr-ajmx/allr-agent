...
```bash
allr teams-pipeline token-health
allr teams-pipeline token-health --force-refresh
```
...
```bash
allr teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
allr teams-pipeline maintain-subscriptions
allr teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Allr cron (recommended if you already run the Allr gateway)

Allr ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.allr/scripts/`. First create the script:

```bash
mkdir -p ~/.allr/scripts
cat > ~/.allr/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec allr teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.allr/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
allr cron create "0 */12 * * *" \
...
```ini
[Unit]
Description=Allr Teams pipeline subscription maintenance
...
ExecStart=/usr/local/bin/allr teams-pipeline maintain-subscriptions
```
...
```ini
[Unit]
Description=Run Allr Teams pipeline subscription maintenance every 12 hours
...
```cron
0 */12 * * * /usr/local/bin/allr teams-pipeline maintain-subscriptions >> /var/log/hermes/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.allr/.env` at the top of a wrapper script that crontab calls.
...
```bash
allr teams-pipeline subscriptions   # should show expirationDateTime advanced
allr teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
allr teams-pipeline list
allr teams-pipeline list --status failed
allr teams-pipeline show <job-id>
```

### Replay a stored job

```bash
allr teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
allr teams-pipeline fetch --meeting-id <meeting-id>
allr teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `allr teams-pipeline maintain-subscriptions --dry-run`
- inspect `allr teams-pipeline list --status failed`
...
- run `allr teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `allr teams-pipeline run`
...
- [ ] `allr teams-pipeline validate` returns an OK snapshot
- [ ] `allr teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Allr cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
