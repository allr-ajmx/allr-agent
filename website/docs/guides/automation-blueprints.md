...
Copy-paste blueprints for common automation patterns. Each blueprint uses Allr's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`allr webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `allr webhook subscribe`) |
...
```bash
allr webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
allr cron create "0 6 * * *" \
  "Run a dependency security audit on the allr-agent project.

1. cd ~/.allr/allr-agent && source .venv/bin/activate
...
```bash
allr webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
allr webhook subscribe alert-triage \
...
```python title="~/.allr/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Allr-Monitor/1.0"})
...
allr cron create "every 30m" \
  ...
  --script ~/.allr/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
allr cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Allr development." \
...
```bash
allr webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
allr cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the allr-agent codebase.
