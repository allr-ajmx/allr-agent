...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `allr send`."
---

# Pipe Script Output to Messaging Platforms

`allr send` is a small, scriptable CLI that pushes a message to any
messaging platform Allr is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | allr send --to slack:#builds`)
...
allr send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
allr send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
allr send --list

# Filter by platform
allr send --list telegram
```
...
Any platform Allr ships adapters for works as a target:
...
`allr send` resolves the message body in this order:

1. **Positional argument** — `allr send --to telegram "hi"`
2. **`--file PATH`** — `allr send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | allr send --to telegram`

When stdin is a TTY (no pipe), Allr does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  allr send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `allr send` reuses your Allr config, the same script works on
any host where Allr is installed — no need to export bot tokens into
...
`allr send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  allr send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | allr send \
...
```bash
./train.py --epochs 200 && \
  allr send --to telegram "training done" || \
  allr send --to telegram "training failed (exit $?)"
```
...
allr send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(allr send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `allr send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `allr send` calls
the platform's REST endpoint directly using credentials from
`~/.allr/.env` and `~/.allr/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `allr gateway start` and retry.
...
The listing is built from `~/.allr/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`allr gateway
...
| Approach | Multi-platform | Reuses Allr creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `allr send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`allr send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `allr send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `allr send` shares with cron delivery.
