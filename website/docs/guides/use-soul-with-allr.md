---
sidebar_position: 7
title: "Use SOUL.md with Allr"
description: "How to use SOUL.md to shape Allr's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Allr

`SOUL.md` is the **primary identity** for your Allr instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Allr to feel like the same assistant every time you talk to it — or if you want to replace the Allr persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Allr should be
- what Allr should avoid stylistically
- how Allr should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Allr is and how Allr speaks
...
Allr now uses only the global SOUL file for the current instance:

```text
~/.allr/SOUL.md
```

If you run Allr with a custom home directory, it becomes:
...
Allr automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Allr does not overwrite it
- if the file exists but is empty, Allr adds nothing from it to the prompt

## How Allr uses it

When Allr starts a session, it reads `SOUL.md` from `ALLR_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Allr feels.
...
Allr already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Allr should sound.

# Avoid
What Allr should not do.

# Defaults
How Allr should behave when ambiguity appears.
```
...
Then restart Allr or start a new session.
...
### I edited SOUL.md but Allr still sounds the same

Check:
- you edited `~/.allr/SOUL.md` or `$ALLR_HOME/SOUL.md`
...
### Allr is ignoring parts of my SOUL.md
