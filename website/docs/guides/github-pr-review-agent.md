...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Allr  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Allr instantly when PRs are opened or updated.
:::
...
- **Allr installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  allr gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.allr/cron/output/`. Great for testing before wiring up notifications.
:::
...
Still in the chat, ask Allr to review a real PR:
...
A skill gives Allr consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.allr/skills/code-review
```

Create `~/.allr/skills/code-review/SKILL.md`:
...
This is what makes the reviewer actually useful. Start a session and teach Allr your team's standards:
...
2. Teach Allr your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
allr gateway status    # Is the gateway running?
allr cron list         # Is the job enabled?
```
