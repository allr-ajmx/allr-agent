...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      allr-agent = self'.packages.default;
      hermesVenv = allr-agent.hermesVenv;
    ...
    in {
      ...
      checks = {
        ...
        build-package = pkgs.runCommand "hermes-build-package" { } ''
          echo "PASS: package built at ${allr-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';
        ...
        package-contents = pkgs.runCommand "hermes-package-contents" { } ''
          ...
          test -x ${allr-agent}/bin/allr || (echo "FAIL: hermes binary missing"; exit 1)
          test -x ${allr-agent}/bin/allr-agent || (echo "FAIL: allr-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${allr-agent}/bin/allr version 2>&1 | grep -qi "hermes" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "hermes-entry-points-sync" { } ''
          ...
          for bin in hermes allr-agent allr-acp; do
            test -x ${allr-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "hermes-cli-commands" { } ''
          ...
          ${allr-agent}/bin/allr --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${allr-agent}/bin/allr --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "hermes-bundled-skills" { } ''
          ...
          test -d ${allr-agent}/share/allr-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          # -L: skills/ is a symlink to the filtered source store path
          SKILL_COUNT=$(find -L ${allr-agent}/share/allr-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ALLR_BUNDLED_SKILLS" ${allr-agent}/bin/allr || \
            (echo "FAIL: ALLR_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ALLR_BUNDLED_SKILLS set in wrapper"

          # Optional skills ship via the wrapper too (pythonSrc excludes
          # them from the wheel, so the env var is the only path in nix).
          test -d ${allr-agent}/share/allr-agent/optional-skills || \
            (echo "FAIL: optional-skills directory missing"; exit 1)
          OPT_COUNT=$(find -L ${allr-agent}/share/allr-agent/optional-skills -name "SKILL.md" | wc -l)
          test "$OPT_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files in optional-skills"; exit 1)
          grep -q "ALLR_OPTIONAL_SKILLS" ${allr-agent}/bin/allr || \
            (echo "FAIL: ALLR_OPTIONAL_SKILLS not in wrapper"; exit 1)
          echo "PASS: $OPT_COUNT optional skills found, ALLR_OPTIONAL_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "hermes-bundled-plugins" { } ''
          ...
          test -d ${allr-agent}/share/allr-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${allr-agent}/share/allr-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "ALLR_BUNDLED_PLUGINS" ${allr-agent}/bin/allr || \
            (echo "FAIL: ALLR_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: ALLR_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "hermes-bundled-locales" { } ''
          ...
          test -d ${allr-agent}/share/allr-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          # -L: locales/ is a symlink to the source store path
          LOC_COUNT=$(find -L ${allr-agent}/share/allr-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${allr-agent}/share/allr-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "ALLR_BUNDLED_LOCALES" ${allr-agent}/bin/allr || \
            (echo "FAIL: ALLR_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: ALLR_BUNDLED_LOCALES set in wrapper"

          # locales/ is a bare data dir (no __init__.py), shipped via a
          # symlink + ALLR_BUNDLED_LOCALES (not via wheel data-files).
          # Verify the wrapper override resolves real strings.
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && ALLR_BUNDLED_LOCALES=${allr-agent}/share/allr-agent/locales \
            ${hermesVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with ALLR_BUNDLED_LOCALES set"; exit 1)
        ...
        # ALLR_OPTIONAL_MCPS (not via wheel data-files).
        bundled-mcps = pkgs.runCommand "hermes-bundled-mcps" { } ''
          set -e
          echo "=== Checking bundled optional-mcps ==="
          test -d ${allr-agent}/share/allr-agent/optional-mcps || (echo "FAIL: optional-mcps directory missing"; exit 1)
          echo "PASS: optional-mcps directory exists"

          MANIFEST_COUNT=$(find -L ${allr-agent}/share/allr-agent/optional-mcps -name "manifest.yaml" | wc -l)
          test "$MANIFEST_COUNT" -gt 0 || (echo "FAIL: no manifest.yaml files found"; exit 1)
          echo "PASS: $MANIFEST_COUNT catalog manifests found"

          grep -q "ALLR_OPTIONAL_MCPS" ${allr-agent}/bin/allr || \
            (echo "FAIL: ALLR_OPTIONAL_MCPS not in wrapper"; exit 1)
          echo "PASS: ALLR_OPTIONAL_MCPS set in wrapper"

          export HOME=$(mktemp -d)
          CATALOG=$(cd "$HOME" && ${allr-agent}/bin/allr mcp catalog 2>/dev/null || true)
          echo "catalog output: $CATALOG"
          test -n "$CATALOG" || (echo "FAIL: allr mcp catalog returned empty"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "hermes-bundled-tui" { } ''
          ...
          test -d ${allr-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${allr-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "ALLR_TUI_DIR" ${allr-agent}/bin/allr || \
            (echo "FAIL: ALLR_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ALLR_TUI_DIR set in wrapper"
        ...
        # Verify ALLR_NODE is set in wrapper and points to Node 26+
        # (Allr pins its toolchain to Node 26 everywhere)
        hermes-node = pkgs.runCommand "hermes-node-version" { } ''
          set -e
          echo "=== Checking ALLR_NODE in wrapper ==="
          grep -q "ALLR_NODE" ${allr-agent}/bin/allr || \
            (echo "FAIL: ALLR_NODE not set in wrapper"; exit 1)
          echo "PASS: ALLR_NODE present in wrapper"

          ALLR_NODE=$(sed -n "s/^export ALLR_NODE='\(.*\)'/\1/p" ${allr-agent}/bin/allr)
          test -x "$ALLR_NODE" || (echo "FAIL: ALLR_NODE=$ALLR_NODE not executable"; exit 1)
          echo "PASS: ALLR_NODE executable at $ALLR_NODE"

          NODE_MAJOR=$("$ALLR_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 26 || \
            (echo "FAIL: Node v$NODE_MAJOR < 26, Allr requires Node 26"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 26"

          echo "=== All ALLR_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ALLR_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "hermes-managed-guard" { } ''
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ALLR_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ALLR_MANAGED guards ==="
          check_blocked "config set" ${allr-agent}/bin/allr config set model foo
          check_blocked "config edit" ${allr-agent}/bin/allr config edit
        ...
        extra-python-packages = let
          ...
          hermesWithExtra = allr-agent.override {
        ...
        in pkgs.runCommand "hermes-extra-python-packages" { } ''
          ...
          grep -q "PYTHONPATH" ${hermesWithExtra}/bin/allr || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${hermesWithExtra}/bin/allr || \
          ...
          if grep -q "PYTHONPATH" ${allr-agent}/bin/allr; then
        ...
        extra-dependency-groups = let
          hermesWithGroups = allr-agent.override {
