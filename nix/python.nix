...
let
    ...
    (callPackage pyproject-nix.build.packages {
      ...
      (
        lib.composeManyExtensions [
          ...
          # Allr derivation. This is deliberately a derivation environment
          # variable, not a devShell variable: ``nix develop -c uv build``
          # must remain blocked.
          (final: prev: {
            allr-agent = prev.allr-agent.overrideAttrs (_old: {
              ALLR_NIX_BUILD = "1";
            });
          })
        ]
  ...
  # $ALLR_PYTHON_SRC_ROOT at runtime.
  workspaceRoot = ./..;
  ...
  editableOverlay = editableWorkspace.mkEditablePyprojectOverlay {
    root = "$ALLR_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        allr-agent = prev.allr-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "allr-agent-env" {
    allr-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "allr-agent-editable-env" {
    allr-agent = dependency-groups;
  };
}
