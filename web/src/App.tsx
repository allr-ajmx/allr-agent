          ...
          >
            ...
            >
              ...
              >
                ...
                <Typography className="font-bold text-[1.125rem] leading-[0.95] tracking-[0.0525rem] text-midground">
                  {t.app.brand}
                </Typography>
              </div>
...
function SidebarSystemActions({
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'allr update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "allr update";
    return (
      t.status.updateHermesConfirmMessage ??
      `This will run 'allr update' (${cmd}) and restart the gateway when it finishes.`
    );
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Allr gateway process. Connected channels and active sessions will reconnect afterward."
      }
