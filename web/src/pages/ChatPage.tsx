...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__ALLR_SESSION_TOKEN__ &&
    !window.__ALLR_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `allr dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__ALLR_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Allr TUI has enabled xterm mouse-events
    ...
    //      payloads upload to ALLR_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to ALLR_HOME/images, then drive `/image`
    ...
    // WebSocket. In gated mode (``window.__ALLR_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets ALLR_HOME pointed at the
    ...
    // forwarding them into Allr. Keyboard input, paste, and resize still
    // behave normally.
