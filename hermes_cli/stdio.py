...
on Japanese locales, etc.).  Allr's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Allr is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``ALLR_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("ALLR_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Allr-managed Git install directories so
    ...
    # install without Allr-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Allr's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Allr's
    ``allr config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Allr (User env var in Windows Settings, or
    export in a PowerShell profile) and Allr picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Allr-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\allr\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess Allr spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Allr-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Allr venv Scripts directory — host of the allr.exe shim itself,
        ...
        os.path.join(local_appdata, "hermes", "allr-agent", "venv", "Scripts"),
