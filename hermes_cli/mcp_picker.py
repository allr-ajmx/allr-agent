"""MCP picker — interactive `allr mcp picker` (also the default `allr mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``allr mcp add``, lets them pick one, and routes to install / enable /
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Allr session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `allr mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: allr mcp install <name>    Picker: allr mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Allr is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Allr — run `allr update` "
...
def show_catalog() -> None:
    """`allr mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`allr mcp picker` (and default `allr mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`allr mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `allr mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
