...
Shows the status of all Allr components.
"""
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``allr config``'s
...
def _estop_status_line():
    """One-line pause banner for `allr status`, or None when not paused.

    Cheap: a single stat on $ALLR_HOME/ESTOP via agent.estop.
    """
    try:
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `allr resume` to lift)"


def show_status(args):
    """Show status of all Allr components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 Allr Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `allr status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: allr portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: allr model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: allr auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: allr auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: allr model)"
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'allr-agent[vercel]')"
    ...
    print(color("  Run 'allr doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'allr setup' to configure", Colors.DIM))
    print()
