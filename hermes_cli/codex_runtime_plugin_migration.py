"""Migrate Allr' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Allr' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Allr mcp_servers.<n>.command/args/env  → codex stdio transport
  Allr mcp_servers.<n>.url/headers       → codex streamable_http transport
  Allr mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Allr mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Allr-specific keys (sampling, etc.) — codex's MCP client has no
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Allr config.")
...
# Allr keys that codex's MCP schema doesn't support — dropped during
...
def _translate_one_server(
    ...
    """Translate one Allr MCP server config to the codex inline-table dict
    ...
    for key in hermes_cfg:
        ...
        elif key not in _KNOWN_ALLR_KEYS:
            skipped.append(f"{key} (unknown Allr key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (ALLR_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Allr)")
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Allr
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a ALLR_HOME pointing
    ...
    ``~/.allr`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    ALLR_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # ALLR_HOME passes through IF SET so the MCP subprocess sees the same
    ...
    # codex's subprocess to inherit whatever ALLR_HOME its launcher sets
    ...
    # — that would override the launcher's ALLR_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("ALLR_HOME", tmp_path) would
    ...
    hermes_home = os.environ.get("ALLR_HOME") or ""
    ...
    env["ALLR_QUIET"] = "1"
    env["ALLR_REDACT_SECRETS"] = env.get("ALLR_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    """Translate Allr mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        hermes_config: full ~/.allr/config.yaml dict
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in Allr config is not a dict; cannot migrate."
        )
        return report
