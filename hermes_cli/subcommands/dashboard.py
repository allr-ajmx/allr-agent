"""``allr dashboard`` / ``allr serve`` subcommand parsers.
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `allr dashboard` / `allr serve` cmdlines and SIGTERM them directly —
    # the same path `allr update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Allr web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Allr web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Allr web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Allr desktop app shells (<= 0.15.x) spawn the
    # backend as `allr dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Allr couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Allr Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Allr backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Allr backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports ALLR_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `allr dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.allr/.env.
    # Nested subparser so bare `allr dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "ALLR_DASHBOARD_OAUTH_CLIENT_ID into ~/.allr/.env, and prints "
            "how to engage the login gate. Requires being logged in (allr setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "portal. Also settable via ALLR_DASHBOARD_PORTAL_URL. Mainly for "
