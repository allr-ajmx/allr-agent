"""``allr sync`` subcommand parser — Skill Sync.
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    allr sync status                 show gate/opt-in/head state
    allr sync pull                   pull and materialize opted-in skills
    allr sync push                   push opted-in skills
    allr sync now                    reconcile: pull then push
    allr sync enable <skill>         opt a skill into sync
    allr sync disable <skill>        opt a skill out of sync
    allr sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    allr sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  allr sync status            what is synced, and from where\n"
            "  allr sync enable my-skill   include a skill in your sync\n"
            "  allr sync now               pull, then push\n"
            "  allr sync propose my-skill  share a skill with your team\n"
        ),
