...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``allr login`` was removed in favor of ``allr auth`` / ``allr model``
    ...
    old scripts/aliases invoking ``allr login [--flags]`` still receive the
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``allr login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``allr model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `allr auth` to manage credentials, "
            "`allr model` to select a provider, or `allr setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `allr model`",
    )
    login_parser.add_argument(
