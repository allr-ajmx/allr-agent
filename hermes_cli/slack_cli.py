"""``allr slack ...`` CLI subcommands.

Today only ``allr slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ allr slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Allr in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Allr in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Allr agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$ALLR_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Allr")
    ...
    """
    name = getattr(args, "name", None) or "Allr"
    description = getattr(args, "description", None) or "Your Allr agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "allr slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"allr slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "allr slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "allr slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Allr app\n"
