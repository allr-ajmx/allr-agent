"""
Unified tool configuration for Allr.

`allr tools` and `allr setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.allr/config.yaml under
...
def _post_setup_no_window_flags(*, streams_to_console: bool = False) -> int:
    ...
    ``allr tools post-setup <key>`` child. On Windows, every console child
...
# They're still in _ALLR_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `allr tools` → Video Generation, which walks
...
# `allr tools` → X (Twitter) Search setup walks users through credential
# setup. The tool's check_fn means the schema still won't appear to the
# model if the credential later goes missing or expires.
_DEFAULT_OFF_TOOLSETS = {"homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "a2a"}


# Config-only capabilities: they appear in `allr tools` for provider/API-key
...
# Platform-scoped toolsets: only appear in the `allr tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``allr tools`` → "reconfigure existing" would
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``allr tools`` checklist actually offers
    for ``platform``.
    ...
    so ``allr tools`` never claims to add or remove a toolset the user was
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Allr routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set ALLR_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ALLR_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ALLR_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ALLR_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ALLR_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ALLR_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `allr model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("ALLR_CUA_DRIVER_CMD", "").strip() or "cua-driver"
...
def _cua_driver_env() -> dict:
    """cua-driver child env with the Allr telemetry policy applied.
...
def _pip_install(
    ...
    *,
    ...
    # Managed uv first: $ALLR_HOME/bin is never on PATH, so a bare which()
    # misses the uv Allr installed and prefers a system one when both exist.
...
def install_cua_driver(
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``allr update`` and
      by ``allr computer-use install --upgrade``.
    ...
    falling through to the full upstream installer. ``allr update`` sets
    ...
    a further ~600s wait on Windows). ``allr computer-use install
    ...
    ``allr update`` already prints a contextual line before its update
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `allr update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `allr computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
    ...
    # caller: `allr update` (require_confirmed_update=True) keeps the
    ...
    # `allr computer-use install --upgrade` falls through and re-runs the
    # installer as before.
    ...
    if binary:
        ...
        if _state is None and require_confirmed_update:
            ...
            _print_info(
                "    Force a refresh with: allr computer-use install --upgrade"
            )
            return True
...
def _run_cua_driver_installer(
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `allr update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `allr update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.allr/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            ...
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Allr process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        # Managed Node first — $ALLR_HOME/node is not on PATH, so a bare
        # which() reports "no npm" on installs whose only Node is the one
        # Allr installed for exactly this toolchain.
        ...
        if not node_modules.exists() and npm_bin:
            ...
            result = subprocess.run(
                ...
                # only, avoiding the apps/* glob which would pull in the app
                # workspaces unnecessarily. See #38772.
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hermes_home()}/allr-agent && npm install --workspaces=false")
                if result.stderr:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in allr-agent directory)")
            return
        ...
        if camofox_dir.exists():
            ...
            result = subprocess.run(
                # --workspaces=false avoids resolving the app workspaces. See #38772.
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_info("    Change via stt.local.model in ~/.allr/config.yaml")
            else:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.allr/config.yaml")
        ...
        # Run the full `allr auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.allr/.env), then continues straight into PKCE. If they
        ...
        try:
            ...
            _print_info("    Run manually: allr auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `allr auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: allr auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run manually: allr plugins enable observability/langfuse")
        _print_info("    Restart Allr for tracing to take effect.")
        _print_info("    Verify: allr plugins list")
        ...
        try:
            ...
            _print_info("    Run later: allr auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `allr auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: allr auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: allr auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``allr tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``allr tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: allr tools post-setup <key>")
        return 2
...
#:
#: Saving ``allr tools`` (or one toggle in the desktop Toolsets UI) replaces
...
def _enable_recently_shipped_toolsets(
    ...
    Either way of saying no outlives this: unchecking in ``allr tools``
    ...
    for ts_key in sorted(_RECENTLY_SHIPPED_TOOLSETS):
        ...
        # narrow composites (allr-acp, hermes-webhook) stay narrow.
...
def _get_platform_tools(
    ...
    *,
    ...
    from toolsets import PLATFORM_TOOLSET_PREFIXES, resolve_toolset, TOOLSETS
    ...
    # "hermes-cli" (which include all _ALLR_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # through ``allr tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `allr tools` (which flips has_explicit_config
    ...
    skip |= {k for k in TOOLSETS if k.startswith(PLATFORM_TOOLSET_PREFIXES)}
    ...
    # shipped as a bundled plugin but user must opt in via `allr tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `allr tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by allr tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # at runtime, not only during the next `allr update`/`allr doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `allr tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `allr tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `allr tools` gets a silent no-op
...
def _agent_browser_installed() -> bool:
    ...
    # The install hook runs in a spawned ``allr tools post-setup`` process,
...
# (apps/hermes-universal/src/app/settings/constants.ts).
STT_MODEL_CATALOG = {
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `allr model`).
    if managed_feature:
        from hermes_cli.nous_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``allr model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    substrate — so this picker lists exactly what the ``allr model`` aux-task
    ...
    if not providers:
        _print_warning(
            ...
            "with `allr model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `allr model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `allr tools` and `allr setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("Allr Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://allr.work/docs/user-guide/features/tools", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, allr mcp
