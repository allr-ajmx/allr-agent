"""allr hooks — inspect and manage shell-script hooks.

Usage::

    allr hooks list
    allr hooks test <event> [--for-tool X] [--payload-file F]
    allr hooks revoke <command>
    allr hooks doctor

Consent records live under ``~/.allr/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.allr/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``allr hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: allr hooks {list|test|revoke|doctor}")
        print("Run 'allr hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs and not outbound:
        print("No shell hooks or outbound webhooks configured in ~/.allr/config.yaml.")
        print("See `allr hooks --help` or")
        print("    website/docs/user-guide/features/hooks.md")
        print("for the config schema and worked examples.")
        return

    if not specs:
        print("No shell hooks configured in ~/.allr/config.yaml.")
    else:
                    ...
                    if entry and entry.get("approved_at"):
                        ...
                        if mtime_now and mtime_at and mtime_now > mtime_at:
                            print(
                                ...
                                f"run `allr hooks doctor` to re-validate"
                            )
            print()
...
# stdin a script sees under `allr hooks test` and `allr hooks doctor`
...
def _print_parsed(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Allr wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `allr hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `allr hooks doctor` would execute
