...
manual BotFather token copy-paste. Allr talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.allr.work"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Allr Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Allr Telegram onboarding service.")
