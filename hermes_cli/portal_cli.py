"""``allr portal`` — the human-readable entry point for Nous Portal.

Running ``allr portal`` with no subcommand performs the one-shot Portal
...
``allr auth add nous --type oauth`` (which still works), is identical to
``allr setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``allr auth`` or ``allr tools``. It's the onboarding + discovery
...
DOCS_URL = "https://allr.work/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `allr model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `allr portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `allr auth add nous --type
    oauth`. It reuses the exact wiring behind `allr setup --portal` (which in
...
def portal_command(args) -> int:
    """Top-level dispatch for `allr portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `allr portal` is the
        # human-readable alias for `allr auth add nous --type oauth` /
        # `allr setup --portal`.
    ...
    print("Run `allr portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `allr portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `allr portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `allr auth add "
            "nous --type oauth`, identical to `allr setup --portal`). "
