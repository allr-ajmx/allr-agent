"""allr webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.allr/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: allr gateway run
"""
...
def webhook_command(args):
    """Entry point for 'allr webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: allr webhook {subscribe|list|remove|test}")
        print("Run 'allr webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (allr gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: allr webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from allr webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (allr gateway run)")
