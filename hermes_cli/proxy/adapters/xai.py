...
# xAI's public API is OpenAI-compatible for the endpoints Allr commonly
# uses. The Responses endpoint is included because Allr's native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Allr-managed OAuth credentials."""

    auth_hint = "allr auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`allr auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`allr auth reset xai-oauth` or re-authenticate with "
                    "`allr auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `allr auth add xai-oauth --type oauth`."
            )
