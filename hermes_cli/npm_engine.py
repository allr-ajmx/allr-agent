...
Scope of the repair is deliberately narrow. Allr only upgrades an npm that
lives inside its **own** managed Node tree (``$ALLR_HOME/node``), installing
...
and their other projects; Allr never modifies those. When the failing npm is
one of those foreign installs, Allr instead provisions its own managed Node
...
def managed_npm_prefix(npm: str | os.PathLike[str] | None) -> Path | None:
    """Return the Allr-managed Node root *npm* lives in, else ``None``.

    Symlinks are resolved first: an install links ``~/.local/bin/npm`` at
    ``$ALLR_HOME/node/bin/npm``, which itself links into
...
def upgrade_managed_npm(
    ...
    *,
    ...
    ``prefix=~/.local`` into ``$ALLR_HOME/node/etc/npmrc`` so that global
    ...
    if not quiet:
        print(
            f"→ Upgrading Allr-managed npm to satisfy {npm_range}…",
            flush=True,
        )
    try:
...
def _print_manual_fix(npm: str, npm_range: str, actual: str | None) -> None:
    ...
    print(
        ...
        f"  Resolved npm: {npm}\n"
        "  Allr could not provision its own Node.js runtime and never\n"
...
def maybe_repair_npm_engine(
    ...
    *output* is the combined stdout/stderr of the npm command that just failed.
    ...
    if prefix is not None:
        # Allr owns this npm — upgrade it in place. Only an npm-range
