"""
allr fallback — manage the fallback provider chain.
...
Subcommands:
  allr fallback [list]   Show the current fallback chain (default when no subcommand)
  allr fallback add      Pick provider + model via the same picker as `allr model`,
                           then append the selection to the chain
  allr fallback remove   Pick an entry to delete from the chain
  allr fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.allr/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `allr model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  allr fallback add")
        print()
        return
    ...
    print("  Docs: https://allr.work/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `allr model`, then append the selection to the chain."""
    ...
    print("  `allr model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `allr fallback list` to view, or `allr fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``allr fallback [subcommand]``."""
