"""`allr checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.allr/checkpoints/``.  Actions:

    allr checkpoints               # same as `status`
    allr checkpoints status        # total size, project count, breakdown
    allr checkpoints list          # per-project checkpoint counts + workdir
    allr checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    allr checkpoints clear [-f]    # nuke the entire base (asks first)
    allr checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    allr checkpoints
    allr checkpoints prune --retention-days 3 --max-size-mb 200
    allr checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: allr checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``allr checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `allr checkpoints` → status
