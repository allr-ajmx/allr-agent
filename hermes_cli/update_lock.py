"""Cross-process mutual exclusion for in-flight Allr updates.

Three different surfaces can start an update of the same install tree:

* ``allr update`` from a terminal,
...
* the desktop's Update button, which hands off to the Tauri
  ``allr-setup --update`` and, on its failure screen, to install-mode
...
once — so a dashboard-spawned ``allr update`` and an installer-driven
...
unchanged and remain byte-compatible with the Rust and Electron readers:
    ...
    <ALLR_HOME>/.hermes-update-in-progress   body: "<pid>\\n<started_at_unix>"
...
then spawns ``allr update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Allr is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``allr-setup`` binary under ``~/.allr`` is only refreshed by a full
  ...
  ancestry check those users get exit 2 ("Allr is still running") on every
  GUI update forever, with no Allr process actually running.
"""
...
# Set by an orchestrating updater (the Tauri `allr-setup --update` flow) to
# its own pid before spawning `allr update` as a child stage. The parent
...
HANDOFF_PID_ENV = "ALLR_UPDATE_HANDOFF_PID"
...
# to show "Allr is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Allr home (never the context-local profile override):
    the Rust updater resolves ``$ALLR_HOME`` or the platform default, and the
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``allr update`` as a (grand)child, so a
    ...
    parent chain. This heals the fleet of staged ``allr-setup`` binaries
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Allr update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `allr update` as a stage): we run under ITS claim
