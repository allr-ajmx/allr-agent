"""
Security advisory checker for Allr.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``allr doctor --ack <id>``; the ack is persisted to
...
1. ``allr doctor`` (and ``allr doctor --ack <id>``)
...
run in environments where the rest of Allr failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.allr/.env (OpenRouter, Anthropic, OpenAI, "
            ...
            "After cleanup: allr doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — Allr requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'allr doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.allr/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``allr doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `allr doctor` on the gateway host for details.")
