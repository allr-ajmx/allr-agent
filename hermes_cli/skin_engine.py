"""Allr skin/theme engine — the theme SDK for every surface.

A data-driven skin system that lets users (and Allr itself) customize the
visual appearance across the CLI, the TUI, and the desktop GUI from a single
file. Skins are defined as YAML files in ~/.allr/skins/ or as built-in presets.
...
``resolve_skin`` / ``skin.changed``). A skin dropped in ~/.allr/skins/ therefore
...
.. code-block:: yaml
    ...
    branding:
      agent_name: "Allr"          # Banner title, status display
      welcome: "Welcome message"          # Shown at CLI startup
      goodbye: "Goodbye!"              # Shown on exit
      response_label: " Allr "       # Response box header label
...
.. code-block:: python
    ...
    print(skin.get_branding("agent_name"))  # "Allr"

    set_active_skin("ares")               # Switch to built-in ares skin
    set_active_skin("mytheme")            # Switch to user skin from ~/.allr/skins/
...
Drop a YAML file in ``~/.allr/skins/<name>.yaml`` following the schema above.
