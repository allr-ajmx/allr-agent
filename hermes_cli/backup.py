...
`allr backup` creates a zip archive of the entire ~/.allr/ directory
(excluding the allr-agent repo and transient files).

`allr import` restores from a backup zip, overlaying onto the current
ALLR_HOME root.
"""
...
# ``allr-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/allr-agent/``
...
# ALLR_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "allr-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under ALLR_HOME) —
...
# File names that ``allr import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE ALLR_HOME
...
class BackupInProgressError(RuntimeError):
    """Raised when another process already owns the Allr backup slot."""
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise BackupInProgressError("another Allr backup is already running")
                    time.sleep(0.05)
        else:
            import fcntl
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside ALLR_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    ...
    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``allr-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/allr-agent/`` — must be preserved.
        if part == "allr-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside ALLR_HOME.
...
# to the user as a hung `allr update` (#70553 follow-up). Sessions databases
...
def run_backup(args) -> None:
    """Create a zip backup of the Allr home directory."""
    hermes_root = get_default_hermes_root()

    if not hermes_root.is_dir():
        print(f"Error: Allr home directory not found at {hermes_root}")
        sys.exit(1)
...
def _run_backup_locked(args, hermes_root: Path) -> None:
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        # ``allr-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "allr-agent" and not is_root)
        ]
    ...
    # outside ALLR_HOME, so the walk above never sees it. Ask the active
    ...
    if not errors:
        print(f"\nRestore with: allr import {out_path.name}")
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.allr/config.yaml` instead of `config.yaml`.
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Allr configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under ALLR_HOME.
        ...
        if profiles_dir.is_dir():
            try:
                ...
                if any(profiles_dir.iterdir()):
                    ...
                    print("  Run: allr profile list  (after installing hermes)")

        # Guidance
        print()
        if not (hermes_root / "allr-agent").is_dir():
            print("Note: The allr-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: allr update")
        ...
        print("Done. Your Allr configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and allr backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ALLR_HOME).
...
# — `allr update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $ALLR_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside ALLR_HOME and the entry is silently
...
def _create_quick_snapshot_locked(
    ...
    if failed_dbs:
        ...
        print(
            ...
            f"{root} and run: allr snapshot list"
        )
        logger.error(
...
# Relative path of the cron job database inside ALLR_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``allr update``.
    ...
    Args:
        ...
        hermes_home: Override for the Allr home directory (tests).
...
def run_quick_backup(args) -> None:
    """CLI entry point for allr backup --quick."""
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `allr claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ALLR_HOME under ``backups/`` before a
    ``allr claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``allr import <archive>``.  Writes to
    ``<ALLR_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `allr import` and the
