...
_effective_terminal_backend = late("_effective_terminal_backend")
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``allr tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `allr tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``allr tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.allr/.env`` via ``save_env_value`` —
    the same store ``allr tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``allr tools post-setup <key>`` and the frontend tails the log via
    ...
    write per-profile state must see the same ALLR_HOME the rest of the
...
async def get_terminal_backends(profile: Optional[str] = None):
    ...
    Returns ``{active, configured, restart_required, backends: [{name, label,
    description, active, pending, status, detail}]}`` where ``status`` is
    ``ready`` / ``needs_setup`` /
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``allr computer-use permissions grant`` as a background action.
