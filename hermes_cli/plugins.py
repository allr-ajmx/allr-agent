...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with allr-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.allr/plugins/<name>/``
3. **Project plugins** – ``./.allr/plugins/<name>/`` (opt-in via
   ``ALLR_ENABLE_PROJECT_PLUGINS``)
...
# ---------------------------------------------------------------------------
#
# Set ``ALLR_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.allr/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("ALLR_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When ALLR_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Allr loggers.
    """
    ...
    logger.debug(
        "ALLR_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Allr's shipped guidance lives in the evidence-based
    ...
    #                              dispatcher or `allr kanban dispatch`),
...
class PluginManifest:
    ...
    #              in ~/.allr/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``allr plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Allr profile name (e.g. ``"default"``).

        Derived from ``ALLR_HOME`` via
        ...
        running under ``~/.allr/profiles/<name>``, or ``"custom"`` when
        ``ALLR_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Allr core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``allr honcho ...``).
    ...
    def register_secret_source(self, source) -> None:
        ...
        ``~/.allr/.env`` loads, before Allr reads credentials — when
        ...
        spawned Allr process (gateway children, cron sessions,
    ...
    def register_slack_action_handler(
        ...
        Allr' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``allr model → Configure auxiliary models`` picker
    ...
    def register_skill(
        ...
        ``~/.allr/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if env_var_enabled("ALLR_SAFE_MODE"):
            logger.info("ALLR_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            # invocation — including plain `allr chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Allr ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `allr plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _collect_directory_manifests(self) -> List[PluginManifest]:
        ...
        # 3. Project plugins (./.allr/plugins/), only when explicitly opted
        # in. This must match the full discovery gate exactly.
        if _env_enabled("ALLR_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".allr" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set ALLR_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``allr plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`allr plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `allr plugins list`.
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``allr tools`` TUI so plugin-provided toolsets appear
