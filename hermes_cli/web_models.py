"""Pydantic request/response models for the Allr dashboard web server.
...
class ModelAssignment(BaseModel):
    ...
    # ``allr model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `allr import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # because skills_hub.SKILLS_DIR is import-time-bound and the ALLR_HOME
...
class ProfileExport(BaseModel):
    ...
    # Where to write the archive. Empty → a staging path under ALLR_HOME.
    output: str = ""
