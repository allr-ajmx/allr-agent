...
The provider list shown by ``allr model`` (CLI/TUI) and the desktop Settings
...
the same universe ``allr model`` renders (``CANONICAL_PROVIDERS``), joining:
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``allr model`` shows.
"""
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `allr model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``allr model`` universe.
    ...
    # Allr overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = ALLR_OVERLAYS.get(slug)

        # auth_type: registry is authoritative; fall back to profile, then the
        # Allr overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
