...
Usage:
    allr pairing list              # Show all pending + approved users
    allr pairing approve <platform> <request-id|code>  # Approve a pairing request
    allr pairing revoke <platform> <user_id> # Revoke user access
    allr pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle allr pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: allr pairing {list|approve|revoke|clear-pending}")
        print("Run 'allr pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: allr pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.allr/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'allr pairing list' to see pending requests.\n")
