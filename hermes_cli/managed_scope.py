...
user's ``~/.allr/config.yaml`` and ``~/.allr/.env`` on a per-leaf-key basis.

This is DISTINCT from ``hermes_cli.config.is_managed()`` / ``ALLR_MANAGED``,
...
def _under_pytest() -> bool:
    ...
    that exercise managed scope set ``ALLR_MANAGED_DIR`` explicitly, which is
...
def get_managed_dir() -> Optional[Path]:
    ...
    Resolution (highest priority first):
      1. ``$ALLR_MANAGED_DIR`` — deployment/bootstrap path override (IT-only;
    ...
    """
    override = os.environ.get("ALLR_MANAGED_DIR", "").strip()
    if override:
        p = Path(override)
