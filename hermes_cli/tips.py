...
TIPS = [
    ...
    "Ctrl+Z suspends Allr to the background — run fg in your shell to resume.",
    ...
    "allr chat -t web,terminal enables only specific toolsets for a focused session.",
    "allr chat -s github-pr-workflow preloads a skill at launch.",
    "allr chat -q \"query\" runs a single non-interactive query and exits.",
    "allr chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "allr chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hermes --yolo bypasses all dangerous command approval prompts for the entire session.",
    "allr chat --source telegram tags the session for filtering in allr sessions list.",
    "hermes -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "allr doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "allr dump outputs a compact setup summary — great for bug reports.",
    "allr config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "allr config edit opens config.yaml in your default editor.",
    "allr config check scans for missing or stale configuration options.",
    "allr sessions browse opens an interactive session picker with search.",
    "allr sessions stats shows session counts by platform and database size.",
    "allr sessions prune --older-than 30 cleans up old sessions.",
    "allr skills search react --source skills-sh searches the skills.sh public directory.",
    "allr skills check scans installed hub skills for upstream updates.",
    "allr skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "allr skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "allr mcp add github --command npx adds MCP servers from the command line.",
    "allr mcp serve runs Allr itself as an MCP server for other agents.",
    "allr auth add lets you add multiple API keys for credential pool rotation.",
    "allr completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "allr logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "allr backup creates a zip backup of your entire Allr home directory.",
    "allr profile create coder creates an isolated profile that becomes its own command.",
    "allr profile create work --clone copies your current config and keys to a new profile.",
    "allr update syncs new bundled skills to ALL profiles automatically.",
    "allr gateway install sets up Allr as a system service (systemd/launchd).",
    "allr memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "allr webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: allr tools disables unused tools, allr skills config trims skills down.",
    ...
    "ALLR_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Allr tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'allr profile create coder' creates the 'coder' command.",
    "allr profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Allr prints a resume command with session ID and stats.",
    "allr sessions export backup.jsonl exports all sessions for backup or analysis.",
    ...
    "allr skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .allr/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: allr cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Allr runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "allr gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Allr via a local macOS server.",
    ...
    "Context files (.allr.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Allr loads project context from .allr.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "allr mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "allr mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "allr mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — allr tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.allr/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "allr chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "allr plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "allr acp runs Allr as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ALLR_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "allr auth add nous or allr auth add openai-codex sets up OAuth-based providers.",
    ...
    "allr status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.allr/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Allr your own.",
    ...
    "Before a gateway session reset, Allr auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.allr/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Allr state via ALLR_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "ALLR_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ALLR_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.allr/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'allr profile create ls' would be rejected.",
    "allr profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "allr claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ALLR_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via allr tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.allr/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.allr/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Allr auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Allr config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Allr is working.',
    ...
    '/reload reloads ~/.allr/.env into the running session — pick up new API keys without restarting.',
    ...
    "ALLR_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.allr/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.allr/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'allr curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "allr curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'allr curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'allr auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'ALLR_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "ALLR_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'allr dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'allr dashboard embeds the full Allr TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.allr/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.allr/dashboard-plugins/ — no npm build required.',
    ...
    'ALLR_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'ALLR_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'ALLR_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'ALLR_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.allr/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.allr/skills/ so drop-in skills appear without restarting the session.',
    ...
    'allr chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'allr chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'allr chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "allr chat --source tool tags programmatic chats so they don't clutter allr sessions list.",
    'allr dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'allr sessions rename <ID> "new title" renames any past session; allr sessions delete <ID> removes one.',
    'allr import restores a full Allr backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'allr fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'allr pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'allr setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'allr status --deep runs the full health sweep across every component; plain allr status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'ALLR_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'ALLR_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.allr/plugins/ — trust-gated by design.',
    "ALLR_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'ALLR_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'ALLR_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'ALLR_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'ALLR_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'ALLR_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'ALLR_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'ALLR_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'ALLR_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'ALLR_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'ALLR_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Allr block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from allr sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.allr/dashboard-plugins/.',
]
