"""Default configuration data for Allr.
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["hermes-cli"],
    # SQLite journal mode used by every Allr database opener. WAL is the
    ...
    "agent": {
        ...
        # the Allr-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Allr
        ...
        # identity slot (SOUL.md). Empty by default. The ALLR_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Allr adds a coding operating brief
        ...
        # ``ALLR_LOCAL_STREAM_STALE_TIMEOUT`` overrides for escape-hatch use.
    ...
    "terminal": {
        ...
        #             ALLR_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force ALLR_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Allr auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Allr sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Allr
        ...
        # bundled Allr image, which drops to the `hermes` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Allr sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Allr should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``allr chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.allr/checkpoints/`` size (MB).  When
        ...
        # ``allr checkpoints prune`` command (add ``--keep-orphans`` to
    ...
    # single read_file page exceeds these limits, Allr truncates the
    ...
    "compression": {
        ...
        "codex_responses_compact_threshold": 200000,  # Server-side compaction trigger
                                      ...
                                      # the server compacts before Allr does.
    ...
    "auxiliary": {
        ...
        # Invoked by ``allr kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``allr kanban decompose`` and the kanban
        ...
        # ``allr profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `allr model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `hermes` (and `allr chat`) launches by default:
        ...
        # REPL and `--tui` (or ALLR_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `hermes --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `hermes -c` muscle memory.  Default off so existing
        # users aren't surprised.  ALLR_TUI_RESUME=<id> always wins.
        ...
        # can set `path`/`paths` to ALLR_HOME-relative YAML files/directories
        ...
        # CLI, TUI, and desktop app. Manage with `allr pets`. Disabled until
    ...
    "dashboard": {
        ...
        # ``ALLR_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``ALLR_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``ALLR_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``ALLR_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``ALLR_DASHBOARD_PUBLIC_URL``).
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Allr appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.allr/cache/piper-voices/
    ...
    # "Hey Allr" hands-free wake word. Always-on, on-device hotword
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.allr/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.allr/cache/delegation/
    ...
    # response. If not, Allr feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Allr auto-pauses the goal and
    ...
    # always goes to ~/.allr/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ALLR_SKILL_DIR} and ${ALLR_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `allr curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `allr curator run --consolidate` overrides
        ...
        # normally restored on every `allr update`, so pruning them only
        ...
        # skipped), snapshot ~/.allr/skills/ into
        # ~/.allr/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `allr curator rollback`.
        "backup": {
            "enabled": True,
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "*Allr*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (ALLR_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.allr/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ALLR_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # .cursorrules, project-local .allr config) always require human
        ...
        # longer trigger the startup banner. Add via `allr doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``hermes_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Allr to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $ALLR_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via ALLR_CRON_MAX_PARALLEL env var.
        ...
        # ALLR_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    "kanban": {
        ...
        # decomposition is manual via `allr kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     allr-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Allr tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.allr/logs/.
    ...
    # update model picker lists without shipping a allr-agent release.
    ...
    "gateway": {
        ...
        # internal ALLR_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.allr/sessions/sessions.json entirely.
        ...
        # the ALLR_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart work.allr.gateway` that defenses 1-2 don't
        ...
        # `allr update` hang because it can never drain the gateway.
        "restart_loop_guard": {
            "max_restarts": 3,
            "window_seconds": 60,
            "max_gap_seconds": 300,
        ...
        # ``ALLR_GATEWAY_MAX_STARTS`` / ``ALLR_GATEWAY_START_WINDOW_S``
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.allr/.env,
        ...
        # behavior: files must live under the Allr cache, under
        ...
        # user. Bridged to ALLR_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Allr
        # cache (~/.allr/cache/{documents,images,audio,video,screenshots})
        ...
        # to ALLR_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # ALLR_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to ALLR_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.allr/state.db.
    ...
    "sessions": {
        ...
        # the default of ``allr sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.allr/sessions/session_{sid}.json`` on every turn
        ...
        # user runs `allr sessions optimize-storage`, because the rebuild is
        # disk-heavy and long on large DBs (see that command's disk preflight).
        #
        #   "advise" (default): `allr update` prints a one-line notice with
        ...
        # ~/.allr/lib/libfts5_cjk.so), 1-2 char CJK terms (일본, 项目, ...)
        ...
        # serve the cjk index. Bridged to ALLR_CJK_FTS (internal carrier).
        "cjk_fts": True,
        ...
        # ALLR_SEARCH_SLOW_MS (internal carrier).
        "search_slow_ms": 1000,
        ...
        # (non-streaming) export such as `allr sessions export`. Checked
    ...
    # ``allr doctor`` behaviour.
    "doctor": {
        # Per-probe timeout (seconds) for the opt-in `allr doctor --live`
        # real-call backend probes (Firecrawl/FAL/browser/MCP/TTS/STT).
        "live_probe_timeout": 10,
    },

    # ``allr update`` behaviour.
    "updates": {
        ...
        #     DBs) into <ALLR_HOME>/state-snapshots/ before the update.
        ...
        #     jobs) safety net.
        #   full — the quick snapshot PLUS a full ``allr backup``-style zip
        #     of ALLR_HOME into <ALLR_HOME>/backups/, restorable with
        #     ``allr import``. Can add minutes on large homes. This is the
        ...
        "backup_keep": 5,
        # What `allr update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<ALLR_HOME>/lsp/bin/`` on first use.
    ...
    # `allr tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.allr/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.allr/.env (or your shell) and never in config.yaml.
            ...
            # material under ~/.allr/cache/. If a later startup cannot reach
            # Bitwarden due to NETWORK/TIMEOUT, Allr may use this encrypted
            ...
            # ~/.allr/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.allr/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Allr disables it for our users unless they
        # explicitly opt in here. When false (default), Allr sets
        ...
        # cua-driver ≥ 0.6.x supports --no-overlay; Allr also calls
    ...
    #
    # Configure with `allr egress setup`.  Disabled by default — the rest of
    # Allr works exactly as before with `enabled: false`.
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned iron-proxy binary into ~/.allr/bin/ on
    ...
    # Allr Desktop (Electron app) launch options. These only affect
    ...
    "desktop": {
        ...
        # Bridged to the ALLR_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Allr mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'allr model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Allr auth store)",
    ...
    "PORCUPINE_ACCESS_KEY": {
        "description": "Picovoice access key for the Porcupine 'Hey Allr' wake word engine (optional; openWakeWord is the free default)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Allr, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ALLR_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'allr-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Allr API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Allr API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Allr API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ALLR_TOOL_PROGRESS_MODE is deprecated — tool progress is configured via
    # display.tool_progress in config.yaml (off|new|all|verbose|log). The
    # gateway still falls back to ALLR_TOOL_PROGRESS_MODE for backward
    ...
    # ALLR_TOOL_PROGRESS is fully unsupported since the v12 config support
    # floor retired its only consumer (the v3→4 migration).
    "ALLR_PREFILL_MESSAGES_FILE": {
