...
def _models_config_is_allowlist(value: Any) -> bool:
    ...
    written by ``_save_custom_provider`` / the ``allr model`` wizard — not a
    ...
    while ``allr model`` still live-probed the full ``/v1/models`` list.
...
_ALLR_MODEL_WARNING = (
    ...
    "for use with Allr. They lack the tool-calling capabilities "
...
_NOUS_ALLR_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``allr config set model.aliases.xxx``)
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'allr model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'allr doctor' — config issues detected:"
...
def list_authenticated_providers(
    ...
    *,
    ...
    # https://allr.work/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring an Allr release. Falls back to the in-repo
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (ALLR_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Allr may not route yet.
        ...
        # /model picker sees the SAME list `allr model` would build, with
    ...
    # --- 2. Check Allr-only providers (nous, openai-codex, copilot, opencode-go) ---
    from hermes_cli.providers import ALLR_OVERLAYS
    from hermes_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Allr provider ID.
    # ALLR_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Allr IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ALLR_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Allr slug — e.g. "github-copilot" → "copilot"
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `allr model` picker deliberately shows ONLY the curated agentic
    ...
    # in PROVIDER_TO_MODELS_DEV or ALLR_OVERLAYS (keeps /model in sync
    # with `allr model`).
    try:
    ...
    if user_providers and isinstance(user_providers, dict):
        ...
        for ep_name, ep_cfg in user_providers.items():
            ...
            # Allr writes ``models:`` as a dict keyed by model id, but older
            ...
            if group_key not in ep_groups:
                ...
                # Allr's own writer uses (mirrors section-4 grouping).
            ...
            # ``allr model``, not an allowlist — see
        ...
        for grp in ep_groups.values():
            ...
            #   Ollama/llama.cpp match ``allr model``. Pin with
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # active model. Allr's own writer (main.py::_save_custom_provider)
        ...
        for grp in groups.values():
            ...
            #   ``allr model``. Pin a dict catalog with
