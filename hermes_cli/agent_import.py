"""allr import-agent — import Claude Code / Codex CLI setups into Allr.

Usage:
    allr import-agent                       # auto-detect ~/.claude or ~/.codex
    allr import-agent claude-code           # import from ~/.claude
    allr import-agent codex                 # import from ~/.codex
    allr import-agent claude-code --dry-run # preview only, no changes
    allr import-agent codex --source /path/to/.codex

Follows the OpenClaw migration pattern (``allr claw migrate`` /
...
--------
claude-code (~/.claude):
    CLAUDE.md                       → memory entries in ALLR_HOME/memories/MEMORY.md
    ...
    skills/<name>/SKILL.md          → ALLR_HOME/skills/claude-code-imports/<name>/

codex (~/.codex):
    AGENTS.md                       → memory entries in ALLR_HOME/memories/MEMORY.md
    config.toml [mcp_servers.*]     → config.yaml mcp_servers
    memories/*.md                   → memory entries in ALLR_HOME/memories/MEMORY.md
    skills/<name>/SKILL.md          → ALLR_HOME/skills/codex-imports/<name>/
...
deliberately via ``allr setup`` or config.yaml.
"""
...
# Same entry delimiter as the Allr memory store and the openclaw migration
...
def load_yaml_file(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ConfigReadError(
            ...
            f"`allr config edit` (or move it aside), then re-run the import."
        )
    return data
...
# ---------------------------------------------------------------------------
# Claude Code permission rules → Allr command patterns
# ---------------------------------------------------------------------------
...
def claude_rule_to_command_pattern(rule: str) -> Optional[str]:
    """Convert a Claude Code ``Bash(...)`` permission rule into an Allr glob.
...
class AgentImporter:
    ...
    def _run_claude_code(self) -> None:
        ...
        if commands_dir.is_dir() and any(commands_dir.glob("*.md")):
            self.record(
                ...
                "Claude slash commands have no direct Allr equivalent — "
    ...
    def import_skills(self, source_root: Path) -> None:
        """skills/<name>/SKILL.md dirs → ALLR_HOME/skills/<category>/<name>."""
...
def import_agent_command(args) -> None:
    """Handle ``allr import-agent`` (invoked from hermes_cli.main)."""
    ...
    if agent is None:
        ...
        if not detected:
            ...
            print_info("Specify one explicitly: allr import-agent claude-code --source /path")
            return
        if len(detected) > 1 and explicit_source is None:
            print()
            print_info("Multiple agent setups detected: " + ", ".join(detected))
            print_info("Pick one: allr import-agent claude-code   or   allr import-agent codex")
            return
        agent = detected[0]
    ...
    print(color("│          Allr — Import From Another Agent          │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))

    if not source_dir.is_dir():
        print()
        print_error(f"Agent directory not found: {source_dir}")
        print_info("Specify a custom path: allr import-agent "
    ...
    print_info("Secrets:     never imported — run 'allr setup' for credentials")
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info(f"To execute, re-run with: allr import-agent {agent} --yes")
            return
    ...
    print_info("API keys and credentials were NOT imported — run 'allr setup' "
               "to configure providers, or add them to ~/.allr/.env.")
...
def print_import_report(report: Dict[str, Any], dry_run: bool) -> None:
    ...
    if stripped:
        ...
        for name in stripped:
            print(f"      {name}")
        print_info("Re-add credentials deliberately via 'allr setup' or ~/.allr/.env.")
        print()
