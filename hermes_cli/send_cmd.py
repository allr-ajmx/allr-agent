"""CLI subcommand: ``allr send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"allr send: {file_path} is not a text file. --file reads the "
                ...
                f'  allr send --to telegram "MEDIA:{file_path}"\n'
                f'  allr send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  allr send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"allr send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"allr send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"allr send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"allr send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `allr send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"allr send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `allr gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.allr/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.allr/.env`` AND bridge top-level
    ...
    startup that ``allr send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.allr/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.allr/config.yaml`` into
    ...
    intentionally reimplement the minimum needed here so ``allr send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.allr/.env and ~/.allr/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "allr send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  allr send --to telegram \"hello\"\n"
            "  allr send --to discord:#ops --file report.md\n"
            "  allr send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "allr send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `allr send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Allr "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.allr/.env + ~/.allr/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  allr send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `allr send --list telegram`.",
    )
