...
3. Running inside a container with no persistent volume mount over the
   ALLR_HOME data dir (state is ephemeral — lost on container restart).
...
logger = logging.getLogger("allr.security_audit")
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Allr as an unprivileged user (or in a "
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("ALLR_DESKTOP_CHILD_PID"):
