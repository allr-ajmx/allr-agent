"""ACP session manager — maps ACP sessions to Allr AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.allr/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Allr itself is running in WSL.

    Windows ACP clients can launch ``allr acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Allr from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["allr-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Allr agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Allr AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Allr runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.allr/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``ALLR_HOME`` dynamically rather than relying on
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Allr daemon's process cwd.
