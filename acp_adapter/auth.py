"""ACP auth helpers — detect and advertise Allr authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "allr-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Allr runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Allr can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Allr.
    ...
    not have Allr provider credentials configured yet, so Allr always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Allr using the currently configured "
