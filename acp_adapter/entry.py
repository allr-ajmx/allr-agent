"""CLI entry point for the allr-agent ACP adapter.

Loads environment variables from ``~/.allr/.env``, configures logging
...
try:
    ...
    # shadowing Allr's own modules — ``allr acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from ALLR_HOME (default ``~/.allr``)."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="allr-acp",
        description="Run Allr as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Allr version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Allr provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.allr/node/ "
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting allr-agent ACP adapter")
    ...
    if os.environ.get("ALLR_ACP_SKIP_CONFIGURED_MCP", "").strip() != "1":
        try:
