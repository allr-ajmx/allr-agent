# Política de Seguridad de Allr

Este documento describe el modelo de confianza de Allr, identifica el
...
vulnerabilidades de seguridad. **Allr no opera un programa de
...
- Detalles del entorno (`allr version`, SHA del commit, SO, versión de Python).
...
Allr es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta Allr,
...
Allr admite dos posturas de aislamiento a nivel de SO. Abordan
...
Allr admite esto de dos maneras:

- **La propia imagen Docker de Allr y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de Allr
...
Allr filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en Allr
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de Allr
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   Allr no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de Allr o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  Allr ha documentado una postura sobre cómo su salida debe ser
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de Allr
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en Allr. Los errores en la ruta de
  instalación de habilidades o plugins de Allr que impidan al
...
- Allr incluye guardias de cadena de suministro para lanzamientos de servidores
