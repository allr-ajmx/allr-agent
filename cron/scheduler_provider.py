...
class InProcessCronScheduler(CronScheduler):
    ...
    def start(
        ...
        *,
        ...
        # only the process-global ALLR_HOME (the default profile) is ticked.
        # Heartbeats and recovery are also scoped per profile so `allr cron
        ...
        # Heartbeat once before the first sleep so `allr cron status` sees a
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # `allr cron status`/`list` (separate processes) can show
            ...
            for entry in profile_homes:
                ...
                try:
                    with use_cron_store(home):
                        ...
                        # so `allr cron status` can show WHY ticks fail
                        # (#68483).
                        if ok:
                            clear_ticker_error()
                        elif _tick_error:
