...
``allr gateway restart`` (or ``launchctl kickstart work.allr.gateway``
or ``systemctl restart allr-gateway``).  When the cron fires, the
...
``allr cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``allr gateway``, ``launchctl ... allr-gateway``,
``systemctl ... allr-gateway``, ``pkill`` against the gateway) so it
...
commands and shell scripts they reference when ``_ALLR_GATEWAY=1``. It also
rejects ``launchctl submit`` in gateway sessions because launchd treats that
primitive as a persistent KeepAlive job, not a one-shot task. ``allr gateway
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `allr gateway restart|stop` — the canonical foot-gun.
...
# lines (e.g. `launchctl submit \` / `  -l work.allr.gateway-... \` / `  -- ...`,
...
# for `systemctl restart allr-gateway` in syslog, a SQL LIKE literal over a
...
# to execution (`grep 'systemctl restart allr-gateway' f | sh`); never mask
# such a line.
...
def _mask_data_sink_arguments(text: str) -> str:
    ...
    ``systemctl restart allr-gateway`` from the same characters appearing
    ...
    (false positives blocking legitimate cron prompts), e.g.::
        ...
        sqlite3 db "SELECT msg FROM log WHERE msg LIKE '%systemctl restart allr-gateway%'"
...
def _iter_referenced_shell_scripts(
    ...
    *,
    ...
    for segment in _iter_command_segments(command):
        ...
        # (e.g. `Path.home() / ".allr"`), not an executable reference.
...
def _resolve_script_path(script_path: str) -> Optional[Path]:
    ...
    under ``<ALLR_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.allr/scripts/restart.sh``) but is passed as the bare name
    ...
    try:
        ...
        # neither ALLR_HOME nor HOME is resolvable (launchd/systemd
...
def check_gateway_lifecycle(
    ...
    if python_script:
        ...
        # `allr gateway restart` embedded in a .py script is still
    ...
    if unsafe:
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `allr gateway restart` from a shell outside "
