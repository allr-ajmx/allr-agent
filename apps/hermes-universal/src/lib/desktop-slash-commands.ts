...
/** Why a known Allr command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Allr profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Allr version', surface: exec() },
 ...
 * Allr's built-in slash commands — i.e. skill commands (`/gif-search`,
 ...
 * activity — the ones that ship with Allr and were never asked for. It is for
