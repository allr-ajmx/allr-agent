// Ported VERBATIM from desktop `src/lib/speech-text.ts` (MJXHRM-369), tests
// included. Markup that reads fine on screen is noise out loud: asterisks,
// hashes, backticks and a bare URL get voiced character by character, and a
// markdown table is unlistenable however it is punctuated.
//
// Verbatim on purpose. These rules are the accumulated result of hearing the
// output, not something anyone would derive twice from first principles, so a
// tidier universal variant would only be a second set of things to notice by
// ear. Applied on the WHOLE-CLIP path only (`lib/voice-playback.ts`) — never per
// streaming delta, where a fence or a link would be split across chunks and the
// regexes would match half of one.
