...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Allr doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // this provider identity — the SAME one `allr model` uses — instead of
...
export interface ModelOptionProvider {
   ...
   *  the `allr model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Allr, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `allr tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Allr); elsewhere, driver health from `cua-driver doctor`. `null`
...
/**
 * `GET /api/fs/search`. An ADDITIVE route (the MJXHRM-511 backend freeze), so
