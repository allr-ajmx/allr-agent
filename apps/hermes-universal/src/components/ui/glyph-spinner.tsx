...
export function GlyphSpinner({ ariaLabel = 'Loading', className, spinner = 'braille' }: GlyphSpinnerProps) {
  ...
  // mounted, and N of these each ticking a setInterval burns CPU for pixels
  // nobody can see. Outside a tab stack `usePaneVisible` is true, so nothing
  // else changes.
  ...
  useEffect(() => {
    ...
    // Frames are written straight to the DOM node rather than through state:
    // a spinner beside every live tool row was committing React work at the
    // spinner's frame rate for a single character of text.
    let frame = 0
