 ...
 *
 * A tab group keeps each ever-active tile MOUNTED and hides the inactive ones
 * (see `tree/renderer/tree-group.tsx`), deliberately preserving their layout box
 * so scroll positions survive a tab round-trip. The cost is that an inactive
 * tab's rect is IDENTICAL to the visible tab's, so neither selector order nor a
 * rect hit-test can tell them apart. A lookup that resolves "the chat surface /
 * composer / viewport" from the document therefore has to skip hidden tiles, or
 * it silently answers with the wrong tab.
 *
 * Ported from desktop, which has run this model for long enough to know the
 * failure mode: it is never a crash, always a subtle "the wrong tab scrolled".
 */

import { createContext, useContext } from 'react'

/** Marks a mounted-but-hidden tile layer (an inactive tab in a stack). */
export const PANE_HIDDEN_ATTR = 'data-pane-hidden'

const HIDDEN_PANE = `[${PANE_HIDDEN_ATTR}]`

/** Spread onto a kept tile layer so the lookups below can skip it. */
export const hiddenPaneProps = (hidden: boolean): Record<string, string> => (hidden ? { [PANE_HIDDEN_ATTR]: '' } : {})

/** React face of the same policy: the tile layer provides its visibility so a
 ...
 *  (secondary windows, plain routes, the mobile shell) are always visible. */
...
/** The layout-tree GROUP (zone) a tile is rendered in — the identity of "this
 *  set of tabs". Tiles stacked as tabs share one group; each split zone is its
 *  own. State that should be per-zone rather than per-window or per-tab keys off
 *  this. Follows a tile dragged between zones, because the provider is the zone
 *  that renders it. */
