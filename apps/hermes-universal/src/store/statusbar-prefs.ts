...
const STATUSBAR_HIDDEN_STORAGE_KEY = 'hermes.universal.statusbarHidden'
const STATUSBAR_VISIBLE_STORAGE_KEY = 'hermes.universal.statusbarVisible'

// Whole-bar visibility, VS Code's `workbench.statusBar.visible`. Hiding it
// unmounts the bar (its status poll goes with it), so the way back is the
// `view.toggleStatusbar` keybind or the command menu — never the bar itself.
...
// route shortcuts (cron/agents) and the terminal toggle are navigation, not
// status, so they start out of the way. The per-turn session readouts (running/
// session timers, context meter) are diagnostics most users don't watch, so they
// start hidden too and the bar stays quiet mid-turn.
