...
// Ported from apps/desktop/src/app/overlays/overlay-split-layout.tsx. The wide
// rail and the narrow dropdown swap at exactly the width where the layout drops
// to a single column, so the rail never stacks. Tauri: no -webkit-app-region
// classes (only elements with data-tauri-drag-region drag).
...
// viewports and a single dropdown bar on narrow ones. Drop it in as the first
  ...
  return (
    <>
      ...
      {/* Narrow: ride the OverlayView titlebar strip so the dropdown shares the
          close button's row. pointer-events-none (children opt back in) so the
          floating X underneath stays clickable; pr clears it. */}
      <div
        className={cn(
      ...
      >
        ...
        {footer && <div className="pointer-events-auto flex shrink-0 items-center gap-1">{footer}</div>}
      </div>
    </>
  )
}
