...
async function fallbackRootFor(_cwd: string): Promise<string | null> {
  // Seam: desktop asks Electron main to sanitize an unreadable local cwd
  // (`window.hermesDesktop.sanitizeWorkspaceCwd`). Universal has no local FS —
  // the gateway owns the workspace and hardens paths itself — so there is
  // nothing to fall back to, exactly as on a remote desktop connection.
  return null
}
