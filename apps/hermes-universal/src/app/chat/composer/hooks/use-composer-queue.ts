...
export function useComposerQueue({
  ...
  // were queued. The map is tiny (only halted sessions) so a plain subscribe is
  // fine; the auto-drain effect below reads it as a gate.
  ...
  const runDrain = useCallback(
    async (pickEntry: (entries: QueuedPromptEntry[]) => QueuedPromptEntry | undefined): Promise<boolean> => {
      ...
      try {
        ...
        // the remaining entries follow. Manual drains (Enter on an empty
  ...
  const sendQueuedNow = useCallback(
    (id: string) => {
      ...
      if (busy) {
        ...
        // Unpark first: this interrupt exists to REACH the queue, so the settle
        // drain must flow — unlike a Stop/Esc halt, which parks.
        unparkQueuedPrompts(activeQueueSessionKey)
