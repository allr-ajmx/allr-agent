...
// (Settings / Command Center / Profiles) open in ONE native screen activity (a fresh
// WebView carrying `?win=activity`, launched from `src-tauri/src/window.rs`).
// `app.tsx` mounts this instead of the full chat shell (MobileController).
//
// The chrome (top bar + section-nav drawer + Status/Files drawer, surface derived
// live from the route) lives in the shared `MobileSurfaceShell` so the iOS /
// generic-mobile in-app overlay (mobile-controller) renders these surfaces with the
// exact same layout (MJX-203). Here the Home / open-session actions map to
// `returnHome` (finish the native scene → back to the sessions activity).
//
