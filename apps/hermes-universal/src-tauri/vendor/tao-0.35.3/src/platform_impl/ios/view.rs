...
pub fn create_delegate_class() {
  ...
  extern "C" fn configuration_for_connecting_scene_session(
    ...
    unsafe {
      ...
      // PATCHED (Allr): upstream returned `Retained::as_ptr(&config)`, a borrowed
      // pointer -- `config` is dropped on the next line and the object released, so
      // UIKit received a dangling UISceneConfiguration and crashed retaining it
      // (EXC_BAD_ACCESS in objc_retain, under -[UIApplication
      // _connectUISceneFromFBSScene:transitionContext:]). Only reachable when
      // UIApplicationSupportsMultipleScenes is set, and only reliably fatal in
      // optimized builds. Fixed upstream in tao 0.36.0 by this same change; drop
      // this vendored copy once tauri-runtime-wry requires that version.
