...
//!   Windows: %LOCALAPPDATA%\allr
//!   macOS:   ~/.allr
//!   Linux:   ~/.allr  (override via $ALLR_HOME)
//!
//! NOTE (macOS): Python's get_hermes_home(), scripts/install.sh, and the
//! Electron desktop's resolveHermesHome() ALL use ~/.allr on macOS — there
...
/// Returns the canonical Allr home directory, respecting $ALLR_HOME if set.
pub fn hermes_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ALLR_HOME") {
    ...
    {
        // %LOCALAPPDATA%\allr — matches scripts/install.ps1's $HermesHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("hermes");
        }
    }

    // macOS + Linux + fallback: ~/.allr (matches Python get_hermes_home(),
    // install.sh, and the Electron desktop's resolveHermesHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".allr");
    }
...
/// ALLR_HOME so it survives repo checkout deletion (unlike anything under
/// allr-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\allr\allr-setup.exe`; on other
...
///
/// Lives directly under ALLR_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves ALLR_HOME identically and pins it into
...
pub fn copy_self_to_hermes_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ALLR_HOME");
    Ok(())
}
...
///   const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ALLR_ROOT, '.hermes-bootstrap-complete')
/// We don't always know ACTIVE_ALLR_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".hermes-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ALLR_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[allr-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ALLR_BOOTSTRAP_LOG")
