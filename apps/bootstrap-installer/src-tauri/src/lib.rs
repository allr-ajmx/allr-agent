...
pub fn run() {
    // Tracing → bootstrap-installer.log under ALLR_HOME/logs/ so install
    ...
    // Allr is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Allr installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Allr is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Allr" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Allr" shortcuts that install.ps1 creates, and a
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::hermes_home().join("allr-agent");
        ...
        .expect("error while running Allr Setup");
}
