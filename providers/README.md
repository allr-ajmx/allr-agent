...
Registry and ABC for every inference provider Allr knows about.
...
`$ALLR_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
under `$ALLR_HOME/plugins/model-providers/` for a private plugin).
