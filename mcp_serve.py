"""
Allr MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Allr-specific extra)

Usage:
    allr mcp serve
    allr mcp serve --verbose
...
logger = logging.getLogger("allr.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ALLR_HOME."""
    try:
        from hermes_constants import get_hermes_home
        return get_hermes_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ALLR_HOME", Path.home() / ".allr")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("ALLR_HOME", Path.home() / ".allr")
...
class EventBridge:
    ...
    This is the Allr equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _establish_baseline(self) -> None:
        ...
        try:
            ...
            db_file = Path(os.environ.get("ALLR_HOME", Path.home() / ".allr")) / "state.db"
        try:
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Allr MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hermes",
        instructions=(
            "Allr messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Allr MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
