 ...
 *
 * The root ``package.json`` is installed by ``allr update`` on every user,
 ...
 *   ``allr tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``allr update`` for
 ...
 * guidance in the ``allr-agent-dev`` skill before changing the
 * expectations.
 */
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      '`allr tools` → Browser Automation → Camofox.'
  )
})
...
test('agent-browser stays eager (default backend)', () => {
  ...
  assert.ok(
    ...
    'agent-browser is the default browser-tool backend used by every ' +
      ...
      'after `allr setup` / `allr update` without an explicit ' +
      'post_setup step.'
  )
})
