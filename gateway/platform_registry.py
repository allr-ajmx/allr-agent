...
class PlatformEntry:
    ...
    # status displays (``allr setup``, ``allr status``, the dashboard
    ...
    # Env vars this platform needs (for ``allr setup`` display).
    ...
    # built-ins).  Used by ``allr gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # invocation -- including plain `allr chat`, which never touches any
        ...
        # `allr setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``allr setup``/``gateway status``, channel
