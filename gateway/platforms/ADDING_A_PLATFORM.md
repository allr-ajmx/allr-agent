...
There are two ways to add a platform to the Allr gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.allr/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Allr code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `allr gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
Checklist for integrating a platform directly into the Allr core.
...
And add it to the `allr-gateway` composite:
