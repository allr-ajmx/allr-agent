...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.allr to the active profile home."""
    ...
    if raw == "~/.allr":
        return hermes_home
    if raw.startswith("~/.allr/"):
        return hermes_home / raw.removeprefix("~/.allr/")
...
def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under ALLR_HOME/scripts."""
    ...
    if raw_text == "~/.allr" or raw_text.startswith("~/.allr/"):
