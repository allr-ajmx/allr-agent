...
transport, discovers their tools, and registers them into the allr-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.allr/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.allr/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Allr's logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Allr explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Allr and
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hermes_home = os.path.expanduser(
                os.getenv(
                    "ALLR_HOME", os.path.join(os.path.expanduser("~"), ".allr")
                )
            )
            candidates = [
                ...
                # image (which the Allr Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Allr MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Allr gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Allr document cache; returns a marker with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active Allr profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Allr's existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (ALLR_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit ALLR_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Allr's logging (agent.log via hermes_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "it is not installed. Run `allr setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Allr process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Allr's approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Allr is fully
                ...
                if not self._ready.is_set():
                    if failure_class == "permanent":
                        ...
                        # re-authenticated with ``allr mcp login``. Parking
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `allr mcp login {server_name}` (or delete the tokens "
        f"file under ~/.allr/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Allr processes (e.g.
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local ALLR_HOME override into ``coro``.
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local ALLR_HOME override onto the MCP loop.
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Allr config file.
    ...
    ``os.environ`` (which includes ``~/.allr/.env`` loaded at startup).
    """
    try:
        from hermes_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("ALLR_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Allr' MEDIA: tag convention and out to messaging
            ...
            # Allr' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Allr's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Allr uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Allr registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``allr tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Allr registry.
