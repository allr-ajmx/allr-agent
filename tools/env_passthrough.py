...
def _is_hermes_provider_credential(name: str) -> bool:
    ...
    Non-Allr API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return True
    # Dynamically-generated Allr-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_hermes_internal_secret(name):
        return True
    return name in _ALLR_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Non-Allr third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Allr provider "
                "credential %r (blocked by _ALLR_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Allr-managed provider credentials must not be passed
                ...
                if _is_hermes_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Allr "
                        "provider credential %r from config.yaml (blocked "
                        "by _ALLR_PROVIDER_ENV_BLOCKLIST). Operator "
