...
def _make_hermes_provider_class() -> Optional[type]:
    ...
    class HermesMCPOAuthProvider(OAuthClientProvider):
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``allr mcp reauth``.
...
_ALLR_PROVIDER_CLS: Optional[type] = _make_hermes_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        if (
            ...
            raise OAuthNonInteractiveError(
                ...
                "cached tokens found. Run `allr mcp login "
    ...
    def remove(
        ...
        *,
        ...
        Called by ``allr mcp remove <name>`` and (indirectly) by
        ``allr mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
