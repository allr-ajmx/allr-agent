...
Backend is selected during ``allr tools`` setup (web.backend in config.yaml).
When available, Allr can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Allr config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Allr's config/.env layer (``allr config set``, ``allr tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.allr/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``allr tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `allr tools` repaint.
        try:
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `allr plugins enable {disabled_key}` "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`allr plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
