"""Base class for all Allr execution environment backends.
...
# ALLR_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("ALLR_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {ALLR_HOME}/sandboxes/.
    """
...
def _cwd_marker(session_id: str) -> str:
    return f"__ALLR_CWD_{session_id}__"
...
# the FIRST session's ALLR_SESSION_ID into the snapshot makes every LATER
...
_SNAPSHOT_EXCLUDED_ENV_REGEX = (
    "^declare -x (ALLR_SESSION_|ALLR_UI_SESSION_ID|ALLR_CRON_AUTO_DELIVER_|ALLR_CRON_SESSION)"
)
...
def _export_dump_excluding_session_vars(
    ...
    Matrix room/display name via ``ALLR_SESSION_CHAT_NAME``) land in the
    ...
    return (
        "{ ( "
        "unset ${!ALLR_SESSION_*} ${!ALLR_CRON_AUTO_DELIVER_*} "
        f"ALLR_UI_SESSION_ID{extra_unset} 2>/dev/null; "
        "export -p; "
        ") || true; } "
        f"> {tmp_path}"
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Allr backends.
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        for name in passthrough_names:
            marker = f"_ALLR_RUNTIME_PASSTHROUGH_{name}"
        ...
        # Restrict Allr metadata files without changing the user's command
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"ALLR_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        # --- Debug tracing (opt-in via ALLR_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __ALLR_CWD_{session}__ marker from stdout output.
