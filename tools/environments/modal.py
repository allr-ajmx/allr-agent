...
wrapper, while preserving Allr's persistent snapshot behavior across sessions.
"""
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        async def _create_sandbox(image_spec: Any):
            app = await _modal.App.lookup.aio("allr-agent", create_if_missing=True)
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        """Download remote .allr/ as a tar archive.

        Modal sandboxes always run as root, so /root/.allr is hardcoded
        (consistent with iter_sync_files call on line 269).
        """
        async def _download():
            proc = await self._sandbox.exec.aio(
                "bash", "-c", "tar cf - -C / root/.allr"
            )
