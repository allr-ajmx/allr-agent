...
Uses the Vercel Python SDK to run commands in cloud sandboxes through Allr'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``ALLR_HOME`` and restores
...
def _ensure_vercel_sdk() -> None:
    ...
    # (vercel-internal-telemetry posts to telemetry.vercel.com). Allr
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            ...
            container_base = f"{self._remote_home.rstrip('/')}/.allr"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_hermes = (
            "/.allr"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.allr"
        )
