...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``allr tools`` repaint, tool-registration scans,
    ...
    2. ``~/.allr/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``allr auth add xai-oauth``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.allr/.env`` first, then ``os.environ``.
...
def hermes_xai_user_agent() -> str:
    """Return a stable Allr-specific User-Agent for xAI HTTP calls."""
    try:
        from hermes_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Allr/{__version__}"
...
def hermes_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Allr,
...
def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Allr config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Allr home, then mark it seen."""
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Allr-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``hermes_cli.config.get_env_value`` so keys
    stored in ``~/.allr/.env`` (the standard Allr location) are honored —
    ...
    try:
        ...
        override_base_url = str(
            get_env_value("ALLR_XAI_BASE_URL")
