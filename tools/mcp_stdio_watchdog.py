...
<url>``) is spawned as a direct child of the Allr process. Allr's own
...
Allr process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Allr startup's opt-in
