...
409), and is driven by:
  ...
  * the ``allr sync status|pull|push|now`` CLI.
...
is toggled via ``allr sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.allr/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``allr sync enable`` — it is
...
# namespace from allr-agent's local ``content_hash`` (skills_guard.py:846),
...
# sync.base_url (or the ALLR_SYNC_BASE_URL bridge env) overrides it to point
...
def resolve_sync_base_url() -> Optional[str]:
    ...
    Order: ALLR_SYNC_BASE_URL env bridge -> config.yaml ``sync.base_url`` ->
    ...
    """
    env = os.getenv("ALLR_SYNC_BASE_URL")
...
#
#   ALLR_SYNC_BASE_URL        -> sync.base_url        (the sync plane URL)
#   ALLR_SYNC_ENABLED         -> sync.enabled         (master on/off; default off)
#   ALLR_SYNC_DEFAULT_OPT_IN  -> sync.default_opt_in  (personal sync policy; default false
...
def sync_feature_enabled() -> bool:
    ...
    """
    return _sync_config_bool("ALLR_SYNC_ENABLED", "enabled", default=False)


def sync_org_auto_propose() -> bool:
    """Whether an agent/user edit to an org skill is proposed automatically.

    ``ALLR_SYNC_ORG_AUTO_PROPOSE`` -> ``sync.org_auto_propose`` -> False.

    False (default): edits to an org-shared skill stay LOCAL until the user
    runs ``allr sync propose <skill>``. The skill keeps working with the
    ...
    return _sync_config_bool(
        "ALLR_SYNC_ORG_AUTO_PROPOSE", "org_auto_propose", default=False
    )
...
def sync_default_opt_in() -> bool:
    ...
    ``ALLR_SYNC_DEFAULT_OPT_IN`` -> ``sync.default_opt_in`` -> False.

    False (default): opt-IN — a skill syncs only after an explicit
    ``allr sync enable`` (or a plane manifest that opted it in). True: opt-OUT
    ...
    Allr Cloud deployment wants. Per the design notes, this default is
    ...
    """
    return _sync_config_bool("ALLR_SYNC_DEFAULT_OPT_IN", "default_opt_in", default=False)


# ---------------------------------------------------------------------------
# Local skill eligibility + the personal sync opt-in "sync" flag
#
# Only agent-created + user-authored skills under ~/.allr/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.allr/skills/, NOT bundled, NOT
...
def list_synced_skill_names() -> List[str]:
    ...
    - **opt-out (Allr Cloud "on by default"):** every *eligible* skill syncs
      UNLESS its usage record explicitly carries ``sync: false``. This is what a
      deployment sets (via ``ALLR_SYNC_DEFAULT_OPT_IN``) so a user's skills
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.allr/skills/ containing
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``allr sync device --name`` to set an explicit
    label."""
    ...
    # Allr Cloud (and any templated deployment) can seed the label
    # declaratively via ALLR_SYNC_DEVICE_NAME, so a hosted instance shows a
    # recognizable name with no CLI call. Env seeds the FIRST-USE value only; it
    # is then persisted, so a later `allr sync device --name` (or editing the
    # file) still wins on that device. An explicit file (above) always wins over
    # the env.
    import os

    env_name = (os.environ.get("ALLR_SYNC_DEVICE_NAME") or "").strip()
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.allr/skills/.sync_device_id, overwriting
...
# ~/.allr/skills/.sync_state as JSON.
#
...
# path under ~/.allr/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.allr/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Allr speaks "
            f"{WIRE_VERSION} — update Allr to sync with it"
        )
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (allr sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.allr/skills/. Only paths the user has opted into (``sync: true``) are
...
def maybe_push_skills(*, message: str = "hermes skill sync") -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not sync_feature_enabled():
            return None  # feature off for this instance (ALLR_SYNC_ENABLED)
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``allr sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# Org skills live under a DISTINCT local namespace, ~/.allr/skills/_org/
...
#
# `allr sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.allr/skills/_org/<org_id>/``.
