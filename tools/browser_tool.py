...
# means a compromised transitive dependency could read every Allr secret
...
def _build_browser_env() -> dict:
    ...
    Strips Allr-managed secrets (provider keys, gateway tokens, GitHub auth,
...
def _get_cdp_override_raw() -> str:
    ...
    *whether* a CDP override is configured (tool ``check_fn`` gates,
    ...
    ``allr doctor``) MUST use this instead of :func:`_get_cdp_override`.
...
# plugins drop in under ``~/.allr/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    browser plugins (``~/.allr/plugins/browser/<vendor>/``) participate
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Allr silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Allr CLI where sys.stdout and sys.stderr both report
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Allr-managed
    ...
    # node_modules binary, which disappears on the next ``allr update`` and
    ...
    # Build an extended search PATH including Allr-managed Node, macOS
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Allr
