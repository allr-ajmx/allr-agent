...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Allr exit
...
# display_hermes_home imported lazily at call site (stale-module safety during allr update)
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("ALLR_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("ALLR_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("ALLR_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (ALLR_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (ALLR_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive Allr password prompt or the sudo -S password-pipe path.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``allr-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Allr processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Allr processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Allr processes a generous grace
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.allr/.env or environment variables."
        )
...
def _ensure_terminal_env_bridged() -> None:
    ...
    be stale from ``allr setup``). Environment values for omitted terminal
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Allr processes that hit SIGKILL / OOM / a closed terminal
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "notify_on_complete=true) so Allr tracks the process, then run readiness "
...
def terminal_tool(
    ...
    try:
        ...
        # restart|stop targeting allr-gateway) must never run inside the
        ...
        # never restart. This mirrors the `allr gateway restart` guard in
            ...
            if contains_launchctl_submit_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Use Allr cron for one-shot delayed work, or install an "
            ...
            if contains_gateway_lifecycle_command_or_referenced_script(
                ...
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "to child processes). Run `allr gateway restart` from a "
        ...
        if background:
            ...
            try:
                ...
                # allr-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in allr-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        _gw_platform = _gse("ALLR_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("ALLR_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("ALLR_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("ALLR_SESSION_USER_ID", "")
                            _gw_user_name = _gse("ALLR_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("ALLR_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("ALLR_INTERACTIVE"):
                    output += (
