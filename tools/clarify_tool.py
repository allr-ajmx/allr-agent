...
CLARIFY_SCHEMA = {
    ...
    "description": (
        ...
        "WHEN TO ASK: only when the answer is genuinely the user's to give — "
        "a decision you cannot resolve from the request, the files, or a "
        "sensible default — and when guessing wrong would cost real work or "
        "be hard to undo. Otherwise take the obvious option, name it as an "
        "assumption, and keep going. Concretely, reach for this when:\n"
        "- The task is ambiguous in a way that changes what you would build\n"
        "- A decision has meaningful trade-offs the user should weigh in on\n"
        "- You want post-task feedback ('How did that work out?')\n"
        "- You want to offer to save a skill or update memory\n\n"
        "HOW TO ASK: everything at once. One to three specific questions in a "
        "single batch, each with concrete `choices`, your recommendation "
        "first. If the user doesn't answer, proceed on your stated "
        "assumptions rather than asking again.\n\n"
        "Do NOT use this tool to ask what the conversation, the files, or the "
        "project's docs already answer; to ask permission to begin work you "
        "have already been asked to do; to re-ask something already settled; "
        "or for simple yes/no confirmation of dangerous commands (the "
        "terminal tool handles that)."
    ),
    "parameters": {
        "type": "object",
        "properties": {
