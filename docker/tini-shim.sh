...
# Allr WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: ALLR_TINI_SHIM_TARGET overrides the `/init` path (and
# ALLR_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${ALLR_TINI_SHIM_TARGET:-/init}"
WRAPPER="${ALLR_TINI_SHIM_WRAPPER:-/opt/hermes/docker/main-wrapper.sh}"
