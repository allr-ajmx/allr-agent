...
#
# Wired into the image as /etc/cont-init.d/01-allr-setup by the
...
# $ALLR_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ALLR_UID/ALLR_GID — or the PUID/PGID aliases — which the
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e ALLR_UID=\$(id -u) -e ALLR_GID=\$(id -g) ...
...
# --- Bootstrap ALLR_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid hermes mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ALLR_HOME paths whose parents only
# root can create (e.g. `ALLR_HOME=/home/hermes/.allr` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ALLR_UID/ALLR_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ALLR_UID/
# ALLR_GID still win when both are set.  See #15290, salvages #25872.
ALLR_UID="${ALLR_UID:-${PUID:-}}"
ALLR_GID="${ALLR_GID:-${PGID:-}}"

if [ -n "${ALLR_UID:-}" ] && validate_uid_gid "$ALLR_UID" && [ "$ALLR_UID" != "$(id -u hermes)" ]; then
    echo "[stage2] Changing hermes UID to $ALLR_UID"
    usermod -u "$ALLR_UID" hermes
fi
if [ -n "${ALLR_GID:-}" ] && validate_uid_gid "$ALLR_GID" && [ "$ALLR_GID" != "$(id -g hermes)" ]; then
    echo "[stage2] Changing hermes GID to $ALLR_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ALLR_GID" hermes 2>/dev/null || true
fi
...
# When ALLR_UID is remapped or the top-level $ALLR_HOME isn't owned by
# the runtime hermes UID, restore ownership to hermes — but ONLY for the
# directories hermes actually writes to. The full $ALLR_HOME may be a
...
needs_chown=false
if [ "$(stat -c %u "$ALLR_HOME" 2>/dev/null)" != "$actual_hermes_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ALLR_HOME (targeted) to hermes ($actual_hermes_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ALLR_HOME: chown the directory itself (not its contents)
    # so hermes can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ALLR_HOME"; then
        :
    else
        chown hermes:hermes "$ALLR_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ALLR_HOME failed (rootless container?) — continuing"
    fi
    # Allr-owned subdirs: recursive chown is safe here because these are
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ALLR_HOME/$sub" ] && tree_has_non_hermes_owner "$ALLR_HOME/$sub"; then
            chown_hermes_tree "$ALLR_HOME/$sub"
        fi
    done
fi
...
# $ALLR_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ALLR_DISABLE_LAZY_INSTALLS=1. Keeping /opt/hermes root-owned and
...
# $ALLR_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ALLR_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# Always reset ownership of $ALLR_HOME/profiles to hermes on every
...
if [ -d "$ALLR_HOME/profiles" ] && tree_has_non_hermes_owner "$ALLR_HOME/profiles"; then
    chown_hermes_tree "$ALLR_HOME/profiles"
fi

# Always reset ownership of $ALLR_HOME/cron on every boot for the same
...
# gate as profiles/).
if [ -d "$ALLR_HOME/cron" ] && tree_has_non_hermes_owner "$ALLR_HOME/cron"; then
    chown_hermes_tree "$ALLR_HOME/cron"
fi
...
# top-level $ALLR_HOME is mis-owned, so a warm volume with hermes-owned
# ALLR_HOME but root-owned logs/gateways would otherwise leave
...
if [ -d "$ALLR_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$ALLR_HOME/logs/gateways"; then
        :
    else
        chown hermes:hermes "$ALLR_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# allr pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged allr gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $ALLR_HOME is
...
# and cron/.
if [ -d "$ALLR_HOME/platforms/pairing" ] && tree_has_non_hermes_owner "$ALLR_HOME/platforms/pairing"; then
    chown_hermes_tree "$ALLR_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$ALLR_HOME/pairing" ] && tree_has_non_hermes_owner "$ALLR_HOME/pairing"; then
    chown_hermes_tree "$ALLR_HOME/pairing"
fi
...
# *subdirectories*; loose state files living directly under $ALLR_HOME
...
# sweep so host-owned files in a bind-mounted $ALLR_HOME are never
...
for f in \
    ...
    active_profile; do
    if [ -e "$ALLR_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ALLR_HOME/$f"; then
            :
        else
            chown hermes:hermes "$ALLR_HOME/$f" 2>/dev/null || true
        fi
    fi
done
...
if [ -f "$ALLR_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ALLR_HOME/config.yaml"; then
        :
    else
        chown hermes:hermes "$ALLR_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ALLR_HOME/config.yaml" 2>/dev/null || true
    fi
fi
...
# shell isn't a second interpreter — defends against $ALLR_HOME values
...
as_hermes mkdir -p \
    ...
    "$ALLR_HOME/home" \
    "$ALLR_HOME/pairing" \
    "$ALLR_HOME/platforms/pairing" \
    "$ALLR_HOME/lazy-packages"
...
# $ALLR_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ALLR_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.allr:/opt/data) and sometimes shared with a
...
# 'allr update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ALLR_HOME if one is present (the host install's
...
if [ -f "$ALLR_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ALLR_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ALLR_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ALLR_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ALLR_HOME/$dest"; then
            :
        else
            as_hermes cp "$INSTALL_DIR/$src" "$ALLR_HOME/$dest"
        fi
    fi
}
...
if [ -f "$ALLR_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ALLR_HOME/.env"; then
        :
    else
        chown hermes:hermes "$ALLR_HOME/.env" 2>/dev/null || true
        chmod 600 "$ALLR_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $ALLR_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `allr update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ALLR_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ALLR_HOME/config.yaml" ]; then
...
if [ ! -f "$ALLR_HOME/auth.json" ] && [ -n "${ALLR_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ALLR_HOME/auth.json"; then
        :
    else
        printf '%s' "$ALLR_AUTH_JSON_BOOTSTRAP" > "$ALLR_HOME/auth.json"
        chown hermes:hermes "$ALLR_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ALLR_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via ALLR_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$ALLR_HOME/auth.json" ] && [ -n "${ALLR_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$ALLR_HOME/auth.json"; then
        :
    else
...
# can set ALLR_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ALLR_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ALLR_HOME/gateway_state.json" ] && \
        [ "${ALLR_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ALLR_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ALLR_HOME/gateway_state.json"
        chown hermes:hermes "$ALLR_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ALLR_HOME/gateway_state.json"
    fi
fi
...
# directory. agent-browser (the runtime CLI Allr spawns for the
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-allr, dashboard, etc.).
