#!/bin/sh
# shellcheck shell=sh
# /opt/hermes/bin/allr — `docker exec` privilege-drop shim.
#
# Background
# ----------
...
# $ALLR_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> allr login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Allr is not logged into Nous
...
# Fix
# ---
# This shim sits at /opt/hermes/bin/allr and is placed earliest on PATH.
...
# $ALLR_HOME is uid-aligned with the supervised processes. When invoked
...
# (/opt/hermes/.venv/bin/allr), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set ALLR_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
case "${ALLR_DOCKER_EXEC_AS_ROOT:-}" in
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "hermes-shim: re-run with --user hermes or set ALLR_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi
