...
Allr supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.allr/config.yaml`:
...
Or set the env var: `ALLR_KANBAN_DISPATCH_IN_GATEWAY=false`
