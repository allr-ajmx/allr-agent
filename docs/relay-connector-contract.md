...
This document is the formal interface between the **Allr gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `allr gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`allr gateway enroll` → connector `/relay/enroll`, or
