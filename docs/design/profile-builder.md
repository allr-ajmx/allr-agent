...
Supersedes: PR #31781 (prompt_toolkit `allr profile wizard`)
...
A profile = a full `~/.allr/profiles/<name>/` directory with its own:
...
| Model + Provider | ModelsPage | `_write_profile_model(profile_dir, …)` | yes — ALLR_HOME override, already wired into create endpoint |
| MCPs | McpPage | `mcp_config._save_mcp_server` + `/api/mcp/catalog` | yes — wrap with ALLR_HOME override |
...
### Seam #1 — hub-skill install cannot use the ALLR_HOME override

`tools/skills_hub.py` binds `SKILLS_DIR = ALLR_HOME / "skills"` at **module
...
A fresh subprocess re-imports `skills_hub` with the profile's ALLR_HOME bound
...
2. write model (synchronous, ALLR_HOME override)
3. write selected MCP servers (synchronous, ALLR_HOME override)
...
class ProfileCreate(BaseModel):
    ...
    mcp_servers: List[MCPServerCreate] = []      # synchronous, ALLR_HOME override
...
page afterward. Mirror `_write_profile_model`'s ALLR_HOME-override helper for
...
- Backend E2E with isolated ALLR_HOME: POST a full create body
