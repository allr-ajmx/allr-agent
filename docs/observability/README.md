...
Allr observer hooks are the read-only telemetry contract for plugins that
...
Allr also has a first-party NeMo Relay shared-metrics path. It uses these
...
```text
telemetry_schema_version = "allr.observer.v1"
```

Hook callbacks are fail-open. Allr catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Allr only builds
