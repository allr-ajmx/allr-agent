...
Allr includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Allr and does not require `allr plugins enable
observability/nemo_relay`. Allr remains importable without Relay on other
native targets. Those targets use an explicit reduced-capability no-op host:
Allr execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `allr-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

Allr requires NeMo Relay 0.6.0 or later within the 0.6 release line. That
...
Allr installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Allr process so configured interceptors can
...
Collection remains off unless Allr policy enables it:
...
Allr core owns one Relay host and one isolated Relay session scope per Allr
...
marks do not require Allr plugin registration. Shared-metrics marks must
...
```text
Allr turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Allr shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Allr sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Allr used for the logical call, such as
...
New calls use `allr.model_route.count`. The previous
`allr.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `allr.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Allr
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Allr processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `allr.task_run`, parented to
the owning Allr session. The start counter contains only bounded execution
...
provider attempts for the same Allr API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Allr tool-call
...
cancellations. Active task ownership follows the task ID if Allr rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`allr.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Allr
...
tool or plugin names. Allr does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `allr.tool_approval` marks and
...
Successful skill mutations emit `allr.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `allr.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Allr derives reuse and patch-generation
...
```text
$ALLR_HOME/telemetry/shared_metrics/metrics.sqlite3
$ALLR_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Allr version,
...
schema field name, its current scope is one `ALLR_HOME`, so it is more
...
Deleting `$ALLR_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `ALLR_HOME`. To reset it, stop Allr
processes and remove `$ALLR_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Allr CLI turn against the deterministic local model server:
