...
has to be thrown away or summarized. Allr has always done this in one batch:
...
Allr folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Allr's normal session flush is
...
With `micro_compact` unset or `false` Allr behaves exactly as it always has:
...
Defaults to `$ALLR_HOME/logs/agent.log`. It reports passes, outcome mix, net
