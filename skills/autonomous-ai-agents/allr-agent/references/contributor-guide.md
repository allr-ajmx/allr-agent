...
For occasional contributors and PR authors. Full developer docs: https://allr.work/docs/developer-guide/
...
Config: `~/.allr/config.yaml` (settings), `~/.allr/.env` (API keys) — both under `$ALLR_HOME` when it is set.
...
`_ALLR_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_hermes_home()` for paths,
never hardcode `~/.allr`. For custom/local-only tools, write a plugin in
`~/.allr/plugins/` instead of editing core — see the developer docs.
...
- Tests auto-redirect `ALLR_HOME` to temp dirs — never touch real `~/.allr/`.
