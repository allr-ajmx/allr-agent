...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Allr itself.
...
- Write access to the Allr home dir — `~/.allr` by default, or `$ALLR_HOME`
  / the active profile's dir. Skins live in `<hermes-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `allr config set`).
...
3. **Apply it yourself — never hand-edit `config.yaml`.** Run the safe writer via
   `terminal`:
   ```
   allr config set display.skin <name>
   ```
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `allr config set display.skin default` (or they can `/skin default`).
...
```
allr skin set <key> <hex>      # e.g. allr skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `allr skin set background "#08201f"` changes
only the background; `allr skin use <name>` / `allr skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.allr`** when a profile is active — resolve the real home
  from `$ALLR_HOME` first, falling back to `~/.allr`.
...
- **Never hand-edit `config.yaml` to activate.** Use `allr config set
...
- **You apply it, not the user.** `allr config set display.skin <name>` is
...
- Run `allr config get display.skin` and confirm it reports `<name>`.
