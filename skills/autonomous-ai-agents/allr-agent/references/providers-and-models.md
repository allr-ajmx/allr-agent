...
Set via `allr model` (picker) or `allr setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://allr.work/docs/integrations/providers
...
| nous | OAuth device code | `allr auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `allr auth add openai-codex` |
| qwen-oauth | OAuth | `allr auth add qwen-oauth` |
| minimax-oauth | OAuth | `allr auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`allr auth`).
Fallback chain when the primary fails: `allr fallback add|remove|list`.
...
#   allr config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
