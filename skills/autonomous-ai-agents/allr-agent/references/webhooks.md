...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Allr agent runs by POSTing events to a URL.
...
All management is via the `allr webhook` CLI command:

### Create a subscription
```bash
allr webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.allr/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.allr/scripts/` (path traversal is blocked).

```bash
allr webhook subscribe todoist-hermes \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-hermes-label.py" \
  --deliver telegram --deliver-chat-id "12345"
```

Full filter syntax: https://allr.work/docs/user-guide/messaging/webhooks#payload-filters
...
```bash
allr webhook remove <name>
```

### Test a subscription
```bash
allr webhook test <name>
allr webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
allr webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
allr webhook subscribe stripe-payments \
...
```bash
allr webhook subscribe ci-builds \
  --events "pipeline" \
...
- Subscriptions persist to `~/.allr/webhook_subscriptions.json`

## How It Works

1. `allr webhook subscribe` writes to `~/.allr/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status allr-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.allr/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `allr webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `allr webhook test <name>` to verify the route works.
