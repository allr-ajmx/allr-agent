...
Edit with `allr config edit` or `allr config set section.key value`.
Full reference: https://allr.work/docs/user-guide/configuration
...
`allr config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `allr tools` (interactive) or `allr tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_ALLR_CORE_TOOLS` is the default bundle most platforms inherit).
