...
**three** code paths in Allr that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Allr — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Allr
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Allr is
   refusing my value."
...
appears, **the cap is the model**, not Allr.
