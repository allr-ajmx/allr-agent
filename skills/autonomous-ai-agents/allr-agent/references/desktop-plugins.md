...
Write plugins for the Allr desktop app: statusbar items, layout panes,
...
system (`~/.allr/plugins/`) is otherwise documented separately.
...
- The Allr desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ALLR_HOME/desktop-plugins/` (usually
  `~/.allr/desktop-plugins/`).

## How to Run

1. Create `$ALLR_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.allr/...` by default, or `~/.allr/profiles/<profile>/...` under a
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  Fires only while the user is away from Allr (use `host.notify` for the
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.allr/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
