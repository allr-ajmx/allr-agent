...
Author widget apps for the Allr TUI (`hermes --tui`): glanceable ambient
...
1. Use `write_file` to create `~/.allr/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.allr/logs/tui_gateway_crash.log` if a widget never appears.
