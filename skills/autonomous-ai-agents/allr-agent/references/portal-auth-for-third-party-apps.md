...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Allr process**. Its LLM calls go through Allr's already-configured provider. | Already uses Portal if user's Allr is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Allr. That LLM call is already authenticated
via Allr's provider config. The plugin's own server auth (e.g.
...
What Allr has that *feels* like OAuth — `allr login --provider nous`
opening a browser, user signs in, token lands in `~/.allr/auth.json` — is a
**Allr-specific browser flow**. Under the hood it produces a credential
Allr uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Allr's existing Portal credential out of `~/.allr/auth.json`.
...
rides on the credential Allr already holds.

Where this could live in Allr:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Allr itself.
...
1. First decide: Allr plugin (runs inside Allr) or separate app? If plugin,
   it already uses Portal via Allr's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Allr agent itself") is fully supported. Allr treats agent
...
pitch in the Tool Gateway announcement is specifically about Allr's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Allr is configured to use Portal as its provider. It is
