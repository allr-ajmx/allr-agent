...
4. Store the token in `${ALLR_HOME:-~/.allr}/.env` (or via `allr setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Allr. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
- **`AIRTABLE_API_KEY` flows from `${ALLR_HOME:-~/.allr}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
