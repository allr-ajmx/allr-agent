#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Allr.
...
TOKEN_PATH = ALLR_HOME / "google_token.json"
CLIENT_SECRET_PATH = ALLR_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ALLR_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Allr profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Allr Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Allr Docker image's "
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Allr home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Allr")
