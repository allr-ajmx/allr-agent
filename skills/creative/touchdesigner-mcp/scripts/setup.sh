...
ALLR_HOME_DIR="${ALLR_HOME:-$HOME/.allr}"
ALLR_CFG="${ALLR_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Allr config has twozero_td MCP entry ──
if [[ ! -f "$ALLR_CFG" ]]; then
    echo -e " ${FAIL} Allr config not found at ${ALLR_CFG}"
    manual_steps+=("Create ${ALLR_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ALLR_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Allr config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Allr config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ALLR_CFG} manually"); }
    manual_steps+=("Restart Allr session to pick up config change")
fi
