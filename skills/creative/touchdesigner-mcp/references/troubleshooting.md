...
## 5. Allr Configuration

### Config location

`$ALLR_HOME/config.yaml` (defaults to `~/.allr/config.yaml` when `ALLR_HOME` is unset)
...
Restart the Allr session for changes to take effect. The MCP connection is
