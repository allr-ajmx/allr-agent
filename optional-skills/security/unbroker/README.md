...
The design is **Allr-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
allr skills install official/security/unbroker
```

Then start a new Allr session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Allr home directory (`0600`, optionally
