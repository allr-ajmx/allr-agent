...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ALLR_HOME", os.path.expanduser("~/.allr")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ALLR_HOME", Path.home() / ".allr")) / "skills" / "red-teaming" / "godmode"
...
ALLR_HOME = Path(os.getenv("ALLR_HOME", Path.home() / ".allr"))
CONFIG_PATH = ALLR_HOME / "config.yaml"
PREFILL_PATH = ALLR_HOME / "prefill.json"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Allr config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.allr/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Allr for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Allr for changes to take effect.")
