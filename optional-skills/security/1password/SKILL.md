...
- `tmux` available for stable authenticated sessions during Allr terminal calls (desktop app flow only)
...
### Service Account (recommended for Allr)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${ALLR_HOME:-~/.allr}/.env` (the skill will prompt for this on first load).
...
## Allr Execution Pattern (desktop app flow)

Allr terminal commands are non-interactive by default and can lose auth context between calls.
