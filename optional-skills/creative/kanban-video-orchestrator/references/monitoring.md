...
allr kanban watch --tenant <project-slug>

# Snapshot of the board
allr kanban list --tenant <project-slug>
allr kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
allr kanban stats --tenant <project-slug>

# Visual dashboard (browser)
allr dashboard

# Inspect a specific task (includes comments + events)
allr kanban show <task-id>

# Follow a single task's event stream
allr kanban tail <task-id>
```

Verify available subcommands with `allr kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `allr kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `allr kanban block <id>` then `allr kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `allr kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `allr kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `allr profile list` (profile exists?), `allr status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(allr kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
allr kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
allr kanban block <task-id>
allr kanban archive <task-id>

# Diagnose what happened
allr kanban show <task-id>      # task body, comments, recent events
allr kanban tail <task-id>      # follow the live event stream
allr kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `allr kanban create` for the director
...
while true; do
    clear
    allr kanban list --tenant <slug>
    echo "---"
    allr kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `allr kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `allr profile list` and start
