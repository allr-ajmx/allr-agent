...
Maps each role archetype to the Allr skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public allr-agent
...
## Allr skills relevant to video production

### Visual / rendering skills (`allr-agent/skills/creative/`)
...
### Design / pre-production skills (`allr-agent/skills/creative/`)
...
### Audio / media skills (`allr-agent/skills/creative/` + `skills/media/`)
...
These are **not** Allr skills but external CLIs / APIs that profiles invoke.
...
## Built-in Allr tools for media review

These are native Allr tools — not invoked via terminal but through their own
...
key is present in `${ALLR_HOME:-~/.allr}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Allr profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ALLR_HOME:-~/.allr}/.env` → environment variable.
...
When a new Allr-public video skill ships:
