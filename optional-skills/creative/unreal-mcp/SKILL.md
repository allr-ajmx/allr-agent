...
Companion skill for the `unreal-engine` entry in the Allr MCP catalog. The
...
Two halves, in this order: the editor side must be up before Allr connects.
...
### One-time, Allr side
    ...
    allr mcp install unreal-engine
...
`~/.allr/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Allr — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Allr connects
...
2. Start the Allr session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Allr
they appear as:
...
then happens at `allr mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Allr runs batched calls
...
- **Start order matters.** Editor + server up first, then the Allr
