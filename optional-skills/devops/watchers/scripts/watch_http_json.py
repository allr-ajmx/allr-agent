...
Usage (via cron with --no-agent):
    ...
    allr cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ALLR_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Allr-Watcher/1.0"})
