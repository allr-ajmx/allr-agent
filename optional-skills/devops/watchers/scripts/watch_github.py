...
Usage (via cron with --no-agent):
    ...
    allr cron create hermes-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ALLR_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hermes-issues --repo NousResearch/hermes-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Allr .env file
(``${ALLR_HOME:-~/.allr}/.env``) to avoid the 60 req/hr
