...
# from shadowing Allr's own top-level modules.  ``hermes_bootstrap`` lives at
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `ALLR_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("ALLR_TUI_SIDECAR_URL")
...
# tested; override via ``ALLR_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("ALLR_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``ALLR_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    # session profile's ALLR_HOME first), so a launch profile with no
...
def mcp_discovery_in_flight() -> bool:
    ...
    and ``allr dashboard`` spawn theirs via
...
def ensure_mcp_discovery_started() -> None:
    ...
    binding the session profile's ALLR_HOME override — the shared owner in
...
def main():
    ...
    # Live-apply skins Allr activates mid-conversation.
