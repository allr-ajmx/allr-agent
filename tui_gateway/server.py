...
# appends every unhandled exception to ~/.allr/logs/tui_gateway_crash.log
...
try:
    _slash_timeout = float(os.environ.get("ALLR_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("ALLR_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("ALLR_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 8
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Allr agent → needs provider credentials.
        ...
        # gateway's launch ALLR_HOME (#40677). The override goes through the
        # build_subprocess_env factory's `extra` (applied last, always wins)
        # instead of a hand-rolled env["ALLR_HOME"] assignment.
        ...
        env = build_subprocess_env(
            ...
            extra={"ALLR_HOME": str(profile_home)} if profile_home else None,
        )
...
try:
    _SESSION_TTL_S = float(os.environ.get("ALLR_TUI_SESSION_TTL_S") or 6 * 3600)
...
# ``profile`` on those calls; we open that profile's db and bind its ALLR_HOME
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s ALLR_HOME around a pet RPC handler.
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``allr dashboard`` was launched) instead of the configured
...
def _inside_compute_host_child() -> bool:
    return os.environ.get("ALLR_COMPUTE_HOST_CHILD") == "1"
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # ALLR_HOME so config/skills/model resolve to it, and hand the
...
def _set_session_context(
    ...
    *,
    ...
    try:
        ...
        # can read ALLR_SESSION_ID. Without this, set_session_vars leaves the
        ...
        # ALLR_SESSION_ID even though agent_init set it via
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["ALLR_GATEWAY_SESSION"] = "1"
    os.environ["ALLR_EXEC_ASK"] = "1"
...
def _broadcast_skin_if_changed() -> None:
    ...
    (``allr config set display.skin``) OR edited the active skin's colors in
...
def _ensure_skin_watcher() -> None:
    ...
    Allr activates, a pet ``/pet`` adopts, a cron the scheduler fires, or a
...
def _resolve_model() -> str:
    env = (
        os.environ.get("ALLR_MODEL", "")
        or os.environ.get("ALLR_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``ALLR_DESKTOP=1`` and ``ALLR_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``ALLR_DESKTOP_TERMINAL=1`` → "tui"
    ...
    """
    if is_truthy_value(os.environ.get("ALLR_DESKTOP")) and not is_truthy_value(
        os.environ.get("ALLR_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads ALLR_MODEL /
    ALLR_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    ...
    # No fallback to _resolve_model() here: that reads ALLR_MODEL /
    # ALLR_INFERENCE_MODEL, which `hermes --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("ALLR_TUI_PROVIDER", "").strip()
    ...
    try:
        ...
        current_provider = (
            ...
            or os.environ.get("ALLR_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("ALLR_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    ...
    Both entries are deliberately off ``_ALLR_CORE_TOOLS`` — every other
    ...
    paths run with ``ALLR_DESKTOP=1``. Keying GUI capability off that env var
    silently stripped every pane/browser tool from URL and cloud gateways while
    the same backend told the model it was "chatting inside the Allr desktop
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    ...
    explicit = [
        item.strip()
        for item in os.environ.get("ALLR_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Allr): with no explicit pin, collapse to the
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] ALLR_TUI_TOOLSETS=all enables every toolset; "
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown ALLR_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in ALLR_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid ALLR_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The client-surface toolsets are off _ALLR_CORE_TOOLS (every other
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid ALLR_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    #
    # We deliberately do NOT write process-global env vars (ALLR_MODEL /
    # ALLR_INFERENCE_MODEL / ALLR_TUI_PROVIDER / ALLR_INFERENCE_PROVIDER)
...
def _get_usage(agent) -> dict:
    ...
    # ALLR_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("ALLR_DEV_CREDITS")):
        try:
...
#     key-addressed (keyless rows render read-only in the GUIs, because bare
#     names collide across category dirs: image_gen/fal vs video_gen/fal).
...
def _agent_cbs(sid: str) -> dict:
    callbacks = {
        ...
        # directly underneath the Allr window, and answers
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("ALLR_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("ALLR_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _make_agent(
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("ALLR_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("ALLR_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("ALLR_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("ALLR_IGNORE_RULES")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            "edit_snapshots": {},
            "tool_started_at": {},
            # Profile-scoped ALLR_HOME for app-global remote mode; None =
...
def _build_persist_message_with_image_refs(user_text: str, image_paths: list[str]) -> str:
    ...
    format the desktop client (directive-text.tsx / ALLR_DIRECTIVE_RE)
...
def _session_home(session: dict) -> Path:
    """The ALLR_HOME the session's durable state lives in (profile-aware)."""
    profile_home = session.get("profile_home")
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Allr has to support both the legacy 8-row petdex atlas and the current
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("ALLR_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
#
# Layout:  $ALLR_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _collect_kanban_notifications(session: dict) -> list:
    ...
    ``platform="tui"`` and ``chat_id=ALLR_SESSION_KEY`` (see
    ...
    # DB when ALLR_KANBAN_DB pins the board path (same guard as the gateway
    # notifier).
...
def _wire_desktop_ui() -> None:
    ...
    Idempotent. The tool hands back the turn's ``ALLR_UI_SESSION_ID`` as
...
def _run_prompt_submit(
    ...
    *,
    ...
    def run():
        ...
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn ALLR_HOME override for a resumed remote profile
        secret_token = None
        ...
        try:
            ...
            # Run while any profile-specific ALLR_HOME override is still active
...
def _session_images_dir(session: dict) -> Path:
    ...
    run BEFORE ``prompt.submit`` installs the session's profile ALLR_HOME
...
def _desktop_attachment_dir(session: dict) -> Path:
    ...
    installs the session's profile ALLR_HOME override, while the docker/ssh
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("ALLR_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
...
def _is_repo_junk(root: str) -> bool:
    ...
    :func:`_non_workspace_dirs`) or anything under ALLR_HOME (~/.allr by
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    ALLR_HOME may be an intentional prose/data workspace. The pre-Projects
    desktop surfaced every such cwd, so exclude only the broad defaults that
    would create catch-all projects: ALLR_HOME itself and the dirs in
...
def _discover_repos_payload(
    ...
    repos even with zero allr sessions. Session-derived roots cover repos
    ...
    try:
        ...
        def _read(c) -> None:
            for entry in pdb.list_discovered_repos(c):
                ...
                # i.e. "just now" — so a git checkout with zero Allr sessions
...
def _rank_slash_completions(
    ...
    *,
    ...
    of skills someone invokes daily lead the ones that shipped with Allr
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `hermes` is interactive — use `/allr chat -q …` or run `hermes` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`allr setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`allr gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`allr sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`allr config edit` needs $EDITOR in a real terminal"
    return None
...
# Fuzzy basename search moved verbatim to ``tui_gateway/file_search.py`` so the
# dashboard (`GET /api/fs/search`) can reuse the exact same ranker without
# importing this module.  These re-imports are LOAD-BEARING: `complete.path`
# lives in `methods_complete.py` and `method_ctx.HandlerRegistry.install()`
# rebinds its `__globals__` to `vars(server)`, so it resolves these names as
# server.py globals at call time.  `_fuzzy_cache`/`_fuzzy_cache_lock` are the
# SAME objects as file_search's (a second copy would silently diverge).
from tui_gateway.file_search import (  # noqa: E402,F401
...
def _format_live_history_output(session: dict) -> str:
    ...
    for idx, message in enumerate(messages, start=1):
        ...
        label = "You" if role == "user" else "Allr" if role == "assistant" else role.title()
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("ALLR_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("ALLR_VOICE_TTS", "").strip() == "1"
        ...
        try:
            ...
            if text:
                ...
                if _is_stop:
                    ...
                    os.environ["ALLR_VOICE"] = "0"
                    os.environ["ALLR_VOICE_TTS"] = "0"
                    try:
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["ALLR_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["ALLR_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
