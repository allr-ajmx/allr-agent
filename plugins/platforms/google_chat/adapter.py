...
# ``model_tools`` import time, so ``allr status``, ``allr chat``, etc.
...
#     fresh "Allr is thinking…" card during the small window between
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${ALLR_HOME}/google_chat_thread_counts.json so
        ...
        # they don't sit in the chat forever as "Allr is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("ALLR_HOME", str(_Path.home() / ".allr"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${ALLR_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Allr is thinking…' marker message.
        ...
        "Allr is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Allr is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Allr is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Allr is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``allr setup``.
    ...
    prompt for env vars, persist them to ``~/.allr/.env`` so the next
    ...
    print()
    print_success("Google Chat configuration saved to ~/.allr/.env")
    print_info("Restart the gateway: allr gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``allr cron`` running as a
    separate process from ``allr gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Allr plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `allr setup` to install Google Chat support.",
        ...
        # the "Allr is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Allr is thinking…' marker message "
