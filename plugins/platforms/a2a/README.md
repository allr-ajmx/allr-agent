# A2A — Agent-to-Agent protocol for Allr
...
peer (another Allr, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
allr gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Allr serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.allr/a2a_audit.jsonl`.
- Conversations persist to `~/.allr/a2a_conversations/` — they survive context
