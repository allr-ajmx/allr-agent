...
Small Node helper that bridges Allr to Photon's Spectrum SDK
(`spectrum-ts`).  Allr is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Allr-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Allr can receive inbound messages
...
The Allr plugin's `allr photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Allr is Python, so replies go through this sidecar
