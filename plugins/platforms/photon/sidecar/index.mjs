...
//
// Inbound  (gRPC -> Allr): the SDK's `app.messages` async iterator is a
...
//                          default off — toggle with `allr photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Allr can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Allr'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Allr allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Allr, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Allr supplied them so a known-good
