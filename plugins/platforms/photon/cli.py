"""
``allr photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Allr
...
# Writable sidecar runtime dir (mirrors to ALLR_HOME on immutable
...
# when called later (gateway start, allr status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `allr photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Allr')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``allr photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Allr" project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.allr/.env,
    ...
    print("  Start the gateway:  allr gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `allr photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`allr photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.allr/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `allr photon telemetry on` / `allr photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.allr/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  allr gateway restart")
    return 0
...
#
# `allr gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``allr photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `allr gateway setup` wizard."""
