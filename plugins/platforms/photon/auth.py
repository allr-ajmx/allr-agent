...
Credential storage mirrors every other Allr channel:

    * runtime SDK creds  -> ``~/.allr/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.allr/auth.json`` under
...
# registers Allr as its own client_id.
...
# Default name of the project Allr provisions for the operator.
DEFAULT_PROJECT_NAME = "Allr"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of allr-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.allr/auth.json`` honouring the active Allr profile."""
    try:
        from hermes_constants import get_hermes_home
        return Path(get_hermes_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.allr")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.allr/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.allr/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.allr/.env`` (canonical runtime store).
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `allr photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `allr photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `allr photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `allr photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `allr photon setup`)"
