...
This plugin connects Allr to iMessage (and other Spectrum
...
TypeScript-only, Allr runs it inside a small supervised Node sidecar and
...
There is no separate `login` command; like every other Allr channel,
...
Run `allr photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.allr/.env` (the same place every other
...
Management metadata lives in `~/.allr/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `allr photon telemetry on\|off` (restart the gateway to apply) |
...
- **Inbound attachments and voice notes are downloaded.** The sidecar reads
  ...
  something arrived. If Spectrum emits a `richlink` content object, Allr
  ...
  `.pluginPayloadAttachment` images immediately after the URL; Allr coalesces
...
- **Native polls are supported.** Allr posts poll content through
...
- **Cron/standalone sends require a running gateway.** Processes outside
  the gateway (cron subprocesses, `allr send`) cannot spawn the sidecar;
...
6. Verify end-to-end: `allr photon status`, a DM and a group roundtrip,
