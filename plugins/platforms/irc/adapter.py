"""
IRC Platform Adapter for Allr.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Allr agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Allr")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Allr shutting down")
...
def check_requirements() -> bool:
    ...
    # allr setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `allr gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Allr to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    print()
    print_success("IRC configuration saved to ~/.allr/.env")
    print_info("Restart the gateway for changes to take effect: allr gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Allr (cron)")
