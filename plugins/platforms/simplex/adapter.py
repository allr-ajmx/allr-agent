"""SimpleX Chat platform adapter (Allr plugin).
...
``plugins/platforms/simplex/``. The Allr plugin loader scans the
...
discoverable and ``allr setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Allr — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("ALLR_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``allr setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.allr/.env`` via
    ``hermes_cli.config``.
    """
    print()
