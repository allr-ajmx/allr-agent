...
# identify HermesAgent traffic — matching other Allr outbound surfaces
# that already set ``HermesAgent/<version>`` for platform-partner attribution.
try:
    from hermes_cli import __version__ as _ALLR_VERSION
except Exception:
    _ALLR_VERSION = "unknown"
_ALLR_SLACK_USER_AGENT_PREFIX = f"HermesAgent/{_ALLR_VERSION}"
...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `allr slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not raw_token:
            logger.error(
                ...
                "error; set SLACK_BOT_TOKEN via `allr gateway setup` "
                "or in the active profile's ~/.allr/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_bot_token",
                "SLACK_BOT_TOKEN not configured. Use `allr gateway setup` "
                "or add it to your active profile's ~/.allr/.env file, "
        ...
        if not app_token:
            logger.error(
                ...
                "error; set SLACK_APP_TOKEN via `allr gateway setup` "
                "or in the active profile's ~/.allr/.env file, then restart "
                "the gateway.",
            )
            self._set_fatal_error(
                "missing_slack_app_token",
                "SLACK_APP_TOKEN not configured. Use `allr gateway setup` "
                "or add it to your active profile's ~/.allr/.env file, "
                "then restart the gateway.",
                retryable=False,
            )
        ...
        try:
            ...
            primary_client = AsyncWebClient(
                token=primary_token,
                user_agent_prefix=_ALLR_SLACK_USER_AGENT_PREFIX,
            )
            ...
            # Allr has no listener for (e.g. user_change,
            ...
            async def handle_unhandled_event(event, body, logger):
                logger.debug(
                    ...
                    "registered; subscribed events not handled by Allr can "
            ...
            # manifest (see `allr slack manifest`). In Socket Mode, Slack
            ...
            if _allow_bots_cfg != "none":
                logger.info(
                    ...
                    "'allr slack manifest' if unsure), and (b) the other bot's "
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Allr handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Allr session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Allr owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        # the shape produced by peer Allr agents in Socket Mode on some
...
def interactive_setup() -> None:
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under ALLR_HOME, and print
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="Allr",
                bot_description="Your Allr agent on Slack",
            )
            ...
            print_info(
                "   Re-run `allr slack manifest --write` anytime to refresh after "
                "Allr adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `allr update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://allr.work/docs/user-guide/messaging/slack/")
    print()
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `allr setup` to install Slack support.",
