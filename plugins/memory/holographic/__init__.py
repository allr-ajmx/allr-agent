...
Config in $ALLR_HOME/config.yaml (profile-scoped):
  plugins:
    hermes-memory-store:
      db_path: $ALLR_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Expand $ALLR_HOME in user-supplied paths so config values like
        # "$ALLR_HOME/memory_store.db" or "~/.allr/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$ALLR_HOME", _hermes_home)
            db_path = db_path.replace("${ALLR_HOME}", _hermes_home)
