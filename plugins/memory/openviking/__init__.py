...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Allr peer ID in OpenViking (default: hermes)
...
_AGENT_PROMPT_LABEL = "Allr peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "ALLR_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Allr slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Allr to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Allr refused the connection."
        ) from exc
...
def _openviking_server_log_path() -> Path:
    try:
        ...
        home = Path(os.environ.get("ALLR_HOME", "")).expanduser() if os.environ.get("ALLR_HOME") else Path.home() / ".allr"
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Allr did not start "
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Allr will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Allr session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Allr only", "write values only to Allr .env"),
    ...
    _print_openviking_ready("Connection saved to Allr .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See allr-agent#28296 review.
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Allr peer ID in OpenViking, sent as the actor peer and "
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        if not hermes_home:
            try:
                ...
                hermes_home = str(Path.home() / ".allr")
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Allr will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.allr/.env`` after startup never reach the live client and tools
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Allr will retry on a "
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Allr canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See allr-agent#28296.
