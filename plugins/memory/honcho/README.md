...
```bash
allr memory setup honcho   # configure Honcho directly (works on a fresh install)
allr memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
allr config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.allr/.env
```

> `allr honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `allr memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `allr model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ALLR_HOME/honcho.json` | Profile-local (isolated Allr instances) |
| 2 | `~/.allr/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Allr profile: `hermes` (default) or `hermes_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `allr honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Allr session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`allr-agent`) |
...
If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-allr-agent`.
...
- **`per-session`** — Allr session ID (timestamp + hex). Every `hermes` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Allr profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Host key is derived from the active Allr profile: `hermes` (default) or `hermes_<profile>` (e.g. `hermes -p coder` -> host key `hermes_coder`). Older `hermes.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ALLR_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `allr-agent`) |
...
|---------|-------------|
| `allr memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `allr honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `allr memory setup`) |
| `allr honcho status` | Show resolved config for active profile |
| `allr honcho enable` / `disable` | Toggle Honcho for active profile |
| `allr honcho mode <mode>` | Change recall or observation mode |
| `allr honcho peer --user <name>` | Update user peer name |
| `allr honcho peer --ai <name>` | Update AI peer name |
| `allr honcho tokens --context <N>` | Set context token budget |
| `allr honcho tokens --dialectic <N>` | Set dialectic max chars |
| `allr honcho map <name>` | Map current directory to a session name |
| `allr honcho sync` | Create host blocks for all Allr profiles |
