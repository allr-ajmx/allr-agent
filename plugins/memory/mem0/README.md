...
Behavioral settings live in `$ALLR_HOME/mem0.json` (set them via `allr memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.allr/.env`.
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Allr session and call `mem0_search` — it connects to your server.
...
```bash
allr memory setup mem0 --mode oss \
...
```bash
allr memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ALLR_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.allr/mem0_qdrant"}}
  }
}
```
...
```bash
allr memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ALLR_HOME/mem0.json`).
