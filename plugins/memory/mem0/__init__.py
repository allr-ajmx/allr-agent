...
Configuration
-------------
Secret (lives in $ALLR_HOME/.env or the environment):
...
Behavioral settings (live in $ALLR_HOME/mem0.json, set via `allr memory
setup`):
...
def _load_config() -> dict:
    """Load config from env vars, with $ALLR_HOME/mem0.json overrides.
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hermes_home):
        """Write config to $ALLR_HOME/mem0.json."""
        import json
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $ALLR_HOME/mem0.json) —
