...
request omits ``thinking``.  Allr' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Allr reasoning
...
def _glm_5_2_reasoning_effort(reasoning_config: dict | None) -> str | None:
    """Map Allr reasoning effort onto GLM-5.2's native ``high``/``max``.
