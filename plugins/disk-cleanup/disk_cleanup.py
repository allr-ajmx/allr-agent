"""disk_cleanup — ephemeral file cleanup for Allr.
...
Rules:
  ...
  - empty dirs    → always delete (under ALLR_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ALLR_HOME and /tmp/hermes-*
Never touches: ~/.allr/logs/ or any system directory.
"""
...
try:
    ...
    def get_hermes_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ALLR_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".allr").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ALLR_HOME/logs/``."""
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ALLR_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ALLR_HOME or ``/tmp/hermes-*``.
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "allr-agent", "backups", "profiles", ".worktrees",
...
# Paths under $ALLR_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so ALLR_HOME is resolved
    # exactly once.
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ALLR_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ALLR_HOME, but never recurse into known
    # durable state trees.  Some installs place the Allr checkout, venv,
    # and desktop build under ALLR_HOME; a full rglob over that tree can
...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        # Path isn't under ALLR_HOME (e.g. /tmp/hermes-*) — fall through.
        pass
