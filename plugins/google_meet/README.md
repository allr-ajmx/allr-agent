...
│                                                                      │
│   NodeServer (from `allr meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `allr meet` CLI |
...
| `cli.py` | `allr meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ALLR_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `allr meet node {run,list,approve,remove,status,ping}` |
...
allr meet install                                      # pip + Chromium
allr meet setup                                        # preflight
allr meet auth                                         # optional
allr meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
allr meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.allr/.env
allr meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
allr meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
allr meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
...
allr plugins enable google_meet
allr meet node run --display-name my-mac --host 0.0.0.0 --port 18789
