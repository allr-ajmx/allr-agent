...
allr meet install                 # pip deps + Chromium (transcribe only)
allr meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
allr meet auth                    # optional; skips guest-lobby wait
allr meet setup                   # preflight checks
```

`allr meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or ALLR_MEET_REALTIME_KEY in ~/.allr/.env
```
...
pip install playwright websockets && python -m playwright install chromium
allr plugins enable google_meet
allr meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
allr meet node approve my-mac ws://<mac-ip>:18789 <token>
allr meet node ping my-mac                   # confirm reachable
```

Run `allr meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "An Allr agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `allr meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `ALLR_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
Local:
```
$ALLR_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$ALLR_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `allr meet node approve`.
