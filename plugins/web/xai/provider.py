...
``{title, url, description, position}`` rows every other Allr web
provider produces.
...
prefers Allr-managed xAI Grok OAuth (via ``allr auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.allr/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``allr tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `allr auth` to sign in with "
