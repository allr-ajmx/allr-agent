...
three virtual model IDs so the ``allr tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `allr tools` → Image "
                    "Generation → OpenAI to configure, or `allr setup` "
                    "to add the key."
                ),
