...
models show up in ``allr tools`` automatically. No model ids are
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `allr tools` → Image "
                    "Generation → DeepInfra to configure, or `allr setup` "
                    "to add the key."
                ),
