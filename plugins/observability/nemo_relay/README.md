...
Optional Allr observability plugin that configures exporters and maps
Allr-specific observer hooks to NeMo Relay marks and ATIF state. Allr core
...
not replace Allr's planner, tools, memory, model provider routing, or
CLI UX. Allr core emits NeMo Relay lifecycle events for provider and tool
...
With this plugin enabled, Allr can:

- Export the Relay scopes and LLM/tool lifecycles emitted by Allr core.
- Add Allr session, turn, approval, and subagent mark events.
...
```bash
allr plugins enable observability/nemo_relay
```

The `ALLR_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `ALLR_HOME` that the
agent run will use:

```bash
env ALLR_HOME=/tmp/hermes-nemo-relay-test \
  allr plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`ALLR_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`ALLR_HOME` is the Allr profile/config home used by both
`allr plugins enable ...` and the later `allr chat ...` run. If unset,
Allr uses the user's default home, usually `~/.allr`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export ALLR_HOME=/tmp/hermes-nemo-relay-test
allr plugins enable observability/nemo_relay
allr chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv sync
uv run allr plugins enable observability/nemo_relay
uv run allr chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
wheel, Allr installs its supported NeMo Relay runtime as a normal dependency:
...
behavior; it does not control Allr shared client metrics.

## Export Configuration

The plugin can configure exporters directly from `ALLR_NEMO_RELAY_*`
...
```bash
export ALLR_NEMO_RELAY_ATOF_ENABLED=1
export ALLR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export ALLR_NEMO_RELAY_ATIF_ENABLED=1
export ALLR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `ALLR_NEMO_RELAY_ATOF_FILENAME`
- `ALLR_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `ALLR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `ALLR_NEMO_RELAY_ATIF_AGENT_NAME`
- `ALLR_NEMO_RELAY_ATIF_AGENT_VERSION`
- `ALLR_NEMO_RELAY_ATIF_MODEL_NAME`
- `ALLR_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Allr at it:

```bash
export ALLR_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `ALLR_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`ALLR_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`ALLR_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Allr does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Allr keeps the direct env-var fallbacks active for that run.

Allr core routes provider and tool execution through NeMo Relay managed APIs
...
Allr uses the dynamic-plugin activation API available in NeMo Relay 0.6 and
later. Configure native or worker plugins with Allr-owned
...
with this Allr-owned section. The gateway combines those records with
separate lifecycle state for enablement, trust policy, and worker environments;
the Python binding does not yet expose that resolver. Allr rejects
...
Allr activates these plugins before registering its managed LLM and tool
...
For the full generic Allr middleware contract, see
...
with Allr and a local Ollama model served through the OpenAI-compatible API.

```bash
export ALLR_HOME=/tmp/hermes-nemo-relay-docs/hermes-home
mkdir -p "$ALLR_HOME"

cat > "$ALLR_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Allr session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export ALLR_NEMO_RELAY_ATOF_ENABLED=1
export ALLR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hermes-nemo-relay-docs/subagent/atof
export ALLR_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export ALLR_NEMO_RELAY_ATOF_MODE=overwrite
export ALLR_NEMO_RELAY_ATIF_ENABLED=1
export ALLR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hermes-nemo-relay-docs/subagent/atif
export ALLR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export ALLR_NEMO_RELAY_ATIF_AGENT_NAME='Allr E2E'
export ALLR_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export ALLR_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"allr.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Allr E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Allr dispatches the read-only tools as one batch, and NeMo Relay
...
export ALLR_NEMO_RELAY_ATOF_ENABLED=1
export ALLR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hermes-nemo-relay-docs/parallel/atof
export ALLR_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export ALLR_NEMO_RELAY_ATOF_MODE=overwrite
export ALLR_NEMO_RELAY_ATIF_ENABLED=1
export ALLR_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hermes-nemo-relay-docs/parallel/atif
export ALLR_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export ALLR_NEMO_RELAY_ATIF_AGENT_NAME='Allr E2E'
export ALLR_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Allr sessions map to `agent` scopes.
- Allr core managed provider calls map to `llm` scope start/end events.
- Allr core managed tool calls map to `tool` scope start/end events.
...
```bash
export ALLR_NEMO_RELAY_PLUGINS_TOML=/tmp/hermes-middleware-test/plugins.toml
```
...
Allr provider call
  -> nemo_relay.llm.execute(...)
    -> Allr provider adapter callback(...)

Allr tool call
  -> nemo_relay.tools.execute(...)
    -> Allr authorization and dispatch callback(...)
```
...
middleware for a local Allr run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`, as provided by NeMo Relay 0.6
and later.

```bash
export ALLR_HOME=/tmp/hermes-middleware-test/hermes-home
mkdir -p "$ALLR_HOME" /tmp/hermes-middleware-test/nemo-relay
...
agent_name = "Allr Middleware E2E"
agent_version = "local"
...
export ALLR_NEMO_RELAY_PLUGINS_TOML=/tmp/hermes-middleware-test/nemo-relay/plugins.toml
