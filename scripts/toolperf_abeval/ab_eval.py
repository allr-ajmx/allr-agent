...
measured in the production session DB) through `allr chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    ALLR_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Allr home with credentials for the
    ...
    for rep in range(reps):
        for name in TASKS:
            ...
            env.update({
                ...
                "ALLR_HOME": str(HOME),
                "ALLR_NEMO_RELAY_ATOF_ENABLED": "1",
                "ALLR_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY": str(atof.parent),
                "ALLR_NEMO_RELAY_ATOF_FILENAME": atof.name,
                "ALLR_NEMO_RELAY_ATOF_MODE": "overwrite",
            })
