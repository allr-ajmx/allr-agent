 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.allr/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.allr', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (ALLR_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.ALLR_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.allr', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.ALLR_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.allr', 'document_cache');
const AUDIO_CACHE_DIR = process.env.ALLR_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.allr', 'audio_cache');
...
// `allr update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '*Allr*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls Allr itself created (tracked when
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Allr and trigger
      ...
      // Ignore Allr' own reply messages in self-chat mode to avoid loops.
