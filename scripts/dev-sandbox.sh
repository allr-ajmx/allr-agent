...
Options:
  ...
  --root                Install as uid 0 with the root FHS layout: code in
                        /usr/local/lib/allr-agent, command in
...
$ALLR_HOME/allr-agent plus a ~/.local/bin launcher. Pass --root for the FHS
...
Environment:
  ALLR_DEV_SANDBOX_DIR    Sandbox directory name, relative to the repo root
                            (default: .hermes-sandbox).

Examples:
  # create a sandbox, install this branch as `main`, and then drop to a shell,
  # skipping `allr setup` & the browser tools for speed.
...
INSTALL_REF=""
UPSTREAM_URL="${ALLR_DEV_SANDBOX_UPSTREAM:-https://github.com/NousResearch/hermes-agent.git}"
...
GIT_ROOT="${ALLR_SANDBOX_SOURCE_ROOT:-$(git rev-parse --show-toplevel)}"
...
SANDBOX_DIR_NAME="${ALLR_DEV_SANDBOX_DIR:-.hermes-sandbox}"
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  mkdir -p "$SANDBOX_ROOT/root/http/allr.work"
  if [ -n "$INSTALL_REF" ]; then
    git -C "$UPSTREAM_REPO" show "$UPSTREAM_COMMIT:scripts/install.sh" \
      > "$SANDBOX_ROOT/root/http/allr.work/install.sh"
  else
    cp -a "$INSTALLER_PATH" "$SANDBOX_ROOT/root/http/allr.work/install.sh"
  fi
  set -- bash -c '
    set +e
    curl -fsSL https://allr.work/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/allr-agent.git update-ref refs/heads/main "$next_main"; then
...
# checkout to $ALLR_HOME; both live under the sandbox HOME, which is bound
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/allr-agent.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q allr-agent.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  git -C "$SNAPSHOT_REPO" config user.name 'Allr sandbox'
