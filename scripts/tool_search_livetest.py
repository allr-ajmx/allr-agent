#!/usr/bin/env python3
"""Live test harness for Allr's Tool Search feature.
...
ORIGINAL_HOME = os.environ.get("ALLR_HOME")
ORIGINAL_AUTH = Path.home() / ".allr" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.allr/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.allr/.env`` so
    ...
    real_env_file = Path.home() / ".allr" / ".env"
    if real_env_file.exists():
        ...
        load_hermes_dotenv(hermes_home=str(Path.home() / ".allr"))
...
def reset_module_state():
    """Drop cached modules so the new ALLR_HOME takes effect."""
...
def main():
    ...
    # Restore original ALLR_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ALLR_HOME"] = ORIGINAL_HOME
    else:
