...
#   2. ~/.allr/node/ from a prior Allr-managed install
#   3. fnm, proto, nvm (in that order) if the user already uses a version manager
#   4. Termux `pkg`, macOS Homebrew
#   5. pinned nodejs.org tarball into ~/.allr/node/ (always works, zero shell rc edits)
#
# Usage:
#   source scripts/lib/node-bootstrap.sh
#   ensure_node   # returns 0 on success, non-zero on failure
#   if [ "$ALLR_NODE_AVAILABLE" = true ]; then ...; fi
#
# Env inputs (set before sourcing to override defaults):
#   ALLR_NODE_MIN_VERSION   (default: 20)   — accepted on PATH
#   ALLR_NODE_TARGET_MAJOR  (default: 22)   — installed when we install
#   ALLR_HOME               (default: $HOME/.allr)
# ============================================================================

ALLR_NODE_MIN_VERSION="${ALLR_NODE_MIN_VERSION:-20}"
ALLR_NODE_TARGET_MAJOR="${ALLR_NODE_TARGET_MAJOR:-22}"
ALLR_HOME="${ALLR_HOME:-$HOME/.allr}"
ALLR_NODE_AVAILABLE=false
...
# Redirect an Allr-managed Node's `npm install -g` to the command link dir
# (already on PATH) instead of the default $ALLR_HOME/node/bin, which is off
...
_nb_configure_npm_prefix() {
    ...
    printf 'prefix=%s\n' "$(dirname "$_link_dir")" > "$ALLR_HOME/node/etc/npmrc"
}
...
_nb_npm_range() {
    if [ -n "${ALLR_NPM_TARGET_RANGE:-}" ]; then
        printf '%s\n' "$ALLR_NPM_TARGET_RANGE"
        return 0
    fi
...
_nb_ensure_bundled_npm_range() {
    local npm_bin="$ALLR_HOME/node/bin/npm"
    [ -x "$npm_bin" ] || return 0
    ...
    _nb_warn "Fix manually: npm install -g --prefix \"$ALLR_HOME/node\" npm@\"$range\""
    return 1
}

_nb_have_modern_node() {
    command -v node >/dev/null 2>&1 || return 1
    [ "$(_nb_node_major)" -ge "$ALLR_NODE_MIN_VERSION" ]
}
...
_nb_try_fnm() {
    ...
    _nb_log "fnm detected — installing Node $ALLR_NODE_TARGET_MAJOR..."
    eval "$(fnm env 2>/dev/null)" || true
    fnm install "$ALLR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    fnm use     "$ALLR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
...
_nb_try_proto() {
    ...
    _nb_log "proto detected — installing Node $ALLR_NODE_TARGET_MAJOR..."
    proto install node "$ALLR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_nvm() {
    ...
    _nb_log "nvm detected — installing Node $ALLR_NODE_TARGET_MAJOR..."
    nvm install "$ALLR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    nvm use     "$ALLR_NODE_TARGET_MAJOR" >/dev/null 2>&1 || return 1
    _nb_have_modern_node || return 1
...
_nb_try_brew() {
    ...
    brew install "node@${ALLR_NODE_TARGET_MAJOR}" >/dev/null 2>&1 \
        || brew install node >/dev/null 2>&1 \
        || return 1
    brew link --overwrite --force "node@${ALLR_NODE_TARGET_MAJOR}" >/dev/null 2>&1 || true
    _nb_have_modern_node || return 1
...
_nb_install_bundled_node() {
    ...
    local index_url="https://nodejs.org/dist/latest-v${ALLR_NODE_TARGET_MAJOR}.x/"
    local tarball
    tarball=$(curl -fsSL "$index_url" \
        | grep -oE "node-v${ALLR_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.xz" \
        | head -1)
    if [ -z "$tarball" ]; then
        tarball=$(curl -fsSL "$index_url" \
            | grep -oE "node-v${ALLR_NODE_TARGET_MAJOR}\.[0-9]+\.[0-9]+-${node_os}-${node_arch}\.tar\.gz" \
            | head -1)
    fi
    if [ -z "$tarball" ]; then
        _nb_warn "Could not resolve Node $ALLR_NODE_TARGET_MAJOR binary for $node_os-$node_arch"
        return 1
    fi
    ...
    _nb_log "Extracting to $ALLR_HOME/node/..."
    ...
    mv "$extracted" "$ALLR_HOME/node"
    rm -rf "$tmp"

    local _link_dir
    _link_dir="$(_nb_get_link_dir)"
    # ALLR_NODE_SKIP_LINKS=1: the caller only wants the private managed tree
    ...
    if [ "${ALLR_NODE_SKIP_LINKS:-0}" != "1" ]; then
        mkdir -p "$_link_dir"
        ln -sf "$ALLR_HOME/node/bin/node" "$_link_dir/node"
        ln -sf "$ALLR_HOME/node/bin/npm"  "$_link_dir/npm"
        ln -sf "$ALLR_HOME/node/bin/npx"  "$_link_dir/npx"
    fi
    ...
    _nb_have_modern_node || return 1
    _nb_ok "Node $(node --version) installed to $ALLR_HOME/node/"
...
# ---------------------------------------------------------------------------
# Heal a broken Allr-managed Node tree (partial upgrade / missing lib/)
# ---------------------------------------------------------------------------
...
_nb_managed_tool_broken() {
    ...
    local probe
    for probe in \
        "$ALLR_HOME/node/bin/$tool" \
        "$ALLR_HOME/node/${tool}.exe" \
        "$ALLR_HOME/node/$tool"; do
...
# The managed node runs but is below ALLR_NODE_TARGET_MAJOR — an old tree
...
_nb_managed_node_outdated() {
    ...
    for probe in "$ALLR_HOME/node/bin/node" "$ALLR_HOME/node/node"; do
        [ -x "$probe" ] || continue
        ver="$("$probe" --version 2>/dev/null)" || return 1
        major="${ver#v}"; major="${major%%.*}"
        case "$major" in ''|*[!0-9]*) return 1 ;; esac
        [ "$major" -lt "$ALLR_NODE_TARGET_MAJOR" ] && return 0
        return 1
    done
    return 1
}
...
heal_managed_node() {
    [ -d "$ALLR_HOME/node" ] || return 1
    if ! _nb_managed_node_needs_heal; then
        return 0
    fi
    _nb_log "Allr-managed Node is broken — redownloading to $ALLR_HOME/node/..."
    _nb_install_bundled_node
}
...
ensure_node() {
    ...
    if _nb_have_modern_node; then
        ...
        ALLR_NODE_AVAILABLE=true
        return 0
    fi

    if [ -x "$ALLR_HOME/node/bin/node" ]; then
        export PATH="$ALLR_HOME/node/bin:$PATH"
        if _nb_have_modern_node; then
            _nb_ok "Node $(node --version) found (Allr-managed)"
            ALLR_NODE_AVAILABLE=true
    ...
    _nb_try_fnm   && { ALLR_NODE_AVAILABLE=true; return 0; }
    _nb_try_proto && { ALLR_NODE_AVAILABLE=true; return 0; }
    _nb_try_nvm   && { ALLR_NODE_AVAILABLE=true; return 0; }

    # Platform package managers.
    _nb_try_termux_pkg && { ALLR_NODE_AVAILABLE=true; return 0; }
    _nb_try_brew       && { ALLR_NODE_AVAILABLE=true; return 0; }

    # Last resort: pinned nodejs.org tarball.
    _nb_install_bundled_node && { ALLR_NODE_AVAILABLE=true; return 0; }

    _nb_warn "Node.js install failed — TUI and browser tools will be unavailable."
    _nb_warn "Install manually: https://nodejs.org/en/download/  (or: \`brew install node\`, \`fnm install $ALLR_NODE_TARGET_MAJOR\`, etc.)"
    return 1
}
