#!/usr/bin/env python3
"""Drive the Allr TUI under ALLR_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.allr/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with ALLR_TUI_RESUME set so it
bypasses the hermes_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  ALLR_PERF_LOG     (default ~/.allr/perf.log)
  ALLR_PERF_NODE    (default node from $PATH)
  ALLR_TUI_DIR      (default: <repo>/ui-tui relative to this script)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests ALLR_DEV_PERF wiring is broken).
"""
...
DEFAULT_TUI_DIR = Path(
    os.environ.get("ALLR_TUI_DIR")
    or str(Path(__file__).resolve().parent.parent / "ui-tui")
)
DEFAULT_LOG = Path(os.environ.get("ALLR_PERF_LOG", str(get_hermes_home() / "perf.log")))
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — ALLR_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["ALLR_DEV_PERF"] = "1"
    env["ALLR_DEV_PERF_MS"] = str(args.threshold_ms)
    env["ALLR_DEV_PERF_LOG"] = str(log)
    env["ALLR_TUI_RESUME"] = sid
    ...
    node = os.environ.get("ALLR_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="ALLR_DEV_PERF_MS (0 = capture all)")
