...
The bootstrap module is imported at the top of every Allr entry point
(hermes, allr-agent, allr-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Allr entry point imports hermes_bootstrap as its
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Allr CLI
...
class TestEntryPointsImportBootstrap:
    """Every Allr entry point must import hermes_bootstrap as its
    ...
    # Entry points that invoke Allr as a process.  Each one must
    # import hermes_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "hermes_cli/main.py",   # hermes CLI (console_script)
        "run_agent.py",          # allr-agent (console_script)
        "acp_adapter/entry.py",  # allr-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        half-finished ``allr update`` (git-reset landed new code but
        ...
        """
        # Resolve relative to the allr-agent repo root.  Tests live
...
class TestHardenImportPath:
    ...
    directory from shadowing Allr's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("ALLR_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["ALLR_PYTHON_SRC_ROOT"] = env
            elif "ALLR_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["ALLR_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/hermes")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("ALLR_PYTHON_SRC_ROOT", None)
            else:
                os.environ["ALLR_PYTHON_SRC_ROOT"] = original_env
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["ALLR_PYTHON_SRC_ROOT"] = "/env/hermes"
