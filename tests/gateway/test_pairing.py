...
class TestProfileScopedDiscovery:
    def test_list_approved_scopes_platform_discovery_to_profile_dir(self, tmp_path):
        ...
        # ALLR_HOME — the current home may itself be a profile, and nesting
...
class TestProfileScopedStorage:
    ...
    under each profile's own Allr home so a multiplexing gateway can keep
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``allr pairing`` CLI continues to work without a profile context."""
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        """Explicit profile stores use that profile's normal Allr layout."""
        monkeypatch.setenv("ALLR_HOME", str(tmp_path))
