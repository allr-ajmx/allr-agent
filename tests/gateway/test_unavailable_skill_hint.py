...
def tmp_skills(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Isolated skills dir + ALLR_HOME so the real user config is untouched."""
    home = tmp_path / ".allr"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_frontmatter_slug_matched_even_when_dir_name_differs(
    ...
    # Config disables by declared name (matches what `allr skills config` writes).
    monkeypatch.setattr(
    ...
    assert "allr skills config" in msg
