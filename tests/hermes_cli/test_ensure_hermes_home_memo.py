...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".allr"
    second = tmp_path / "b" / ".allr"

    monkeypatch.setenv("ALLR_HOME", str(first))
    cfg.ensure_hermes_home()

    # Profile switch: ALLR_HOME moves → the new path is ensured too.
    monkeypatch.setenv("ALLR_HOME", str(second))
    cfg.ensure_hermes_home()
