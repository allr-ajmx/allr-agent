"""Regression test: `allr dashboard --tui` must not hard-crash.

Older Allr desktop app shells (<= 0.15.x) spawn the backend as::

    allr dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Allr couldn't
