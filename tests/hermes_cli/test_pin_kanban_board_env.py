...
board into `ALLR_KANBAN_BOARD` at boot so subprocess shell-outs (e.g.
`allr kanban …`) inherit the same board the in-process kanban tools resolve.
Without this, a concurrent `allr kanban boards switch` from another session
...
def _isolate_kanban_board_env():
    """Snapshot `ALLR_KANBAN_BOARD` and restore it after the test.
    ...
    """
    prev = os.environ.get("ALLR_KANBAN_BOARD")
    ...
    try:
        ...
        if prev is None:
            ...
            os.environ["ALLR_KANBAN_BOARD"] = prev
...
def test_pin_writes_resolved_board_when_env_unset(monkeypatch):
    ...
    assert main_mod.os.environ.get("ALLR_KANBAN_BOARD") == "space"


def test_pin_does_not_overwrite_existing_env(monkeypatch):
    monkeypatch.setenv("ALLR_KANBAN_BOARD", "preset")
    main_mod = importlib.import_module("hermes_cli.main")
    ...
    assert main_mod.os.environ.get("ALLR_KANBAN_BOARD") == "preset"
