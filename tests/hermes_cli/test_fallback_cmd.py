"""Tests for `allr fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate ALLR_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".allr" / "config.yaml"
...
class TestListCommand:
    def test_list_empty(self, isolated_home, capsys):
        ...
        assert "allr fallback add" in out
...
class TestArgparseWiring:
    """Verify `allr fallback` is wired into main.py's argparse tree.
