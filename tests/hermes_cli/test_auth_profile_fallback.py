...
When ``ALLR_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.allr/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.allr            (has its own auth.json fixture)
    * Profile     -> tmp_path/.allr/profiles/coder   (active, ALLR_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Allr is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (ALLR_HOME != ~/.allr, no profiles)."""
