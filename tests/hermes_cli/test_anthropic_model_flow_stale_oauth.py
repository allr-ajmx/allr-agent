"""Tests for Bug #12905 fix — stale OAuth token detection in allr model flow.

Bug 3: `allr model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.allr/.env but no valid
