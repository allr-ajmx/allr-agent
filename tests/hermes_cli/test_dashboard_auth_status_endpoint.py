...
def test_health_reports_liveness_without_loading_gateway_config(gated_client, monkeypatch):
    ...
    assert body == {
        ...
        # cannot tell "no terminal API" from "you are not allowed". Reattach
        # rides the same substrate, so it is advertised alongside it.
        "features": {"shell_pty": True, "shell_pty_reattach": True},
    }
