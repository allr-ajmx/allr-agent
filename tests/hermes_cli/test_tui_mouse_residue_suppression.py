...
where stdin is still in cooked + echo mode. If a previous Allr session
...
class TestEarlyMouseDisable:
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        ...
        monkeypatch.setenv("ALLR_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_oserror_is_swallowed(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["hermes", "--tui"])
        monkeypatch.delenv("ALLR_TUI", raising=False)
        monkeypatch.delenv("ALLR_TUI_NO_EARLY_DISABLE", raising=False)
