...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``allr update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    # Exact flags used by allr update (hermes_cli/main.py).
...
def test_update_autostash_survives_undeletable_untracked_dir(tmp_path):
    ...
    (pkg / "allr-agent.rb").write_text("formula\n")
    ...
    try:
        ...
        assert (pkg / "allr-agent.rb").read_text() == "formula\n"
    finally:
        os.chmod(pkg, 0o755)
