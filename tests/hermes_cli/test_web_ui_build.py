...
(mirroring the desktop build), recorded in a stamp file under $ALLR_HOME,
NOT mtime comparison — so ``git pull`` / ``allr update`` that rewrite
...
def _isolated_hermes_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ALLR_HOME", str(tmp_path / "_hermes_home"))
...
class TestWebUIBuildNeeded:
    ...
    def test_mtime_only_change_is_not_stale(self, tmp_path):
        ...
        ``git pull`` / ``allr update`` do) must NOT report stale."""
