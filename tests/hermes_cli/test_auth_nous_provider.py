...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ALLR_HOME", str(hermes_home))
    monkeypatch.setenv("ALLR_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `allr model` → Nous Portal → Skip (keep current) after
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `allr auth add nous`
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Allr must surface an
    ...
    re-authenticate" string led users to report this as an Allr persistence
    ...
    assert "allr auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect ALLR_SHARED_AUTH_DIR to a tmp_path.
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without ALLR_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.allr/shared/`` across CI runs.
    """
    from hermes_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("ALLR_SHARED_AUTH_DIR", raising=False)
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `allr auth add nous
