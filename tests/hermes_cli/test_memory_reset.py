"""Tests for the `allr memory reset` CLI command.
...
- Profile-scoped reset (uses ALLR_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ALLR_HOME with memory files."""
    ...
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.allr/allr-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `allr memory reset` is run.
    """
...
class TestMemoryReset:
    """Tests for `allr memory reset` subcommand."""
