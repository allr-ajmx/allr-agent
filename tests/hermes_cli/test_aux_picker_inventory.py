...
Every aux picker (``allr model`` → Configure auxiliary models, the
``allr tools`` vision picker, and any future one) must route through
...
def configured_home(tmp_path, monkeypatch):
    """A ALLR_HOME with one ``providers:`` entry and one legacy
