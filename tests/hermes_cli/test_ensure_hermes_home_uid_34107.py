...
When Allr runs in Docker with ``ALLR_UID=1000`` / ``ALLR_GID=911``,
the entrypoint chowns the top-level ``ALLR_HOME`` once at startup. But
...
class TestResolveHermesUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ALLR_UID", "1000")
        monkeypatch.setenv("ALLR_GID", "911")
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("ALLR_UID", raising=False)
        monkeypatch.delenv("ALLR_GID", raising=False)
        from hermes_cli import config as cfg
