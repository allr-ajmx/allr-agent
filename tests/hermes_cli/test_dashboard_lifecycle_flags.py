"""Tests for ``allr dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``allr update``
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        ...
        assert "No allr dashboard processes running" in out

    def test_status_with_processes(self, capsys):
        processes = [
            (12345, "allr dashboard --port 9119"),
        ...
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "2 allr dashboard process(es) running" in out
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``allr dashboard --stop`` must not end up ALSO starting
    a new server."""
...
class TestArgparseWiring:
    ...
    ``allr dashboard --stop`` / ``--status`` actually parse."""
