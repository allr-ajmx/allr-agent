...
- Bare `allr setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `allr setup --quick` runs the narrower "fill in missing items" flow.
- `allr setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
class TestExistingInstallDefault:
    """Bare `allr setup` on an existing install = full reconfigure wizard."""
