...
* Be supported-platform-only — no-op silently elsewhere so ``allr update``
...
class TestRequireConfirmedUpdate:
    """`allr update` passes require_confirmed_update=True: the full
    ...
    Explicit `allr computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `allr computer-use install --upgrade` (default flag): the old
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `allr update`.
    """
