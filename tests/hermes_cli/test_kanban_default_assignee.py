...
def isolated_kanban_home(monkeypatch):
    """Spin up a fresh ALLR_HOME with a clean kanban DB."""
    test_home = tempfile.mkdtemp(prefix="kanban_default_assignee_test_")
    monkeypatch.setenv("ALLR_HOME", test_home)
    # Force-reimport so the fresh ALLR_HOME is picked up.
    ...
    # gives each test its own monkeypatched ALLR_HOME so no cross-test
    # contamination.
