...
Uses a throwaway ALLR_HOME so it never touches ~/.hermes. No real names.
"""
...
def _fresh_home():
    """Point ALLR_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_hermes_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["ALLR_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER ALLR_HOME is set.
...
def main():
    ...
    print(f"ALLR_HOME (throwaway): {HOME}")
