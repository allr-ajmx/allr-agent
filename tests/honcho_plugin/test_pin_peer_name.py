...
Under a gateway (Telegram, Discord, Slack, ...) Allr passes the
...
class TestPinPeerNameConfigParsing:
    ...
    def test_root_level_true(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ALLR_HOME", str(tmp_path / "isolated"))
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Allr via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each Allr profile can pin to its own unique peerName.
