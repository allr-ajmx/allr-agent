"""Tests for ALLR_HOME credential-file read blocking in file_safety.

Regression for https://github.com/NousResearch/hermes-agent/issues/17656 —
``read_file`` was previously only sandboxed against ``ALLR_HOME`` itself,
...
for the credential stores while leaving arbitrary ``ALLR_HOME`` files
...
def test_arbitrary_hermes_home_file_not_blocked(fake_home):
    """Non-credential files inside ALLR_HOME stay readable."""
...
def test_identically_named_hermes_files_outside_home_not_blocked(
    ...
    """Allr-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside ALLR_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside ALLR_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside ALLR_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, ALLR_HOME = <root>/profiles/<name>, but
