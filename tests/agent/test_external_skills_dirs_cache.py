...
def hermes_home_with_config(tmp_path, monkeypatch):
    """Isolated ``~/.allr/`` with a config.yaml referencing one external dir."""
    home = tmp_path / ".allr"
    home.mkdir()
...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different ALLR_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".allr"
    ...
    monkeypatch.setenv("ALLR_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("ALLR_HOME", str(home_b))
