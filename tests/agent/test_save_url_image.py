...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate ALLR_HOME under tmp_path."""
    monkeypatch.setenv("ALLR_HOME", str(tmp_path / ".allr"))
    (tmp_path / ".allr").mkdir()

    # Force the constants/image cache helpers to re-read ALLR_HOME.
    import sys
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_hermes_home_cache(self, http_server):
        ...
        # The cache directory must be under ALLR_HOME — gateway cleanup
