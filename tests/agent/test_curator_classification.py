...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"allr-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "allr-agent-dev", "declared": True},
        },
    )
...
# `allr curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `allr curator pin <umbrella>` in the rename block so
