...
def backup_env(monkeypatch, tmp_path):
    """Isolate ALLR_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".allr"
    home.mkdir()
    (home / "skills").mkdir()
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under ALLR_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level ALLR_DIR picks up the tmp HOME."""
    import hermes_constants
    importlib.reload(hermes_constants)
