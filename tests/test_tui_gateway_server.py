...
# ---------------------------------------------------------------------------
# Pinned sessions (MJXHRM-368)
#
# Pins used to live only in one client's localStorage, so two clients on the
# same gateway kept private lists and each looked like the other had lost its
# pins. They are gateway state now, read/written through config.* and pushed to
# the other clients by the `pins.changed` watch.
# ---------------------------------------------------------------------------


def _pins_cfg(monkeypatch, stored):
    """Point server._load_cfg/_write_config_key at an in-memory config."""
    cfg = {"display": {"pinned_sessions": stored}} if stored is not None else {}
    written = {}

    def _write(key_path, value):
        written[key_path] = value
        cfg.setdefault("display", {})["pinned_sessions"] = value
...
def test_pinned_sessions_round_trip_through_config_rpcs(monkeypatch):
    written = _pins_cfg(monkeypatch, None)

    empty = server.handle_request(
        {"id": "1", "method": "config.get", "params": {"key": "pinned_sessions"}}
    )
    ...
    saved = server.handle_request(
        {
            "id": "2",
            "method": "config.set",
            "params": {"key": "pinned_sessions", "value": ["b", "a"]},
        }
    )
    assert saved["result"]["value"] == ["b", "a"]
    # Order is the client's, so it must survive the round trip verbatim.
    assert written["display.pinned_sessions"] == ["b", "a"]
    assert server.handle_request(
        {"id": "3", "method": "config.get", "params": {"key": "pinned_sessions"}}
    )["result"]["value"] == ["b", "a"]


def test_pinned_sessions_sanitises_blanks_and_duplicates(monkeypatch):
    _pins_cfg(monkeypatch, None)

    saved = server.handle_request(
        {
            "id": "1",
            "method": "config.set",
            "params": {"key": "pinned_sessions", "value": ["a", "", "  a  ", "b", None]},
        }
    )
    assert saved["result"]["value"] == ["a", "b"]


def test_pinned_sessions_rejects_a_non_list(monkeypatch):
    _pins_cfg(monkeypatch, None)
    ...
    resp = server.handle_request(
        {
            "id": "1",
            "method": "config.set",
            "params": {"key": "pinned_sessions", "value": "a,b"},
        }
    )
    assert resp["error"]["code"] == 4002


def test_pinned_sessions_read_survives_a_hand_edited_config(monkeypatch):
    # config.yaml is hand-editable, so the read path sanitises too.
    _pins_cfg(monkeypatch, ["a", "a", "", "b"])
    assert server.load_pinned_sessions() == ["a", "b"]

    _pins_cfg(monkeypatch, "not-a-list")
    assert server.load_pinned_sessions() == []


def test_pins_changed_signature_tracks_the_list_not_the_file(monkeypatch):
    """The watch fires on the pin list, not on every config write.

    config.yaml's mtime moves for theme/skin/model writes too, which would
    broadcast a pointless pin refresh to every client on each of them.
    """
    _pins_cfg(monkeypatch, ["a"])
    before = server._pinned_sessions_sig()

    _pins_cfg(monkeypatch, ["a"])
    assert server._pinned_sessions_sig() == before

    _pins_cfg(monkeypatch, ["a", "b"])
    assert server._pinned_sessions_sig() != before
    assert server._pinned_sessions_payload() == {"value": ["a", "b"]}
