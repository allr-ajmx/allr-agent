...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_ALLR_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_ALLR_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_ALLR_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_ALLR_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_ALLR_TEST_CRON_UNSET_VAR}"
...
class TestRunJobModelResolution:
    ...
    ``model: null``. At fire time the scheduler must:
      1. fall back to ``ALLR_MODEL`` env if set,
    ...
    def test_null_job_model_falls_back_to_env(self, tmp_path, monkeypatch):
        """``model: null`` on the job uses ALLR_MODEL when set."""
        (tmp_path / "config.yaml").write_text("")
        monkeypatch.setenv("ALLR_MODEL", "env-model")
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``allr cron create --deliver=<platform>`` silently
