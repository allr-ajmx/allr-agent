...
class TestParseSchedule:
    ...
    def test_naive_iso_anchors_to_configured_tz_not_server_local(self, monkeypatch):
        """A naive ISO timestamp must be interpreted in the CONFIGURED Allr
...
class TestNaiveScheduleTimezoneDivergence:
    ...
    become due even when the configured Allr timezone differs from the
...
class TestLateEnvRepointScopesStore:
    """A ALLR_HOME set AFTER cron.jobs import must scope the store even
    ...
    def test_use_cron_store_override_still_wins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ALLR_HOME", str(tmp_path / "env-home"))
    ...
    def test_public_io_after_late_env_repoint_leaves_old_file_untouched(
        ...
        called after a post-import ALLR_HOME repoint must operate on the NEW
        ...
        monkeypatch.setattr(jobs, "ALLR_DIR", sim_old_home)
        ...
        monkeypatch.setenv("ALLR_HOME", str(new_home))
