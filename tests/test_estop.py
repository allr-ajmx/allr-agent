"""Global emergency stop (`allr pause` / `allr resume`) — agent/estop.py.
...
already in flight is never touched. Removing the sentinel (`allr resume`)
...
def hermes_home(tmp_path, monkeypatch):
    """Point ALLR_HOME at a temp dir and reset estop module log state."""
...
def test_is_engaged_fails_safe_on_stat_error(hermes_home, monkeypatch):
    ...
    hold even when ALLR_HOME is misbehaving, matching the module's
