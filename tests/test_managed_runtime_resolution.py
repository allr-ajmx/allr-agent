"""Guard: Allr-owned subprocesses must not resolve managed runtimes by bare PATH.

Allr installs runtimes for itself — ``uv`` at ``$ALLR_HOME/bin/uv``, Node at
``$ALLR_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Allr's own
...
* when a system copy also exists, the one Allr does not own wins — which is
...
# Runtimes Allr provisions into ALLR_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Allr-owned subprocess code: plugins ship their own
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for an Allr-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$ALLR_HOME/bin (uv) and $ALLR_HOME/node are not on an "
