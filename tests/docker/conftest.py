...
Override the image with ``ALLR_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ALLR_TEST_IMAGE", "allr-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ALLR_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ALLR_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
#
# Background: every Allr runtime path inside the container drops to UID
