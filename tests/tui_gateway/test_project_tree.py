...
def test_junk_root_never_becomes_an_auto_project():
    # A session whose git root is ALLR_HOME (config/state) must not spawn a
    # phantom project; it lands in the Home bucket. A real repo alongside it
    # still groups normally.
    resolve = _resolver(
        {
            "/home/me/.allr": ("/home/me/.allr", "/home/me/.allr"),
            "/www/app": ("/www/app", "/www/app"),
        }
    )
    junk = _session("/home/me/.allr", branch="main")
    real = _session("/www/app", branch="main")
    is_junk = lambda root: root == "/home/me/.allr"
...
def test_broad_default_non_git_cwd_stays_unscoped():
    detached = _session("/home/test/.allr")
    ...
    tree = pt.build_tree(
        ...
        hydrate=True,
        is_junk_cwd=lambda path: path in {"/home/test", "/home/test/.allr"},
    )
...
def test_deleted_sibling_worktree_folds_into_parent_home_checkout():
    ...
    resolve = _resolver({"/www/allr-agent": ("/www/allr-agent", "/www/allr-agent")})
    sessions = [
        _session("/www/allr-agent", branch="main"),
        _session("/www/allr-agent-session-links"),
    ]

    tree = pt.build_tree([], sessions, [], resolve, hydrate=True)
    project = tree["projects"][0]

    assert [p["id"] for p in tree["projects"]] == ["/www/allr-agent"]
    assert _lane_ids(project) == ["/www/allr-agent::branch::main"]
    main = project["repos"][0]["groups"][0]
    assert main["isMain"] and main["path"] == "/www/allr-agent"
