...
SESSION_REVOKED / REFRESH_DEAD), the ALLR_HOME resolution fallback, the
...
def _run_main(mod, tokens_dir, argv, responses):
    ...
    with patch.object(mod.os, "environ", dict(mod.os.environ, ALLR_HOME=str(tokens_dir.parent))), \
...
def test_skill_md_frontmatter_invariants():
    ...
    assert fm["author"].split(",")[0].strip() != "Allr"  # human credited first
