...
def test_registered_observable_metric_names_cover_snapshot_metrics(monkeypatch):
    ...
    gateway_snapshot = type("S", (), {"metrics": [
        _M("allr.gateway.up"), _M("allr.gateway.active_agents"),
        _M("allr.gateway.busy"), _M("allr.gateway.drainable"),
        _M("allr.gateway.restart_requested"),
        _M("allr.platform.up"), _M("allr.platform.degraded"),
    ]})()
    cron_snapshot = type("S", (), {"metrics": [
        _M("allr.cron.scheduler.heartbeat_age_seconds"),
        _M("allr.cron.scheduler.last_success_age_seconds"),
        _M("allr.cron.scheduler.catch_up_occurrences"),
        _M("allr.cron.jobs.enabled"), _M("allr.cron.jobs.running"),
        _M("allr.cron.jobs.overdue"),
    ]})()
...
def test_monitoring_docs_distinguish_relay_health_scope_and_terminal_flush():
    ...
    assert "Allr-owned Relay transport health" in text
