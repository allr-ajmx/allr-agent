...
``os.environ["ALLR_YOLO_MODE"]``. That env var is captured once at
...
prompt. Only ``hermes --yolo`` (process-start env), ``ALLR_YOLO_MODE=1``,
and ``allr config set approvals.mode off`` actually bypassed.
...
def _clear_approval_state(monkeypatch):
    ...
    # Allr-driven test runs may inherit ALLR_YOLO_MODE=1 from the parent
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``hermes --yolo`` sets ``ALLR_YOLO_MODE`` before tool imports, so
