"""Shared fixtures for the allr-agent test suite.
...
2. **Isolated ALLR_HOME.** ALLR_HOME points to a per-test tempdir so
   code reading ``~/.allr/*`` via ``get_hermes_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".allr"`` instead of the canonical
   ``get_hermes_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ALLR_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox ALLR_HOME before ANY test module is imported ──────────────────
...
# real `~/.allr/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes ALLR_HOME, but fixtures run
...
# the REAL Allr root — capture it BEFORE the sandbox rewires ALLR_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.allr (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("ALLR_KANBAN_HOME", "").strip()
_PRE_SANDBOX_ALLR_HOME = os.environ.get("ALLR_HOME", "")


def _hermes_home_points_at_production(value: str) -> bool:
    """True when a pre-set ALLR_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    ALLR_HOME=~/.allr``) hand pytest the PRODUCTION home. Historically
    ...
    inside the real ``~/.allr`` — the escape vector that landed pytest
...
if _hermes_home_points_at_production(os.environ.get("ALLR_HOME", "")):
    _SESSION_ALLR_HOME = tempfile.mkdtemp(prefix="hermes-test-home-")
    os.environ["ALLR_HOME"] = _SESSION_ALLR_HOME
    atexit.register(shutil.rmtree, _SESSION_ALLR_HOME, True)

#: ALLR_HOME as it stood when conftest was imported - i.e. before any test
...
ALLR_HOME_AT_CONFTEST_IMPORT = os.environ.get("ALLR_HOME", "")
...
# ALLR_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ALLR_BEHAVIORAL_VARS = frozenset({
    ...
    "ALLR_MAX_ITERATIONS",
    "ALLR_SESSION_PLATFORM",
    "ALLR_SESSION_CHAT_ID",
    "ALLR_SESSION_CHAT_NAME",
    "ALLR_SESSION_CHAT_TYPE",
    "ALLR_SESSION_THREAD_ID",
    "ALLR_SESSION_SOURCE",
    "ALLR_SESSION_KEY",
    "ALLR_GATEWAY_SESSION",
    "ALLR_CRON_SESSION",
    "_ALLR_GATEWAY",
    "ALLR_PLATFORM",
    "ALLR_MODEL",
    "ALLR_INFERENCE_MODEL",
    "ALLR_INFERENCE_PROVIDER",
    "ALLR_TUI_PROVIDER",
    "ALLR_MANAGED",
    "ALLR_MANAGED_DIR",
    "ALLR_DEV",
    ...
    "ALLR_TIMEZONE",
    "ALLR_REDACT_SECRETS",
    "ALLR_BACKGROUND_NOTIFICATIONS",
    "ALLR_EXEC_ASK",
    "ALLR_HOME_MODE",
    "ALLR_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.allr/kanban.db instead of the per-test ALLR_HOME.
    "ALLR_KANBAN_DB",
    "ALLR_KANBAN_BOARD",
    "ALLR_KANBAN_HOME",
    "ALLR_KANBAN_WORKSPACES_ROOT",
    "ALLR_KANBAN_LOGS_ROOT",
    "ALLR_KANBAN_TASK",
    "ALLR_KANBAN_WORKSPACE",
    "ALLR_KANBAN_RUN_ID",
    "ALLR_KANBAN_CLAIM_LOCK",
    "ALLR_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "ALLR_DASHBOARD_OAUTH_CLIENT_ID",
    "ALLR_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ALLR_HOME to per-test tempdirs so code that
    reads ``~/.allr/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ALLR_* vars that could change test semantics.
    for name in _ALLR_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("ALLR_HONCHO_HOST", "hermes")

    # 3. Redirect ALLR_HOME to a per-test tempdir. Code that reads
    #    ``~/.allr/*`` via ``get_hermes_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.allr/*`` via
    #    ``Path.home() / ".allr"`` instead of ``get_hermes_home()``
    ...
    monkeypatch.setenv("ALLR_HOME", str(fake_hermes_home))
    ...
    monkeypatch.setenv("ALLR_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.allr/plugins/ (which, per step 3, is now empty — but the
...
# allow-list because test-level fixtures legitimately move ALLR_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session ALLR_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``ALLR_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Allr root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_ALLR_HOME and not _hermes_home_points_at_production(
        _PRE_SANDBOX_ALLR_HOME
    ):
        # ALLR_HOME was genuinely set to a CUSTOM root before the sandbox
    ...
    # No pre-existing ALLR_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".allr").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.allr`` captured at import time. Hermetic tests that legitimately
    move ALLR_HOME to sibling tempdirs are unaffected.
...
# path that resolves inside the REAL Allr root. This fixture wires the
# test-side knobs:
...
#     installs where ALLR_HOME is not ~/.allr) into the guard's
...
def _state_db_write_guard(request, monkeypatch):
    ...
    if _PRE_SANDBOX_ALLR_HOME and not _hermes_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_ALLR_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Allr home override redirects every later
...
@pytest.fixture()
def mock_config():
    """Return a minimal allr config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``allr-gateway`` process
...
#    ``check_output`` reject any ``systemctl ... <verb> allr-gateway``
...
def _wal_is_usable() -> bool:
    """True when Allr will actually put a database into WAL mode here.

    Allr refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Allr can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside an Allr managed runtime on 3.53.1
    ...
    redirects ``ALLR_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.allr/state.db`` for the whole session. Tests then read live
...
#      process environment: ``os.environ["ALLR_VOICE_TTS"] = "1"``. The
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Allr falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Allr uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart allr-gateway"``,
    ...
    def _matches_hermes_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ALLR_TOKENS)
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live allr-gateway systemd unit. Mock "
        ...
        # Block any subprocess that would run `allr update` (or the
        ...
        if "update" in low and (
            # allr update / allr update --gateway / setsid bash -c ... allr update
            ("hermes" in low and "update" in low.split())
            or
            # python -m hermes_cli.main update --gateway
            ("hermes_cli" in low and "update" in low.split())
            or
            # venv/bin/allr update  (absolute path variant used in tests)
            (".venv/bin/allr" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`allr update` against the real checkout, fetching "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_ALLR_BEHAVIORAL_VARS``: the env blanking stops the flag
