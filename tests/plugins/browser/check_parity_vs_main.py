...
Run from the PR worktree:
    ...
    cd ~/.allr/allr-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.allr/allr-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.allr/allr-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a allr-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a allr-agent checkout"
)
...
# Isolated ALLR_HOME for the config write.
