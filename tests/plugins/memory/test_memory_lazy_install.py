...
(``ALLR_DISABLE_LAZY_INSTALLS=1``) and lazy installs are redirected to a
writable durable target (``ALLR_LAZY_INSTALL_TARGET``). honcho/hindsight
...
#
# This is the exact hosted-Fly condition: ALLR_DISABLE_LAZY_INSTALLS=1 seals
# the venv, but ALLR_LAZY_INSTALL_TARGET redirects installs to a writable
...
class TestSealedVenvDurableTarget:
    ...
    def test_ensure_installs_into_durable_target_on_sealed_venv(
        ...
        monkeypatch.setenv("ALLR_LAZY_INSTALL_TARGET", str(tmp_path / "lazy"))
        ...
        def fake_install(specs, **kw):
            ...
            captured["target_env"] = os.environ.get("ALLR_LAZY_INSTALL_TARGET")
    ...
    def test_sealed_venv_without_target_blocks(self, feature, monkeypatch):
        ...
        monkeypatch.setenv("ALLR_DISABLE_LAZY_INSTALLS", "1")
        monkeypatch.delenv("ALLR_LAZY_INSTALL_TARGET", raising=False)
        monkeypatch.setattr(
            "hermes_cli.config.load_config",
            lambda: {"security": {"allow_lazy_installs": True}},
        )
