"""Tests for the bundled allr-achievements dashboard plugin.
...
``plugins/allr-achievements/tests/`` covering the achievement engine
internals (tier math, secret-state handling, catalog invariants). These
tests live at the allr-agent level and focus on the integration
...
def plugin_api(tmp_path, monkeypatch):
    """Load plugin_api with isolated ~/.allr so state/snapshot files don't collide.
