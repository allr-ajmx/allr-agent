...
``~/.allr/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.allr/.env`` would be told
...
class TestEndToEndRegressionGuard:
    ...
    ``~/.allr/.env`` carrying the key while ``os.environ`` does not.
