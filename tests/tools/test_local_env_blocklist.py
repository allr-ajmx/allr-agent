...
Verifies that Allr-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Allr secrets.
...
from tools.environments.local import (
    LocalEnvironment,
    _ALLR_PROVIDER_ENV_BLOCKLIST,
    _ALLR_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.allr/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is an Allr inference secret
        ...
        models`` run inside an Allr terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _ALLR_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        Allr-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "ALLR_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _ALLR_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_ALLR_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_ALLR_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_ALLR_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_ALLR_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestActiveVenvMarkerStripping:
    ...
    into the Allr venv path instead of the project's own ``.venv`` —
    silently clobbering the Allr environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Allr venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.allr/allr-agent/venv",
        })
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_ALLR_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Allr (#55878).
        """
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _ALLR_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _ALLR_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Allr-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _ALLR_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Allr, and blocklisting
        ...
        leaked_block = general_chain & _ALLR_PROVIDER_ENV_BLOCKLIST
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _ALLR_PROVIDER_ENV_BLOCKLIST
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        for name, metadata in OPTIONAL_ENV_VARS.items():
            ...
            if category in {"tool", "messaging"}:
                assert name in _ALLR_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_hermes_bin_injection(self):
        ...
        saved = local_mod._ALLR_BIN_DIR
        local_mod._ALLR_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._ALLR_BIN_DIR = saved
...
class TestHermesBinDirOnPath:
    ...
    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._ALLR_BIN_DIR = local_mod._SENTINEL

    def test_resolves_via_which(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/hermes/bin/allr" if name == "hermes" else None)
    ...
    def test_make_run_env_injects_hermes_bin_dir(self, monkeypatch):
        ...
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._ALLR_BIN_DIR = "/opt/hermes/bin"
...
class TestHermesInternalDynamicSecrets:
    """Dynamically-named Allr secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_ALLR_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _ALLR_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _ALLR_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _ALLR_PROVIDER_ENV_BLOCKLIST
