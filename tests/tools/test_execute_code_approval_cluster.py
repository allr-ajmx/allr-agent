...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad ALLR_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ALLR_GATEWAY_SESSION set, a bound session
    key, and isolated gateway queues/callbacks. Yields the session_key."""
    monkeypatch.setenv("ALLR_GATEWAY_SESSION", "1")
...
def test_env_scrub_hermes_allowlist_and_secret_blocks():
    ...
    env = {
        ...
        "ALLR_HOME": "/h", "ALLR_PROFILE": "p",
        "ALLR_CONFIG": "/c.yaml", "ALLR_ENV": "/e",
        "ALLR_DELEGATED_CHILD_CONTEXT": "1",
        # other ALLR_* → dropped (broad prefix removed)
        "ALLR_BASE_URL": "https://x", "ALLR_INTERACTIVE": "1",
        "ALLR_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in (
        ...
        "ALLR_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ALLR_BASE_URL", "ALLR_INTERACTIVE", "ALLR_KANBAN_DB",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ALLR_* vars."""
    import logging
