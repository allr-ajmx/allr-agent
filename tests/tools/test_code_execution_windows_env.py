...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``ALLR_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # ALLR_* handling (#27303): only the operational allowlist passes;
        # every other ALLR_* is dropped (the broad prefix was removed).
        "ALLR_HOME": "/home/alice/.allr",        # allowlisted → kept
        "ALLR_PROFILE": "default",                 # allowlisted → kept
        "ALLR_INTERACTIVE": "1",                   # not allowlisted → dropped
        "ALLR_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "ALLR_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
