...
# Org mirrors live under ~/.allr/skills/_org/<org_id>/. Resolution is
...
def _detect_environment(env: str) -> bool:
    ...
    ALLR_KANBAN_* vars without owning them), so caching it process-wide would
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``ALLR_KANBAN_TASK`` / ``ALLR_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("ALLR_KANBAN_TASK") or os.getenv("ALLR_KANBAN_BOARD"):
        ...
        # The Allr Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``ALLR_PLATFORM`` or
            ``ALLR_SESSION_PLATFORM`` env vars.  Returns the global
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.allr/skills/`` are silently skipped.
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against ALLR_HOME, not cwd
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.allr/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.allr/skills/`` (including via symlinks) or configured
    ...
    # ~/.allr/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Allr can discover and view
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Allr metadata, VCS, virtualenv/dependency, cache, and skill
