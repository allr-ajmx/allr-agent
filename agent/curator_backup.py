...
A pre-run snapshot of ``~/.allr/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.allr/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.allr/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.allr/cron/jobs.json``)."""
...
def snapshot_skills(reason: str = "manual", *, protect_ids: Optional[Set[str]] = None) -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.allr/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.allr/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.allr/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.allr/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `allr curator rollback --list` to see available snapshots)",
            None,
        )
