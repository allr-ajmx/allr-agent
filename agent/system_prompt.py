...
upstream prefix cache warm.  See ``allr-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``ALLR_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("ALLR_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Allr never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want ALLR_HOME persona while keeping
    ...
    # Pointer to the allr-agent skill + docs for user questions about Allr itself.
    stable_parts.append(ALLR_AGENT_HELP_GUIDANCE)
    ...
    # ALLR_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Active-profile hint — names the Allr profile the agent is running
    # under so it doesn't conflate ~/.allr/skills/ (default profile) with
    # ~/.allr/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        ...
        post_workspace_parts.append(
            f"Active Allr profile: {active_profile}. This session reads "
    ...
    if not agent.skip_context_files:
        ...
        # (developing Allr). Every other surface (desktop chat panel,
