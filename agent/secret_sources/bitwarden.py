...
Allr pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.allr/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<hermes_home>/bin/bws`` on
  first use.  Allr pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.allr/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  hammer the API.
* Failures NEVER block Allr startup.  Missing binary, no network,
...
# Disk-persisted cache so back-to-back CLI invocations (e.g. `allr chat -q ...`
...
# never the access token. It's plaintext-equivalent to ~/.allr/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    ...
    callers; falls back to `$ALLR_HOME` / `~/.allr` when home is None.
    """
...
def _hermes_bin_dir() -> Path:
    """Where Allr stores its managed binaries.  Profile-aware."""
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``allr secrets bitwarden setup``
    ...
    *,
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `allr secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `allr secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `allr secrets bitwarden setup` to install."
        )
        return result
    ...
    *fetches* — precedence, override semantics, conflict warnings, and
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            return (
                "Run `allr secrets bitwarden token` to paste a fresh access "
                "token (create one in the Bitwarden web app: Secrets Manager → "
                "Machine accounts → Access tokens).  Wrong region?  Re-run "
                "`allr secrets bitwarden setup` and pick EU/self-hosted."
            )
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`allr secrets bitwarden token`) so the
