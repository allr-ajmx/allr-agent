...
cache problem must never block Allr startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Allr home used for cache paths.

    ``home_path`` is whatever ``load_hermes_dotenv()`` already resolved;
    falling back to ``$ALLR_HOME`` / ``~/.allr`` keeps direct callers
...
class DiskCache(Generic[K]):
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if ALLR_HOME is ever made traversable.
            try:
