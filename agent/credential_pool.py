...
# un-throttled log fires on *every* selection. On Windows several Allr
...
# readiness handshake ("Timed out connecting to Allr backend after
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``allr model`` / ``allr auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Allr process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (e.g. setup wizard, ``allr auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Allr
    ...
    def _single_use_refresh_lock_timeout(self) -> float:
        ...
        resolves.  Reads the provider's ``ALLR_*_REFRESH_TIMEOUT_SECONDS``
        override.
        """
        env_var = (
            "ALLR_CODEX_REFRESH_TIMEOUT_SECONDS"
            if self.provider == "openai-codex"
            else "ALLR_XAI_REFRESH_TIMEOUT_SECONDS"
        )
    ...
    def _refresh_entry_impl(
        ...
        try:
            if self.provider == "anthropic":
                ...
                # refresh_token — single-use tokens consumed by another Allr
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Allr process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Allr profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `allr model` / `allr auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `allr model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``allr auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `allr auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `allr auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Allr PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `allr setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.allr/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `allr setup` writes.
        _env_file = load_env()
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `allr auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (allr auth remove copilot gh_cli) must not pay either on
        ...
        # Allr auth store or env vars, so resolve them here.
        ...
        # MiniMax OAuth tokens live in ~/.allr/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `allr auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `allr auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Allr auth store.  Without this gate the removal is instantly
        ...
        # Allr owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``allr model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``allr auth list`` reflects the logged-in state and so the pool
...
# Prefer ~/.allr/.env over os.environ — the user's config file is the
# authoritative source for Allr credentials. Stale env vars from parent
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `allr auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.allr/.env.
    ...
    if provider == "openrouter":
        # Prefer ~/.allr/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `allr auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Allr
