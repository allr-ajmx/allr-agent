...
Storage
-------
Bundles live in ``~/.allr/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``ALLR_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``allr bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under ALLR_HOME.

    Honors ``ALLR_BUNDLES_DIR`` for tests; falls back to
    ``<ALLR_HOME>/skill-bundles``.
    """
    override = os.environ.get("ALLR_BUNDLES_DIR")
    if override:
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `allr bundles` CLI subcommand.
# ---------------------------------------------------------------------------
