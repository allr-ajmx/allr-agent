...
Data resolution order:
  ...
  2. Disk cache (~/.allr/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Allr ↔ models.dev
# ---------------------------------------------------------------------------

# Allr provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Allr (built lazily)
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``allr config refresh``, the
...
# Gemma models whose TPM quotas are too small for normal Allr agent traffic.
...
def get_model_info(
    ...
    Accepts Allr or models.dev provider ID.  Tries exact match then
