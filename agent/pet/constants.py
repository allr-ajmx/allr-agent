...
row taxonomy is inferred from the atlas shape so Allr can render both legacy
...
# rows (1536x1872), while older Allr/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Allr's activity state names. They are not always identical to the
...
# Legacy Allr/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Allr activity names -> accepted row-name aliases in descending
