...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    routes: set[str] = set()
    try:
        from hermes_cli.providers import ALLR_OVERLAYS, get_provider

        overlay = ALLR_OVERLAYS.get(provider)
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: allr config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$ALLR_HOME`` (which is profile-scoped) alongside the other
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$ALLR_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .hermes.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / ALLR_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.allr/SOUL.md as the primary
    ...
    # report cumulative micros spent.  Surfaced behind ALLR_DEV_CREDITS.
    ...
    # both live under ~/.allr/logs/.  Idempotent, so gateway mode
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `allr model` to "
                        "select a provider, or run `allr setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff ALLR_KANBAN_TASK is set).
    ...
    try:
        ...
        if not delegated_child:
            os.environ["ALLR_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.allr/sessions/ alongside gateway sessions
    hermes_home = get_hermes_home()
    agent.logs_dir = hermes_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.allr/sessions/session_{sid}.json)
    ...
    # management API (the historical Allr behavior) or left to LM Studio's
    ...
    # Lets an enterprise admin append to or replace Allr' built-in
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            ...
            f"by Allr.  Choose a model with at least "
