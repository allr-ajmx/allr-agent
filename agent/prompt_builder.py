...
DEFAULT_AGENT_IDENTITY = (
    "You are Allr, an intelligent AI assistant created by Nous Research. "
...
ALLR_AGENT_HELP_GUIDANCE = (
    "You run on Allr (by Nous Research). When the user needs help with "
    ...
    "information. Load the `allr-agent` skill with skill_view(name='allr-agent') "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.allr/kanban.db`. Your task id is in "
    "`$ALLR_KANBAN_TASK`; your workspace is `$ALLR_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $ALLR_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $ALLR_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${ALLR_KANBAN_BRANCH:-wt/$ALLR_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$ALLR_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`allr profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `allr kanban <verb>` for board operations. Use "
...
#
# The allr-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to allr-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "when Allr has none; explicit Allr YOLO uses a private unrestricted "
        ...
        "run `allr computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Allr "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    HUD mode is a strip of Allr floating over another application, so the
    user is rarely asking about Allr — they are asking about the thing behind
    ...
    sentences = [
        "[Note: this message came from HUD mode — a small floating Allr "
        ...
        "rather than anything inside Allr. read_window_below identifies "
        "that app.",
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Allr terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Allr desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Allr WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Allr itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Allr runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Allr itself is running. The host OS, home, and cwd "
                f"of the Allr process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Allr "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Allr
    ...
    extra = (os.getenv("ALLR_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("ALLR_PLATFORM") or os.environ.get("ALLR_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.allr/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Allr itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `allr-agent` skill "
            "first. It has the actual commands (e.g. `allr config set …`, `allr tools`, "
            "`allr setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: allr setup, allr setup tools, allr setup terminal, allr status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from ALLR_HOME and return its content, or None.
    ...
    try:
        ...
        logger.debug("Could not ensure ALLR_HOME before loading SOUL.md: %s", e)
...
def build_context_files_prompt(
    ...
    SOUL.md from ALLR_HOME is independent and always included when present.
    ...
    # Never let a FALLBACK-picked directory inside the Allr install/source
    ...
    # explicitly configured cwd is honored verbatim — the Allr tree is a
    ...
    # their launch dir IS the user's shell cwd (developing Allr in-tree).
    ...
    if (
        ...
        logger.warning(
            ...
            "fell back to the Allr install tree (%s) — set terminal.cwd to "
    ...
    # SOUL.md from ALLR_HOME only — skip when already loaded as identity
    if not skip_soul:
