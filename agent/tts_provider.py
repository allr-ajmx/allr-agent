...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Allr with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.allr/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Allr
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``allr tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``allr setup`` for
