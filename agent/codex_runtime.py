...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into Allr accounting.
    ...
    Even when Codex omits usage for a turn, Allr should still count that turn
...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in Allr state.

    The app-server owns the compacted thread context, so Allr should not
    ...
    if compressor is not None:
        ...
        if callable(record_boundary):
            # Codex owns this summary. A prior Allr deterministic-fallback
...
# ---------------------------------------------------------------------------
# Codex app-server → Allr UI bridge (#33200)
#
# The codex_app_server runtime hands the entire turn to a subprocess and
# bypasses the normal Allr tool loop. Without this bridge gateway
...
# vision_analyze, ...) since the user thinks of these as Allr tools,
# not as MCP calls.
_INTERNAL_MCP_SERVER = "hermes-tools"


def _codex_item_to_tool_name(item: dict) -> str:
    """Synthetic Allr tool name for a codex item. Mirrors
...
def _codex_item_to_preview(item: dict) -> Any:
    ...
    None when no useful preview is available (Allr' UI tolerates None)."""
    item_type = item.get("type") or ""
    if item_type == "commandExecution":
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into Allr'
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # explicitly opted out of Allr approvals — via `approvals.mode: off`
        # in config, the /yolo session toggle, or --yolo / ALLR_YOLO_MODE —
        # honor that and let codex's own sandbox permission profile
        # (~/.codex/config.toml) be the policy gate instead of double-gating
        # with a missing Allr UI. Defaults (manual/smart/unset) preserve the
