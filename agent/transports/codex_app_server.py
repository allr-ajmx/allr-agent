...
projection into Allr's display, approval bridging, transcript projection into
...
"codex_app_server"`. Allr's default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Allr secret — gateway
        ...
        if spawn_env.get("ALLR_KANBAN_TASK"):
            kanban_db = spawn_env.get("ALLR_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "ALLR_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("ALLR_HOME", os.path.expanduser("~/.allr")),
                        "kanban",
                    ),
                )
            )
