"""Allr-tools-as-MCP server for the codex_app_server runtime.
...
This module exposes a curated subset of those Allr tools to the
spawned codex subprocess via stdio MCP. Codex registers it as a normal
MCP server (per `~/.codex/config.toml [mcp_servers.hermes-tools]`) and
the user gets full Allr capability inside a Codex turn.
...
Scope (what we expose):
  ...
  - kanban_* (complete/block/comment/    — kanban worker + orchestrator
    ...
    unblock/link)                          write ~/.allr/kanban.db)
...
What we DO NOT expose:
  ...
  - delegate_task / memory /             — `_AGENT_LOOP_TOOLS` in Allr
...
# Tools we expose. Each name MUST match a registered Allr tool that
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    "skills_list",
    "text_to_speech",
    # Kanban worker handoff tools — gated on ALLR_KANBAN_TASK env var
    # (set by the kanban dispatcher when spawning a worker). Without these
    # in the callback, a worker spawned with openai_runtime=codex_app_server
    # could do the work but couldn't report completion back to the kernel,
    # making it hang until timeout. Stateless dispatch — they just read
    # the env var and write to ~/.allr/kanban.db.
    "kanban_complete",
    "kanban_block",
    "kanban_comment",
    "kanban_heartbeat",
    ...
    # only — the kanban tool gates them on ALLR_KANBAN_TASK being unset.
...
def _build_server() -> Any:
    """Create the FastMCP server with Allr tools attached. Lazy imports
    ...
    # Discover Allr tools so dispatch works.
    ...
    mcp = FastMCP(
        "hermes-tools",
        instructions=(
            "Allr's tool surface, exposed for use inside a Codex "
    ...
    # Pull authoritative Allr tool schemas for the ones we expose, so
    # MCP clients see the same parameter docs Allr gives the model.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug(
                "skipping %s — not registered in this Allr process", name
            )
            continue

        description = spec.get("description") or f"Allr {name} tool"
