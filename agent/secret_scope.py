...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the ALLR_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Allr runtime / deployment
    "ALLR_HOME", "ALLR_PROFILE", "ALLR_GATEWAY_LOCK_DIR",
    "ALLR_MAX_ITERATIONS", "ALLR_MAX_TOKENS", "ALLR_API_TIMEOUT",
    "ALLR_REDACT_SECRETS", "ALLR_NOUS_TIMEOUT_SECONDS",
    "_ALLR_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "ALLR_KANBAN_DB", "ALLR_KANBAN_WORKSPACES_ROOT", "ALLR_KANBAN_BOARD",
...
_GLOBAL_ENV_PREFIXES = (
    "ALLR_KANBAN_",
    "ALLR_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def load_env_file(env_path: Path) -> Dict[str, str]:
    ...
    ``set_secret_scope``. Parses the small KEY=VALUE subset Allr writes
    ...
    # Parse values with the canonical Allr parser: save_env_value
