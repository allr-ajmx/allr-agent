...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Allr starts and finds ``~/.openclaw/``.

    Points users at ``allr claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``allr claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Allr, run "
        "`allr claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `allr claw cleanup` (renames it to ~/.openclaw.pre-migration — "
