...
Every Allr request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Allr and bucket it by client release.
...
def _hermes_version() -> str:
    """Return the current Allr release version, e.g. ``"0.13.0"``.
...
def conversation_tag(session_id: str) -> str:
    ...
    used for ``~/.allr/sessions/`` storage, session logs, and lineage.
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    appended so Portal usage can be attributed to a specific Allr
    ...
    """
    tags = ["product=allr-agent", hermes_client_tag()]
