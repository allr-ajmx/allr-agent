# Allr Security Policy

This document describes Allr's trust model, names the one
...
security vulnerabilities. **Allr does not operate a bug
bounty program.**
...
- Environment details (`allr version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Allr access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Allr's documentation
...
Allr supports two OS-level isolation postures. They address
...
Allr supports this in two ways:

- **Allr's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Allr's in-process heuristics
...
Allr filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Allr
itself. Bugs in Allr's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Allr does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Allr's own documentation, or reasonable
  operator expectations would predict — including cases where
  Allr has documented a stance about how its output should
...
- Allr includes supply-chain guards for MCP server
