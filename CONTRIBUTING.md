# Contributing to Allr

Thank you for contributing to Allr! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Allr to work everywhere.
...
Related: #38284 covers the agent-side analog — Allr itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Allr install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `allr skills browse` (labeled "official") and install it with `allr skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `allr skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.allr/plugins/` (or via a pip entry point).
...
- Integrate with `allr memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Allr ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.allr/plugins/`, project `.allr/plugins/`, or a pip entry point) — see [Build an Allr Plugin](https://allr.work/docs/guides/build-a-hermes-plugin)
...
```bash
mkdir -p ~/.allr/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.allr/config.yaml
touch ~/.allr/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.allr/.env
```
...
If you used the manual clone fallback, run `./allr` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/allr" ~/.local/bin/allr
```
...
│
├── skills/                   # Bundled skills (copied to ~/.allr/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (allr.work)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.allr/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.allr/`)

| Path | Purpose |
|------|---------|
| `~/.allr/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.allr/.env` | API keys and secrets |
| `~/.allr/auth.json` | OAuth credentials (Nous Portal) |
| `~/.allr/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.allr/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.allr/state.db` | SQLite session database |
| `~/.allr/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.allr/cron/` | Scheduled job data |
| `~/.allr/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.allr/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_ALLR_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
The user may skip setup and keep loading the skill. Allr only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `allr setup` or update `~/.allr/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Allr tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Allr" is the secondary collaborator. If the contributor's commit shows "Allr" as author because they used Allr to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Allr tools (`web_extract`, `terminal`, `read_file`).
...
Allr uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.allr/skins/<name>.yaml`:
...
Allr runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
Allr has terminal access. Security matters.
