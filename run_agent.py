...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = os.environ.get("ALLR_SESSION_SOURCE", "")
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"HermesAgent/{_ALLR_VERSION}",
    }
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Allr-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.allr/sessions/``.  Without
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[allr-agent: tool call arguments were corrupted in this session and "
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``allr chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``ALLR_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``ALLR_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("ALLR_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``ALLR_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("ALLR_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        allr-agent #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See allr-agent#21444 for symptom history."
        )
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Allr against xai-oauth.  Lots of
    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        while current is not None and id(current) not in seen:
            ...
            if any(
                ...
                return (
                    "Allr can't reach the model provider. You may be offline. "
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("ALLR_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _redact_message_content(content):
        ...
        Respects ``ALLR_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.allr/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Allr-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects ALLR_REDACT_SECRETS via
    ...
    def interrupt(self, message: Optional[str] = None, *, hard_cancel: bool = False) -> None:
        ...
        # interrupt event rather than Allr's per-thread flag.
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.allr/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``ALLR_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("ALLR_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
        ...
        if reason == "session_persistence_failed":
            ...
            if cause == "locked":
                return (
                    ...
                    "(another Allr process was writing to the state "
            ...
            return (
                ...
                "Check the state database health (`allr doctor`), then "
    ...
    def _touch_activity(
        ...
        *,
        ...
        process is a dispatcher-spawned worker (ALLR_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        """
        # Dev test fixture (ALLR_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset ALLR_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("ALLR_DEV_CREDITS"))
        ...
        if _dev:
            # ALLR_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `allr logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def close(self) -> None:
        ...
        # a private embedded daemon when Allr YOLO selected unrestricted
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``ALLR_CA_BUNDLE`` settings.  It is passed on the client AND on
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``allr auth add xai-oauth``) whose tokens belong to
        ...
        *,
        ...
        try:
            ...
            creds = resolve_nous_runtime_credentials(
                timeout_seconds=env_float("ALLR_NOUS_TIMEOUT_SECONDS", 15),
                force_refresh=force,
            )
        except Exception as exc:
    ...
    def _try_refresh_env_client_credentials(self) -> bool:
        """Adopt ~/.allr/.env credential/base-url edits at the turn boundary.

        A Settings save (desktop ``PUT /api/env``, ``allr setup``) updates
    ...
    def _compress_context(
        ...
        *,
        ...
        try:
            ...
            # ALLR_SESSION_ID to the child id after an out-of-place
