...
# ============================================================================
# Seed bundled skills into ~/.allr/skills/
# ============================================================================

ALLR_SKILLS_DIR="${ALLR_HOME:-$HOME/.allr}/skills"
mkdir -p "$ALLR_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.allr/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ALLR_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  allr status        # Check configuration"
if is_termux; then
    echo "  allr gateway       # Run gateway in foreground"
else
    echo "  allr gateway install # Install gateway service (messaging + cron)"
fi
echo "  allr cron list     # View scheduled jobs"
echo "  allr doctor        # Diagnose issues"
echo ""
