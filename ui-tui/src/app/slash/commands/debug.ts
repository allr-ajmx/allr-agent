...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $ALLR_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see ALLR_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['ALLR_TUI_BACKGROUND', process.env.ALLR_TUI_BACKGROUND ?? '(unset)'],
            ['ALLR_TUI_THEME', process.env.ALLR_TUI_THEME ?? '(unset)'],
