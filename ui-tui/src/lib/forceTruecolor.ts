...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ALLR_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Allr on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ALLR_TUI_TRUECOLOR=1.
