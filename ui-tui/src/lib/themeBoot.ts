...
// Profile-aware: the Python launcher exports ALLR_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.allr matches get_hermes_home()'s default.
const bootFilePath = () => join(process.env.ALLR_HOME ?? join(homedir(), '.allr'), 'tui-theme-boot.json')

// Never touch the user's real ~/.allr from test runs (the TS suite has no
// ALLR_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into ALLR_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.ALLR_TUI_THEME || env.ALLR_TUI_LIGHT) {
    return result
  }
  ...
  if (
    ...
    !env.ALLR_TUI_BACKGROUND
  ) {
    env.ALLR_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.ALLR_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.ALLR_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into ALLR_TUI_THEME.
