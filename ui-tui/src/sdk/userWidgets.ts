...
/**
 * User widget apps — Allr authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$ALLR_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.allr/plugins/`: files under ALLR_HOME execute
...
const widgetsDir = () => join(process.env.ALLR_HOME?.trim() || join(homedir(), '.allr'), 'tui-widgets')
 ...
 *  every change, so a widget Allr writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Allr) ever writes must hot-load too — a 10s poll here read
